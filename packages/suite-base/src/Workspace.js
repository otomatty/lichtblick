...
import { DataSourceDialog, } from "@lichtblick/suite-base/components/DataSourceDialog";
...
import { StudioLogsSettings, StudioLogsSettingsSidebar, } from "@lichtblick/suite-base/components/StudioLogsSettings";
...
import { useCurrentUser, useCurrentUserType, } from "@lichtblick/suite-base/context/CurrentUserContext";
import { useEvents } from "@lichtblick/suite-base/context/EventsContext";
import { usePlayerSelection } from "@lichtblick/suite-base/context/PlayerSelectionContext";
import { SidebarItemKeys, useWorkspaceStore, } from "@lichtblick/suite-base/context/Workspace/WorkspaceContext";
...
function isInjectedSidebarItem(item) {
    return (SidebarItemKeys.some((itemKey) => itemKey === item[0]) &&
...
const selectPlayerIsPresent = ({ playerState }) => playerState.presence !== PlayerPresence.NOT_PRESENT;
const selectPlayerAlerts = ({ playerState }) => playerState.alerts;
const selectIsPlaying = (ctx) => ctx.playerState.activeData?.isPlaying === true;
const selectPause = (ctx) => ctx.pausePlayback;
const selectPlay = (ctx) => ctx.startPlayback;
const selectSeek = (ctx) => ctx.seekPlayback;
const selectPlayUntil = (ctx) => ctx.playUntil;
const selectPlayerId = (ctx) => ctx.playerState.playerId;
const selectEventsSupported = (store) => store.eventsSupported;
const selectSelectEvent = (store) => store.selectEvent;
const selectWorkspaceDataSourceDialog = (store) => store.dialogs.dataSource;
const selectWorkspaceLeftSidebarItem = (store) => store.sidebars.left.item;
const selectWorkspaceLeftSidebarOpen = (store) => store.sidebars.left.open;
const selectWorkspaceLeftSidebarSize = (store) => store.sidebars.left.size;
const selectWorkspaceRightSidebarItem = (store) => store.sidebars.right.item;
const selectWorkspaceRightSidebarOpen = (store) => store.sidebars.right.open;
const selectWorkspaceRightSidebarSize = (store) => store.sidebars.right.size;
function WorkspaceContent(props) {
    const { PerformanceSidebarComponent } = useAppContext();
    const { classes } = useStyles();
    const containerRef = useRef(ReactNull);
    ...
    const getTimeInfo = useCallback(() => getMessagePipeline().playerState.activeData ?? {}, [getMessagePipeline]);
    ...
    const allowedDropExtensions = useMemo(() => {
        const extensions = [AllowedFileExtensions.FOXE, AllowedFileExtensions.JSON];
    ...
    const [enableDebugMode = false] = useAppConfigurationValue(AppSetting.SHOW_DEBUG_PANELS);
    const { currentUser, signIn } = useCurrentUser();
    const supportsAccountSettings = signIn != undefined;
    const [enableStudioLogsSidebar = false] = useAppConfigurationValue(AppSetting.SHOW_DEBUG_PANELS);
    ...
    const dropHandler = useCallback(async ({ files, handles }) => {
        const filesArray = [];
        if (handles?.length === 1) {
    ...
    const DataSourceSidebarItem = useMemo(() => {
        return function DataSourceSidebarItemImpl() {
            return _jsx(DataSourceSidebar, { disableToolbar: enableNewTopNav });
        };
    }, [enableNewTopNav]);
    const PanelSettingsSidebar = useMemo(() => {
        return function PanelSettingsSidebarImpl() {
            return _jsx(PanelSettings, { disableToolbar: true });
        };
    }, []);
        ...
        const topItems = new Map([
            [
                "connection",
                {
                    ...
                    badge: playerAlerts && playerAlerts.length > 0 ? { count: playerAlerts.length } : undefined,
                },
            ],
        ]);
        if (!enableNewTopNav) {
        ...
        const items = new Map([
            ...
            [
                "alerts",
                {
                    ...
                    badge: playerAlerts && playerAlerts.length > 0
                        ? {
    ...
    const keyboardEventHasModifier = (event) => navigator.userAgent.includes("Mac") ? event.metaKey : event.ctrlKey;
    function AddPanel() {
        const addPanel = useAddPanel();
        const { openLayoutBrowser } = useWorkspaceActions();
        const selectedLayoutId = useCurrentLayoutSelector(selectedLayoutIdSelector);
        const { t: tAddPanel } = useTranslation("addPanel");
        return (_jsx(SidebarContent, { disablePadding: selectedLayoutId != undefined, title: tAddPanel("addPanel"), children: selectedLayoutId == undefined ? (_jsx(Typography, { color: "text.secondary", children: _jsx(Trans, { t: tAddPanel, i18nKey: "noLayoutSelected", components: {
                        selectLayoutLink: _jsx(Link, { onClick: openLayoutBrowser }),
                    } }) })) : (_jsx(PanelCatalog, { mode: "list", onPanelSelect: addPanel })) }));
    }
    function ExtensionsSidebar() {
        return (_jsx(SidebarContent, { title: "Extensions", disablePadding: true, children: _jsx(ExtensionsSettings, {}) }));
    }
    const keyDownHandlers = useMemo(() => {
        return {
            ...
            o: (ev) => {
                ...
                void dialogActions.openFile.open().catch((err) => {
                    console.error(err);
                });
            },
        };
    ...
    const [unappliedSourceArgs, setUnappliedSourceArgs] = useState(targetUrlState ? { ds: targetUrlState.ds, dsParams: targetUrlState.dsParams } : undefined);
    ...
    const [unappliedTime, setUnappliedTime] = useState(targetUrlState ? { time: targetUrlState.time } : undefined);
    // Seek to time in URL.
    useEffect(() => {
    ...
    useSeekTimeFromCLI();
    const appBar = useMemo(() => (_jsx(AppBarComponent, { leftInset: props.appBarLeftInset, onDoubleClick: props.onAppBarDoubleClick, showCustomWindowControls: props.showCustomWindowControls, isMaximized: props.isMaximized, initialZoomFactor: props.initialZoomFactor, onMinimizeWindow: props.onMinimizeWindow, onMaximizeWindow: props.onMaximizeWindow, onUnmaximizeWindow: props.onUnmaximizeWindow, onCloseWindow: props.onCloseWindow })), [
        AppBarComponent,
    ...
    return (_jsxs(PanelStateContextProvider, { children: [dataSourceDialog.open && _jsx(DataSourceDialog, {}), _jsx(DocumentDropListener, { onDrop: dropHandler, allowedExtensions: allowedDropExtensions }), _jsx(SyncAdapters, {}), _jsx(KeyListener, { global: true, keyDownHandlers: keyDownHandlers }), _jsxs("div", { className: classes.container, ref: containerRef, tabIndex: 0, children: [appBar, _jsxs(Sidebars, { selectedKey: "", onSelectKey: () => { }, items: sidebarItems, leftItems: leftSidebarItems, bottomItems: sidebarBottomItems, selectedLeftKey: leftSidebarOpen ? leftSidebarItem : undefined, onSelectLeftKey: sidebarActions.left.selectItem, leftSidebarSize: leftSidebarSize, setLeftSidebarSize: sidebarActions.left.setSize, rightItems: rightSidebarItems, selectedRightKey: rightSidebarOpen ? rightSidebarItem : undefined, onSelectRightKey: sidebarActions.right.selectItem, rightSidebarSize: rightSidebarSize, setRightSidebarSize: sidebarActions.right.setSize, children: [_jsx(RemountOnValueChange, { value: playerId, children: _jsx(Stack, { children: _jsx(PanelLayout, {}) }) }), _jsx("div", { style: { flexShrink: 0 }, children: _jsx(PlaybackControls, { play: play ?? (() => { }), pause: pause ?? (() => { }), seek: seek ?? (() => { }), playUntil: playUntil, isPlaying: isPlaying, getTimeInfo: getTimeInfo }) })] })] }), _jsx(WorkspaceDialogs, {})] }));
}
export default function Workspace(props) {
    const [showOpenDialogOnStartup = true] = useAppConfigurationValue(AppSetting.SHOW_OPEN_DIALOG_ON_STARTUP);
    const { workspaceStoreCreator } = useAppContext();
    const isPlayerPresent = useMessagePipeline(selectPlayerIsPresent);
    const initialItem = isPlayerPresent || !showOpenDialogOnStartup ? undefined : "start";
    const initialState = {
        dialogs: {
            dataSource: {
    ...
    return (_jsx(WorkspaceContextProvider, { initialState: initialState, workspaceStoreCreator: workspaceStoreCreator, disablePersistenceForStorybook: props.disablePersistenceForStorybook, children: _jsx(WorkspaceContent, { ...props }) }));
}
