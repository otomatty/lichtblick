 ...
 * @since 2023
 */
export function ColorSchemeThemeProvider({ children }) {
    // アプリケーション設定からカラースキーム設定を取得
    // デフォルトは "system"（システム設定に従う）
    const [colorScheme = "system"] = useAppConfigurationValue(AppSetting.COLOR_SCHEME);
    // システムのダークモード設定を監視
    ...
    return _jsx(ThemeProvider, { isDark: isDark, children: children });
}
