...
import { useClearHoverValue, useSetHoverValue, useTimelineInteractionState, } from "@lichtblick/suite-base/context/TimelineInteractionStateContext";
...
export default function TimeBasedChart(props) {
    ...
    const { currentTime, data, provider, typedData, typedProvider, dataBounds, datasetId, defaultView, height, isSynced = false, resetButtonPaddingBottom = 4, showXAxisLabels, type, width, xAxes, xAxisIsPlaybackTime, yAxes, } = props;
    // データセット境界の状態管理
    ...
    // ダウンサンプリング機能の初期化
    const { downsampler, setScales } = useDownsample(React.useMemo(() => filterMap(data?.datasets ?? [], (dataset) => {
        const { label } = dataset;
    ...
    const canvasContainer = useRef(ReactNull);
    const [hasUserPannedOrZoomed, setHasUserPannedOrZoomed] = useState(false);
    const pauseFrame = useMessagePipeline(useCallback((messagePipeline) => messagePipeline.pauseFrame, []));
    const resumeFrame = useRef();
    const requestedResumeFrame = useRef();
    ...
    const syncedGlobalBounds = useMemo(() => (isSynced ? globalBounds : undefined), [globalBounds, isSynced]);
    // some callbacks don't need to re-create when the current scales change, so we keep a ref
    const currentScalesRef = useRef(undefined);
    const onResetZoom = () => {
        setHasUserPannedOrZoomed(false);
        setGlobalBounds(undefined);
    };
    const [hasVerticalExclusiveZoom, setHasVerticalExclusiveZoom] = useState(false);
    const [hasBothAxesZoom, setHasBothAxesZoom] = useState(false);
    ...
    const mouseYRef = useRef(undefined);
    ...
    const updateTooltip = useCallback((elements) => {
    ...
    const onHover = useCallback((elements) => {
    ...
    const onScalesUpdate = useCallback((scales, { userInteraction }) => {
        if (!isMounted()) {
            return;
        }
    ...
    const colorsByDatasetIndex = useMemo(() => {
        ...
        return Object.fromEntries(datasets.map((dataset, index) => [index, JSON.stringify(dataset.borderColor)]));
    }, [datasets]);
    ...
    const tooltipContent = useMemo(() => {
        return activeTooltip ? (_jsx(TimeBasedChartTooltipContent, { content: activeTooltip.data, multiDataset: datasetsLength > 1, colorsByConfigIndex: colorsByDatasetIndex, labelsByConfigIndex: labelsByDatasetIndex })) : undefined;
    ...
    return (_jsx(Stack, { direction: "row", fullWidth: true, fullHeight: true, children: _jsx(Tooltip, { arrow: false, classes: { tooltip: classes.tooltip }, open: activeTooltip != undefined, placement: "right", title: tooltipContent ?? _jsx(_Fragment, {}), disableInteractive: true, followCursor: true, TransitionComponent: Fade, TransitionProps: { timeout: 0 }, children: _jsx(Stack, { direction: "row", style: { width }, children: _jsxs("div", { className: classes.root, onDoubleClick: onResetZoom, children: [_jsx(HoverBar, { componentId: componentId, isPlaybackSeconds: xAxisIsPlaybackTime, scales: currentScalesRef.current, children: _jsx("div", { className: classes.bar, style: {
                                    backgroundColor: xAxisIsPlaybackTime
                                        ? theme.palette.warning.main
                                        : theme.palette.info.main,
                                } }) }), xAxisIsPlaybackTime && (_jsx(VerticalBarWrapper, { scales: currentScalesRef.current, xValue: currentTime, children: _jsx("div", { className: cx(classes.bar, classes.playbackBar) }) })), _jsx("div", { ref: canvasContainer, onMouseMove: onMouseMove, onMouseOut: onMouseOut, children: _jsx(ChartComponent, { ...chartProps }) }), showReset && (_jsx("div", { className: classes.resetZoomButton, style: { paddingBottom: theme.spacing(resetButtonPaddingBottom) }, children: _jsx(Button, { variant: "contained", color: "inherit", title: "(shortcut: double-click)", onClick: onResetZoom, children: "Reset view" }) })), _jsx(KeyListener, { global: true, keyDownHandlers: keyDownHandlers, keyUpHandlers: keyUphandlers })] }) }) }) }));
}
