...
import { MINIMUM_PIXEL_DISTANCE, downsampleTimeseries, initDownsample, continueDownsample, finishDownsample, downsampleScatter, } from "./downsample";
describe("downsampleTimeseries", () => {
    ...
    it("merges nearby points", () => {
        const result = downsampleTimeseries(iterateObjects([
            { x: 0, y: 0, value: 0 },
            { x: 10, y: 0, value: 0 },
            { x: 20, y: 0, value: 0 },
            { x: 20, y: 1, value: 1 },
    ...
    it("correctly pauses and resumes downsampling", () => {
        ...
        const numSplits = 400;
        const [indices, finalState] = R.reduce((a, v) => {
        ...
        }, [[], initDownsample(realBounds)], R.splitEvery(Math.trunc(numPoints / numSplits), dataset));
...
describe("downsampleScatter", () => {
    ...
    it("ignores out of bounds points", () => {
        const result = downsampleScatter(iterateObjects([
            { x: -1, y: 0, value: 0 },
            { x: 0, y: -1, value: -1 },
            { x: 200, y: 0, value: 0 },
            { x: 0, y: 200, value: 200 },
