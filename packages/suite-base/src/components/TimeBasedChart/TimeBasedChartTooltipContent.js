...
function OverflowMessage() {
    ...
    return _jsx("div", { className: classes.overflow, children: "<multiple values under cursor>" });
}
/**
 ...
 */
export default function TimeBasedChartTooltipContent(props) {
    const { colorsByConfigIndex: colorsByDatasetIndex, content, labelsByConfigIndex: labelsByDatasetIndex, multiDataset, } = props;
    ...
    if (!multiDataset) {
        ...
        const value = typeof tooltip.value === "string"
            ...
            : typeof tooltip.value === "bigint"
                ? tooltip.value.toString()
                : JSON.stringify(tooltip.value);
        return (_jsxs(Stack, { className: classes.root, "data-testid": "TimeBasedChartTooltipContent", children: [_jsxs("div", { children: [value, tooltip.constantName != undefined ? ` (${tooltip.constantName})` : ""] }), content.length > 1 && _jsx(OverflowMessage, {})] }));
    }
    // 複数データセットモード: グリッドレイアウト
    // アイコン・ラベル・値の3列構成
    return (_jsx("div", { className: cx(classes.root, classes.grid), "data-testid": "TimeBasedChartTooltipContent", children: sortedItems.map(([datasetIndex, item], idx) => {
            ...
            const value = typeof tooltip.value === "string"
                ...
                : typeof tooltip.value === "bigint"
                    ? tooltip.value.toString()
                    : JSON.stringify(tooltip.value);
            return (_jsxs(Fragment, { children: [color ? (_jsx(Square12Filled, { className: classes.icon, primaryFill: color })) : (_jsx("span", { className: classes.colorIconReplacement })), _jsx("div", { className: classes.path, children: label ?? "" }), _jsxs("div", { className: classes.value, children: [value, tooltip.constantName != undefined ? ` (${tooltip.constantName})` : ""] }), item.hasMultipleValues && _jsx(OverflowMessage, {})] }, idx));
        }) }));
}
