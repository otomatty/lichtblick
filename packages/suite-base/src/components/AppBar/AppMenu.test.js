...
describe("AppMenu", () => {
    ...
    beforeEach(() => {
        ...
        usePlayerSelection.mockReturnValue({
            recentSources: [{ id: "1", title: "Recent Source 1" }],
            selectRecent: mockSelectRecent,
        });
        useWorkspaceStore.mockImplementation((selector) => selector({
    ...
    const renderAppMenu = (props = {}) => render(_jsx(AppMenu, { open: true, handleClose: mockHandleClose, disablePortal: false, ...props }));
    ...
    it("opens documentation link in Help menu", () => {
        ...
        expect(window.open).toHaveBeenCalledWith(LICHTBLICK_DOCUMENTATION_LINK, "_blank", "noopener,noreferrer");
