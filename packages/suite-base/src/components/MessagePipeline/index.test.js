...
import { MessagePipelineProvider, useMessagePipeline } from ".";
...
async function doubleAct(fn) {
    let promise;
    act(() => void (promise = fn()));
    await act(async () => await promise);
}
function makeTestHook({ player }) {
    const all = [];
    function Hook() {
    ...
    function Wrapper({ children }) {
        ...
        return (_jsx(AppConfigurationContext.Provider, { value: config, children: _jsx(MockCurrentLayoutProvider, { children: _jsx(MessagePipelineProvider, { player: currentPlayer, children: children }) }) }));
    }
...
describe("MessagePipelineProvider/useMessagePipeline", () => {
    ...
    it("updates datatypes when player datatypes change", async () => {
        ...
        expect(all[2].datatypes).toEqual(new Map([
            ["Foo", { definitions: [] }],
            ["Bar", { definitions: [] }],
        ]));
    });
        ...
        await doubleAct(async () => {
            await player.emit({
                activeData: {
                    ...
                    topicStats: new Map([["/input/foo", { numMessages: 1 }]]),
    ...
    it("renders with the same callback functions every time", async () => {
        ...
        for (const [key, value] of Object.entries(result.current)) {
            if (typeof value === "function") {
                expect(lastContext[key]).toBe(value);
            }
        }
    });
    ...
    describe("pauseFrame", () => {
        ...
        it("can wait for promises multiple frames in a row", async () => {
            ...
            const { Hook, Wrapper } = makeTestHook({ player });
            const { result } = renderHook(Hook, { wrapper: Wrapper });
            async function runSingleFrame({ shouldPause }) {
                let resumeFn;
                if (shouldPause) {
