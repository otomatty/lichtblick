 ...
 * ```
 */
export function makeSubscriptionMemoizer() {
    return moize((val) => val, { isDeepEqual: true, maxSize: Infinity });
}
/**
 ...
 * ```
 */
function mergeSubscription(a, b) {
    const isAllFields = a.fields == undefined || b.fields == undefined;
    const fields = R.pipe(R.chain((payload) => payload.fields ?? []), R.map((v) => v.trim()), R.filter((v) => v.length > 0), R.uniq)([a, b]);
    return {
        ...a,
 ...
 * ```
 */
function denormalizeSubscriptions(subscriptions) {
    return R.pipe(R.groupBy((v) => v.topic), R.values, 
    /**
 ...
 * ```
 */
export function mergeSubscriptions(subscriptions) {
    return R.pipe(
    /**
     ...
     * プリロードタイプ別に分離して処理
     */
    R.partition((v) => v.preloadType === "full"), ([full, partial]) => [...denormalizeSubscriptions(full), ...denormalizeSubscriptions(partial)])(subscriptions);
}
