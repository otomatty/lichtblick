 ...
 */
export const ContextInternal = createContext(undefined);
/**
 ...
 * ```
 */
export function useMessagePipelineGetter() {
 ...
 * ```
 */
export function useMessagePipeline(selector) {
    const store = useGuaranteedContext(ContextInternal);
    return useStore(store, useCallback((state) => selector(state.public), [selector]));
}
/**
 ...
 */
const selectSubscriptions = (state) => state.public.subscriptions;
/**
 ...
 * ```
 */
export function MessagePipelineProvider({ children, player }) {
    /**
     ...
     */
    useEffect(() => debouncedPlayerSetSubscriptions(subscriptions), [debouncedPlayerSetSubscriptions, subscriptions]);
    /**
     * メッセージフレームレート制御
     * 設定値が60未満の場合、メッセージパイプラインのフレームレートを制限
     */
    const [messageRate] = useAppConfigurationValue(AppSetting.MESSAGE_RATE);
    /**
     * レイアウト完了通知
     * レンダリング完了をリスナーに通知する
     */
    ...
    useEffect(() => {
         ...
         */
        let lastGlobalVariablesInstance = currentLayoutContext?.actions.getCurrentLayoutState().selectedLayout?.data?.globalVariables ??
            EMPTY_GLOBAL_VARIABLES;
        player?.setGlobalVariables(lastGlobalVariablesInstance);
        /**
         * レイアウト状態更新時のグローバル変数同期
         */
        const onLayoutStateUpdate = (state) => {
 ...
 */
function concatAlerts(origState, alerts) {
    if (alerts.length === 0) {
        return origState;
    }
    return {
...
function createPlayerListener(args) {
     ...
     * 4. 状態更新
     */
    const listener = async (listenerPlayerState) => {
        if (closed) {
            return;
        }
        if (resolveFn) {
