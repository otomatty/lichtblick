...
import { fillInGlobalVariablesInPath, getMessagePathDataItems, useCachedGetMessagePathDataItems, useDecodeMessagePathsForMessagesByTopic, } from "./useCachedGetMessagePathDataItems";
function addValuesWithPathsToItems(messages, messagePath, providerTopics, datatypes) {
...
describe("useCachedGetMessagePathDataItems", () => {
    it("supports types with reference cycles (i.e. reference themselves within their children)", () => {
        const topics = [{ name: "/topic", schemaName: "datatype" }];
        ...
        const { result } = renderHook(() => useCachedGetMessagePathDataItems(["/topic", "/topic.self"]), {
            wrapper: function Wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { children: _jsx(MockMessagePipelineProvider, { topics: topics, datatypes: datatypes, children: children }) }));
            },
        });
        const message = {
            topic: "/topic",
    ...
    describe("getMessagePathDataItems", () => {
        it("traverses down the path for every item", () => {
            ...
            const topics = [{ name: "/some/topic", schemaName: "some_datatype" }];
            const datatypes = new Map(Object.entries({
                some_datatype: {
        ...
        it("returns items even when a schema is not available", () => {
            ...
            const topics = [{ name: "/some/topic", schemaName: undefined }];
            const datatypes = new Map();
            expect(addValuesWithPathsToItems(messages, "/some/topic.some_num", topics, datatypes)).toEqual([
                [
                    {
                        value: 1,
        ...
        it("handles fields inside times", () => {
            const topics = [{ name: "/some/topic", schemaName: "std_msgs/Header" }];
            const datatypes = new Map(Object.entries({
            ...
            expect(addValuesWithPathsToItems(messages, "/some/topic.stamp.nsec", topics, datatypes)).toEqual([[{ constantName: undefined, path: "/some/topic.stamp.nsec", value: 2 }]]);
        });
        it("filters properly, and uses the filter name in the path", () => {
            ...
            expect(addValuesWithPathsToItems(messages, "/some/topic.some_array[:]{some_filter_value==0}.some_id", topics, datatypes)).toEqual([
                [
                    {
                        constantName: undefined,
                        value: 10,
        ...
        it("filters entire messages", () => {
            ...
            expect(addValuesWithPathsToItems(messages, "/some/topic{str_field=='B'}.num_field", topics, datatypes)).toEqual([[], [], [{ value: 2, path: "/some/topic{str_field=='B'}.num_field" }]]);
            ...
            expect(addValuesWithPathsToItems(messages, "/some/topic{str_field=='A'}{num_field==2}.num_field", topics, datatypes)).toEqual([
                [],
        ...
        it("filters correctly with bigints", () => {
            ...
            expect(addValuesWithPathsToItems(messages, "/some/topic{num_field==18446744073709551616}.num_field", topics, datatypes)).toEqual([
                [
                    {
            ...
            expect(addValuesWithPathsToItems(messages, "/some/topic{num_field==18446744073709552020}.num_field", topics, datatypes)).toEqual([
                [],
                [
                    {
...
describe("fillInGlobalVariablesInPath", () => {
    ...
    it("fills in global variables in slices", () => {
        expect(fillInGlobalVariablesInPath({
            topicName: "/foo",
            topicNameRepr: "/foo",
            messagePath: [
