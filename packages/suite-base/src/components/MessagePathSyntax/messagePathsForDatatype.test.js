...
import { traverseStructure, messagePathsForStructure, messagePathStructures, validTerminatingStructureItem, } from "./messagePathsForDatatype";
...
describe("messagePathStructures", () => {
    ...
    it("supports types which reference themselves", () => {
        const selfReferencingDatatypes = new Map(Object.entries({
            "some.type": {
...
describe("messagePathsForStructure", () => {
    ...
    it("returns all possible message paths when not passing in `validTypes`", () => {
        expect(messagePathsForStructure(unwrap(structures["pose_msgs/PoseDebug"])).map(({ path }) => path)).toEqual([
            "",
            ".header",
            ".header.frame_id",
            ".header.seq",
        ...
        expect(messagePathsForStructure(unwrap(structures["msgs/Log"])).map(({ path }) => path)).toEqual(["", ".id"]);
        expect(messagePathsForStructure(unwrap(structures["tf/tfMessage"])).map(({ path }) => path)).toEqual([
            "",
            ".transforms",
        ...
        expect(messagePathsForStructure(unwrap(structures["visualization_msgs/MarkerArray"])).map(({ path }) => path)).toEqual(["", ".markers", ".markers[:]{id==0}", ".markers[:]{id==0}.id"]);
    });
    it("returns an array of possible message paths for the given `validTypes`", () => {
        expect(messagePathsForStructure(unwrap(structures["pose_msgs/PoseDebug"]), {
            validTypes: ["float64"],
        }).map(({ path }) => path)).toEqual([".some_pose.dummy_array[:]", ".some_pose.x"]);
    });
    it("does not suggest hashes with multiple values when setting `noMultiSlices`", () => {
        expect(messagePathsForStructure(unwrap(structures["pose_msgs/PoseDebug"]), {
            validTypes: ["float64"],
            noMultiSlices: true,
        }).map(({ path }) => path)).toEqual([".some_pose.dummy_array[0]", ".some_pose.x"]);
    });
    it("preserves existing filters matching isTypicalFilterName", () => {
        expect(messagePathsForStructure(unwrap(structures["tf/tfMessage"]), {
...
describe("validTerminatingStructureItem", () => {
    ...
    it("works for structureType", () => {
        ...
        expect(validTerminatingStructureItem({ structureType: "message", nextByName: {}, datatype: "" }, [
            "message",
        ])).toEqual(true);
        expect(validTerminatingStructureItem({ structureType: "message", nextByName: {}, datatype: "" }, [
            "array",
    ...
    it("works for time and duration", () => {
        expect(validTerminatingStructureItem({ structureType: "message", nextByName: {}, datatype: "time" }, ["time"])).toEqual(true);
        expect(validTerminatingStructureItem({ structureType: "message", nextByName: {}, datatype: "duration" }, ["duration"])).toEqual(true);
    });
    it("works for primitiveType", () => {
        expect(validTerminatingStructureItem({ structureType: "primitive", primitiveType: "uint32", datatype: "" }, ["uint32"])).toEqual(true);
    });
});
describe("traverseStructure", () => {
    ...
    it("returns whether the path is valid for the structure, plus some metadata", () => {
        ...
        // Valid:
        expect(traverseStructure(structure, [
            { type: "name", name: "some_pose", repr: "some_pose" },
            { type: "name", name: "x", repr: "x" },
        ])).toEqual({
            valid: true,
            msgPathPart: undefined,
            structureItem: (structure?.nextByName.some_pose).nextByName.x,
        });
        expect(traverseStructure(structure, [
            { type: "name", name: "some_pose", repr: "some_pose" },
        ...
        expect(traverseStructure(structure, [{ type: "name", name: "some_pose", repr: "some_pose" }])).toEqual({
            valid: true,
            msgPathPart: undefined,
            structureItem: structure?.nextByName.some_pose,
        });
