 ...
 * ```
 */
export default function useMessagesByPath(paths, historySize = Infinity) {
    const memoizedPaths = useShallowMemo(paths);
    // メッセージパスからサブスクリプションペイロードを生成
    const subscribeTopics = useMemo(() => filterMap(memoizedPaths, (path) => subscribePayloadFromMessagePath(path)), [memoizedPaths]);
    // 必要なトピックのメッセージを取得
    ...
    return useMemo(() => decodeMessagePathsForMessagesByTopic(messagesByTopic), [decodeMessagePathsForMessagesByTopic, messagesByTopic]);
}
