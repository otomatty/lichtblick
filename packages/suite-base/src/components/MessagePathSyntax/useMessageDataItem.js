...
import { useCachedGetMessagePathDataItems, } from "./useCachedGetMessagePathDataItems";
/**
 ...
 * ```
 */
export function useMessageDataItem(path, options) {
     ...
     * @returns 更新された状態値
     */
    const addMessages = useCallback((prevValue, messageEvents) => {
     ...
     * @returns 復元された状態値
     */
    const restore = useCallback((prevValue) => {
        if (!prevValue) {
            return {
                matches: [],
                messageEvents: [],
    ...
    const reducedValue = useMessageReducer({
        topics,
        addMessages,
        restore,
    });
