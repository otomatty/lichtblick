...
import { enumValuesByDatatypeAndField, extractTypeFromStudioEnumAnnotation, } from "@lichtblick/suite-base/util/enums";
 ...
 * ```
 */
export function useCachedGetMessagePathDataItems(paths) {
    ...
    const parsedPaths = useMemo(() => {
        return filterMap(memoizedPaths, (path) => {
            ...
            return rosPath ? ([path, rosPath]) : undefined;
        });
    }, [memoizedPaths]);
    ...
    // 列挙値のマッピング
    const enumValues = useMemo(() => enumValuesByDatatypeAndField(relevantDatatypes), [relevantDatatypes]);
    return useCallback((path, message) => {
        ...
        const messagePathDataItems = getMessagePathDataItems(message, filledInPath, topicsByName, structures, enumValues);
 ...
 * ```
 */
export function getMessagePathDataItems(message, filledInPath, topicsByName, structures, enumValues) {
    ...
    value, pathIndex, path, structureItem) {
        if (value == undefined) {
            return;
        }
 ...
 * ```
 */
export function useDecodeMessagePathsForMessagesByTopic(paths) {
    const memoizedPaths = useShallowMemo(paths);
    ...
    return useCallback((messagesByTopic) => {
        const obj = {};
