...
describe("filterMatches", () => {
    ...
    describe("value matching", () => {
        ...
        it("returns error when filter value is an object", () => {
            ...
            expect(() => filterMatches(filter, { a: BasicBuilder.number() })).toThrow(new Error("filterMatches only works on paths where global variables have been filled in"));
        });
    ...
    describe("operator matching", () => {
        it.each([
            ...
            const filter = setup({ value: filterValue, operator: operator });
            expect(filterMatches(filter, { a: testValue })).toBe(expectedResult);
        });
        it("returns false for invalid operator", () => {
            const filter = setup({ value: BasicBuilder.number(), operator: "invalid" });
            expect(filterMatches(filter, { a: BasicBuilder.number() })).toBe(false);
        });
    });
});
