...
describe("useMessageDataItem", () => {
    ...
    it("uses the latest message", async () => {
        ...
        const { result, rerender } = renderHook(({ path }) => useMessageDataItem(path), {
            initialProps: { path: "/topic.value" },
            wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { children: _jsx(MockMessagePipelineProvider, { messages: messages, topics: topics, datatypes: datatypes, children: children }) }));
            },
        });
        expect(result.current).toEqual([
    ...
    it("only keeps messages that match the path", async () => {
        const { result } = renderHook(({ path }) => useMessageDataItem(path), {
            ...
            wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { children: _jsx(MockMessagePipelineProvider, { messages: fixtureMessages, topics: topics, datatypes: datatypes, children: children }) }));
            },
        });
        expect(result.current).toEqual([
            {
    ...
    it("restores previously received message when topics and datatypes becomes available", async () => {
        ...
        const { result, rerender } = renderHook(({ path }) => useMessageDataItem(path), {
            initialProps,
            wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { children: _jsx(MockMessagePipelineProvider, { messages: fixtureMessages, topics: rosTopics, datatypes: rosDatatypes, children: children }) }));
            },
        });
        expect(result.current).toEqual([]);
    ...
    it("clears previous messages on topic change", async () => {
        const { result, rerender } = renderHook(({ path }) => useMessageDataItem(path, { historySize: 2 }), {
            initialProps: { path: "/topic.value" },
            wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { children: _jsx(MockMessagePipelineProvider, { messages: fixtureMessages, topics: topics, datatypes: datatypes, children: children }) }));
            },
