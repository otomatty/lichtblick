...
import { quoteTopicNameIfNeeded, parseMessagePath, } from "@lichtblick/message-path";
...
import { traverseStructure, messagePathStructures, messagePathsForStructure, validTerminatingStructureItem, } from "./messagePathsForDatatype";
/**
 ...
 * ```
 */
export function tryToSetDefaultGlobalVar(variableName, setGlobalVariables) {
    const defaultGlobalVars = new Map();
 ...
 * ```
 */
export function getFirstInvalidVariableFromRosPath(rosPath, globalVariables, setGlobalVariables) {
 ...
 * ```
 */
function getExamplePrimitive(primitiveType) {
 ...
 */
export default React.memo(function MessagePathInput(props) {
    const { globalVariables, setGlobalVariables } = useGlobalVariables();
    const { datatypes, topics } = PanelAPI.useDataSourceInfo();
    const { supportsMathModifiers, path, prioritizedDatatype, validTypes, placeholder, noMultiSlices, inputStyle, disableAutocomplete = false, variant = "standard", } = props;
    const { classes } = useStyles();
    // データ型からメッセージパス構造を生成（メモ化）
    const messagePathStructuresForDataype = useMemo(() => messagePathStructures(datatypes), [datatypes]);
    /**
     * 各メッセージパスと対応するMessagePathStructureItemのマップ
     * 自動補完候補の生成に使用される
     */
    const allStructureItemsByPath = useMemo(() => new Map(topics.flatMap((topic) => {
     ...
     * フィルター条件の入力を支援する。
     */
    const onChange = useCallback((event, rawValue) => {
     ...
     * 必要に応じてカーソル位置を調整する。
     */
    const onSelect = useCallback((rawValue, autocomplete, autocompleteType, autocompleteRange) => {
        ...
        const isSimpleField = completedField != undefined && completedField.structureType === "primitive";
        ...
        const keepGoingAfterTopicName = autocompleteType === "topicName" && !messageIsValidType && !isSimpleField;
    ...
    const structureTraversalResult = useMemo(() => {
        ...
        return traverseStructure(messagePathStructuresForDataype[topic.schemaName], rosPath.messagePath);
    ...
    // トピック名の自動補完候補を生成
    const topicNamesAutocompleteItems = useMemo(() => topics.map(({ name }) => quoteTopicNameIfNeeded(name)), [topics]);
    // トピック名とフィールドパスの自動補完候補を生成
    const topicNamesAndFieldsAutocompleteItems = useMemo(() => topicNamesAutocompleteItems.concat(Array.from(allStructureItemsByPath.keys())), [allStructureItemsByPath, topicNamesAutocompleteItems]);
    // 現在の自動補完タイプを決定
    const autocompleteType = useMemo(() => {
        ...
        else if (structureTraversalResult == undefined ||
    ...
    const { autocompleteItems, autocompleteFilterText, autocompleteRange } = useMemo(() => {
        if (disableAutocomplete) {
            ...
            if (structureTraversalResult &&
                ...
                const initialFilterLength = rosPath.messagePath[0]?.type === "filter" ? rosPath.messagePath[0].repr.length + 2 : 0;
                const structure = topic.schemaName != undefined ? structures[topic.schemaName] : undefined;
                return {
                    autocompleteItems: structure == undefined
                        ? []
                        : filterMap(messagePathsForStructure(structure, {
                            validTypes,
                            noMultiSlices,
            ...
            return {
                ...
                autocompleteRange: {
                    ...
                    end: invalidGlobalVariablesVariable.loc +
    ...
    const orderedAutocompleteItems = useMemo(() => {
        ...
        return _.flatten(_.partition(autocompleteItems, (item) => topicsByName[item]?.schemaName === prioritizedDatatype));
    }, [autocompleteItems, prioritizedDatatype, topicsByName]);
    // サポートされていない数学修飾子の使用をチェック
    const usesUnsupportedMathModifier = (supportsMathModifiers == undefined || !supportsMathModifiers) && path.includes(".@");
    // エラー状態を判定
    const hasError = usesUnsupportedMathModifier ||
        (autocompleteType != undefined && !disableAutocomplete && path.length > 0);
    return (_jsx(Autocomplete, { className: classes.root, variant: variant, items: orderedAutocompleteItems, disabled: props.disabled, readOnly: props.readOnly, filterText: autocompleteFilterText, value: path, onChange: onChange, onSelect: (value, autocomplete) => {
            onSelect(value, autocomplete, autocompleteType, autocompleteRange);
        }, hasError: hasError, placeholder: placeholder != undefined && placeholder !== "" ? placeholder : "/some/topic.msgs[0].field", inputStyle: inputStyle, 
