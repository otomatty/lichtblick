...
import MessagePathInput, { tryToSetDefaultGlobalVar, getFirstInvalidVariableFromRosPath, } from "./MessagePathInput";
...
jest.mock("@lichtblick/suite-base/services/LayoutManager/LayoutManager", () => jest.fn(() => new MockLayoutManager()));
...
describe("getFirstInvalidVariableFromRosPath", () => {
    it("returns all possible message paths when not passing in `validTypes`", () => {
        ...
        expect(setGlobalVars).not.toHaveBeenCalled();
        expect(getFirstInvalidVariableFromRosPath(rosPath, { not_yet_set_global_var: 5 }, setGlobalVars)).toEqual(undefined);
        expect(setGlobalVars).not.toHaveBeenCalled();
    });
});
describe("MessagePathInput Component", () => {
    ...
    useMessagePipeline.mockReturnValue({});
    useUserProfileStorage.mockReturnValue({
        getUserProfile: {},
    ...
    const renderComponent = (propsOverride = {}) => {
        ...
        const ui = (_jsx(MockPanelContextProvider, { children: _jsx(MessagePathInput, { ...props }) }));
        return {
            ...render(ui),
