...
import { quoteFieldNameIfNeeded, } from "@lichtblick/message-path";
 ...
 * ```
 */
function structureItemIsIntegerPrimitive(item) {
    return (item.structureType === "primitive" && STRUCTURE_ITEM_INTEGER_TYPES.includes(item.primitiveType));
}
/**
 ...
 * ```
 */
export function messagePathStructures(datatypes) {
    const structureFor = _.memoize((datatype, seenDatatypes) => {
        const nextByName = {};
        ...
        for (const msgField of rosDatatype.definitions) {
            ...
            const next = isPrimitiveType(msgField.type)
                ? {
                    structureType: "primitive",
 ...
 * ```
 */
export function validTerminatingStructureItem(structureItem, validTypes) {
 ...
 * ```
 */
export function messagePathsForStructure(structure, { validTypes, noMultiSlices, messagePath = [], } = {}) {
    let clonedMessagePath = [...messagePath];
    const messagePaths = [];
    function traverse(structureItem, builtString) {
        ...
        if (structureItem.structureType === "message") {
            ...
            if (structureItem.next.structureType === "message") {
                ...
                const typicalFilterItem = Object.entries(structureItem.next.nextByName).find(([name]) => isTypicalFilterName(name));
                if (typicalFilterItem) {
                    ...
                    const matchingFilterPart = clonedMessagePath.find((pathPart) => pathPart.type === "filter" && pathPart.path[0] === typicalFilterName);
                    ...
                    else if (typicalFilterValue.structureType === "primitive" &&
...
export const traverseStructure = (initialStructureItem, messagePath) => {
    ...
    for (const msgPathPart of messagePath) {
        ...
        if (msgPathPart.type === "name") {
            ...
            const next = structureItem.nextByName[msgPathPart.name];
            structureItem = next;
        }
