...
const singleTopic = [{ name: "/some/topic", schemaName: "some/datatype" }];
function queriedMessage(index) {
    return {
        messageEvent: fixture.messages[index],
        queriedData: [{ value: fixture.messages[index].message, path: "/some/topic" }],
    };
}
function makeMessagePipelineWrapper(initialProps) {
    const setSubscriptions = jest.fn();
    let topics = initialProps?.topics ?? [];
    let datatypes = initialProps?.datatypes ?? new Map();
    let messages = initialProps?.messages ?? [];
    let activeData = initialProps?.activeData;
    const wrapper = ({ children }) => (_jsx(MockCurrentLayoutProvider, { initialState: { globalVariables: initialProps?.globalVariables }, children: _jsx(MockMessagePipelineProvider, { topics: topics, datatypes: datatypes, messages: messages, setSubscriptions: setSubscriptions, activeData: activeData, children: children }) }));
    return {
        setSubscriptions,
        wrapper,
        setWrapperProps: ({ topics: newTopics, datatypes: newDatatypes, messages: newMessages, activeData: newActiveData, }) => {
...
describe("useMessagesByPath", () => {
    ...
    describe("global variables in paths", () => {
        const exampleDatatypes = new Map(Object.entries({
            "dtype/Foo": {
