...
const useStyles = makeStyles()((theme) => {
    ...
    return {
        toolbar: {
            ...
            backgroundImage: `linear-gradient(to top, transparent, ${palette.background.paper} ${spacing(1.5)}) !important`,
        },
        toolbarMenu: {
            backgroundImage: `linear-gradient(to top, transparent, ${palette.background.menu} ${spacing(1.5)}) !important`,
        },
        toolbarGrid: {
    ...
    for (const panel of panels) {
        ...
        if (existingPanel) {
            ...
            if (bothHaveEmptyConfigs) {
                ...
                throw new Error(`Two components have the same panelType ('${type}') and no preset configs: ${otherDisplayName} and ${dispName}`);
            }
        }
        panelTypes.set(type, panel);
    }
}
export const PanelCatalog = forwardRef(function PanelCatalog(props, ref) {
    ...
    const [highlightedPanelIdx, setHighlightedPanelIdx] = useState();
    ...
    const { allRegularPanels, allPreconfiguredPanels } = useMemo(() => {
        ...
        const sortByTitle = (a, b) => a.title.localeCompare(b.title, undefined, { ignorePunctuation: true, sensitivity: "base" });
        return {
    ...
    const getFilteredPanels = useCallback((panels) => {
    ...
    const allFilteredPanels = useMemo(() => [...filteredPreconfiguredPanels, ...filteredRegularPanels], [filteredPreconfiguredPanels, filteredRegularPanels]);
    ...
    return (_jsxs(Stack, { fullHeight: true, ref: ref, children: [_jsx("div", { className: cx(classes.toolbar, {
                    [classes.toolbarMenu]: isMenu,
                    [classes.toolbarGrid]: mode === "grid",
                }), children: _jsx(TextField, { fullWidth: true, variant: "filled", placeholder: t("searchPanels"), value: searchQuery, onChange: handleSearchChange, onKeyDown: onKeyDown, autoFocus: true, "data-testid": "panel-list-textfield", InputProps: {
                        startAdornment: (_jsx(InputAdornment, { position: "start", children: _jsx(SearchIcon, { fontSize: "small" }) })),
                        endAdornment: searchQuery && (_jsx(IconButton, { size: "small", edge: "end", onClick: () => {
                                setSearchQuery("");
                            }, children: _jsx(CancelIcon, { fontSize: "small" }) })),
                    } }) }), mode === "grid" ? (_jsx(PanelGrid, { searchQuery: searchQuery, filteredPanels: allFilteredPanels, onPanelSelect: onPanelSelect })) : (_jsx(PanelList, { searchQuery: searchQuery, filteredPanels: allFilteredPanels, selectedPanelType: selectedPanelType, highlightedPanelIdx: highlightedPanelIdx, onDragStart: onDragStart, onPanelSelect: onPanelSelect })), noResults && (_jsx(Stack, { padding: 2, children: _jsx(EmptyState, { children: t("noPanelsMatchSearchCriteria") }) }))] }));
});
