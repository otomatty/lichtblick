...
const useExtensionSettings = () => {
    const [undebouncedFilterText, setUndebouncedFilterText] = useState("");
    ...
    const [marketplaceEntries, refreshMarketplaceEntries] = useAsyncFn(async () => await marketplace.getAvailableExtensions(), [marketplace]);
    const marketplaceMap = useMemo(() => _.keyBy(marketplaceEntries.value ?? [], (entry) => entry.id), [marketplaceEntries]);
    ...
    const namespacedEntries = useMemo(() => _.groupBy(installedEntries, (entry) => entry.namespace), [installedEntries]);
    useEffect(() => {
