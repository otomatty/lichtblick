...
describe("renderState", () => {
    function makeBlock(topic, receiveTime) {
        return {
            topic,
            schemaName: "schema",
            receiveTime,
    ...
    it("should run the correct converter", () => {
        ...
        expect(converter1).toHaveBeenCalledWith("raw-message", expect.objectContaining({
            message: "raw-message",
            schemaName: "srcschema",
            topic: "another",
        }));
    ...
    it("should deliver new allframes when a second converter from the same topic is enabled", () => {
        ...
        const state2 = buildRenderState(produce(initialState, (draft) => {
    ...
    it("should renderStateField when activeData contains parameters", async () => {
        ...
        const activeData = PlayerBuilder.activeData({
            parameters: BasicBuilder.genericMap(BasicBuilder.string),
        });
