...
describe("SyncInstanceToggle", () => {
    const useWorkspaceActionsMock = useWorkspaceActions;
    const useAppConfigurationValueMock = useAppConfigurationValue;
    const useWorkspaceStoreMock = useWorkspaceStore;
    ...
    it("returns null and disables sync if config is false", () => {
        ...
        useWorkspaceStoreMock.mockImplementation((selector) => selector({ playbackControls: { syncInstances: true } }));
        // WHEN
        const { container } = render(_jsx(SyncInstanceToggle, {}));
        // THEN
    ...
    it("renders button with correct text when sync is on", () => {
        ...
        useWorkspaceStoreMock.mockImplementation((selector) => selector({ playbackControls: { syncInstances: true } }));
        // WHEN
        render(_jsx(SyncInstanceToggle, {}));
        // THEN
    ...
    it("renders button with correct text when sync is off", () => {
        // GIVEN
        useAppConfigurationValueMock.mockReturnValue([true]);
        useWorkspaceStoreMock.mockImplementation((selector) => selector({ playbackControls: { syncInstances: false } }));
        // WHEN
        render(_jsx(SyncInstanceToggle, {}));
        // THEN
        expect(screen.getByText("Sync")).toBeInTheDocument();
    ...
    it("toggles sync state on button click (turn on)", () => {
        ...
        useAppConfigurationValueMock.mockReturnValue([true]);
        useWorkspaceStoreMock.mockImplementationOnce((selector) => selector({ playbackControls: { syncInstances: false } }));
    ...
    it("toggles sync state on button click (turn off)", () => {
        ...
        useAppConfigurationValueMock.mockReturnValue([true]);
        useWorkspaceStoreMock.mockImplementationOnce((selector) => selector({ playbackControls: { syncInstances: true } }));
        // WHEN user clicks the button
        render(_jsx(SyncInstanceToggle, {}));
        fireEvent.click(screen.getByRole("button"));
