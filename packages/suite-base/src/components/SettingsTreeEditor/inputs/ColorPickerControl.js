...
export function ColorPickerControl(props) {
    const { alphaType, onChange, onEnterKey, swatchColor, updatePrefixedColor, editedValueIsInvalid, editedValue, updateEditedValue, onInputBlur, } = props;
    const { classes, theme } = useStyles();
    return (_jsxs(Stack, { padding: 1.5, gap: 1, children: [alphaType === "alpha" ? (_jsx(HexAlphaColorPicker, { className: classes.picker, color: swatchColor, onChange: updatePrefixedColor })) : (_jsx(HexColorPicker, { className: classes.picker, color: swatchColor, onChange: updatePrefixedColor })), _jsx(TextField, { size: "small", error: editedValueIsInvalid, InputProps: {
                    onFocus: (event) => {
                        event.target.select();
                    },
                    role: "input",
                    startAdornment: _jsx(TagIcon, { fontSize: "small" }),
                    style: { fontFamily: theme.typography.fontMonospace },
                }, placeholder: alphaType === "alpha" ? "RRGGBBAA" : "RRGGBB", value: editedValue, onKeyDown: (event) => event.key === "Enter" && onEnterKey?.(), onChange: (event) => {
                    onChange(event.target.value);
                    updateEditedValue(event.target.value);
                }, onBlur: onInputBlur })] }));
}
const hexMatcher = /^#?([0-9A-F]{3,8})$/i;
function isValidHexColor(color, alphaType) {
...
export function useColorPickerControl(props) {
    ...
    const displayValue = alphaType === "alpha" ? parsedValue?.toHex8String() : parsedValue?.toHexString();
    ...
    const updateColor = useDebouncedCallback((newValue) => {
        onChange(`#${newValue}`);
