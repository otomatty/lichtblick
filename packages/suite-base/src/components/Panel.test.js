...
describe("Panel", () => {
    ...
    it("saves defaultConfig when there is no saved config", async () => {
        ...
        const actions = [];
        render(_jsx(PanelSetup, { onLayoutAction: (action) => actions.push(action), children: _jsx(DummyPanel, { childId: childId }) }));
    ...
    it("gets the config from the store", () => {
        ...
        const actions = [];
        render(_jsx(PanelSetup, { fixture: { savedProps: { [childId]: { someString } } }, onLayoutAction: (action) => actions.push(action), children: _jsx(DummyPanel, { childId: childId }) }));
        expect(renderFn.mock.calls).toEqual([
    ...
    it("merges saved config with defaultConfig when defaultConfig has new keys", async () => {
        ...
        const childId = "Dummy!1my2ydk";
        const actions = [];
        render(_jsx(PanelSetup, { fixture: { savedProps: { [childId]: { someNumber: 42 } } }, onLayoutAction: (action) => actions.push(action), children: _jsx(DummyPanel, { childId: childId }) }));
        expect(renderFn.mock.calls).toEqual([
    ...
    it("does not re-save configs when defaultConfig has fewer keys than saved config", async () => {
        ...
        const someString = "someNewString";
        const actions = [];
        render(_jsx(PanelSetup, { fixture: { savedProps: { [childId]: { someNumber: 42, someString } } }, onLayoutAction: (action) => actions.push(action), children: _jsx(DummyPanel, { childId: childId }) }));
        expect(renderFn.mock.calls).toEqual([
    ...
    it("does not rerender when another panel changes", () => {
        ...
        const { result: actions } = renderHook(() => useCurrentLayoutActions(), {
            wrapper({ children }) {
                return (_jsxs(PanelSetup, { children: [children, _jsx(DummyPanel, { childId: childId })] }));
            },
        });
