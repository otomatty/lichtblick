 ...
 */
const AutoSizingCanvas = ({ draw, overrideDevicePixelRatioForTest, }) => {
    // リサイズ検出の設定
    // デバウンス処理と0のリフレッシュレートにより、リサイズ処理中に新たなリサイズ監視が
    // トリガーされることを防ぐ
    // 参考: https://github.com/maslianok/react-resize-detector/issues/45
    const { width, height, ref: canvasRef, } = useResizeDetector({
        refreshRate: 0,
    ...
    return (_jsx("canvas", { ref: canvasRef, width: actualWidth, height: actualHeight, style: { width: "100%", height: "100%" } }));
};
