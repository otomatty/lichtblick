...
export default function DocumentDropListener(props) {
    ...
    const onDrop = useCallback(async (ev) => {
        ...
        for (const item of Array.from(dataItems)) {
            ...
            if (window.isSecureContext && "getAsFileSystemHandle" in item) {
                ...
                log.info("getAsFileSystemHandle not available on a dropped item. Features requiring handles will not be available for the item", item);
            }
        ...
        const filteredHandles = handles?.filter((handle) => allowedExtensions.includes(extname(handle.name)));
    ...
    return (_jsxs(_Fragment, { children: [_jsx("input", { type: "file", style: { display: "none" }, onChange: (event) => {
                    if (event.target.files) {
                        props.onDrop?.({ files: Array.from(event.target.files) });
                    }
                }, "data-puppeteer-file-upload": true, multiple: true }), _jsx(DropOverlay, { open: hovering, children: "Drop a file here" })] }));
}
