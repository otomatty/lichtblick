...
describe("UnconnectedPanelLayout", () => {
    ...
    it("does not remount panels when changing split percentage", async () => {
        ...
        const { rerender, unmount } = render(_jsx(UnconnectedPanelLayout, { layout: { first: "a", second: "b", direction: "row", splitPercentage: 50 }, onChange: onChange }), {
            wrapper: function Wrapper({ children }) {
                const [config] = useState(() => makeMockAppConfiguration());
                return (_jsx(DndProvider, { backend: HTML5Backend, children: _jsx(WorkspaceContextProvider, { children: _jsx(AppConfigurationContext.Provider, { value: config, children: _jsx(MockCurrentLayoutProvider, { children: _jsx(MessagePipelineProvider, { children: _jsx(PanelStateContextProvider, { children: _jsx(ExtensionCatalogProvider, { loaders: [], children: _jsx(PanelCatalogContext.Provider, { value: panelCatalog, children: children }) }) }) }) }) }) }) }));
            },
        });
        await waitFor(() => {
        ...
        rerender(_jsx(UnconnectedPanelLayout, { layout: { first: "a", second: "c", direction: "row", splitPercentage: 40 }, onChange: onChange }));
        await waitFor(() => {
