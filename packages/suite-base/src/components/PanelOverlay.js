 ...
 * @since 2023
 */
import { Backdrop, Button, Chip, ClickAwayListener, Paper, buttonClasses, } from "@mui/material";
 ...
 * @since 2023
 */
export const PanelOverlay = forwardRef(function PanelOverlay(props, ref) {
    const { actions, variant, highlightMode, dropMessage, open, onClickAway } = props;
    const { classes, cx } = useStyles();
    return (_jsx(ClickAwayListener, { onClickAway: onClickAway ?? _.noop, children: _jsx(Backdrop, { transitionDuration: 0, unmountOnExit // 非表示時はDOMから削除
            : true, ref: ref, open: open, className: cx(classes.backdrop, {
            ...
            }), children: dropMessage && variant === "validDropTarget" ? (_jsx(Chip, { size: "small", color: "primary", label: dropMessage, className: classes.chip })) : (
            /* アクションボタン群表示 */
            actions && (_jsx("div", { className: classes.buttonGroup, children: actions.map((action) => (_jsx(Paper, { square: false, elevation: 0, className: classes.buttonPaper, children: _jsx(Button, { fullWidth: true, variant: "outlined", className: classes.button, onClick: action.onClick, startIcon: action.icon, color: action.color, children: _jsx("div", { className: classes.buttonText, children: action.text }) }) }, action.key))) }))) }) }));
});
