...
import { Fzf, basicMatch } from "fzf";
import * as _ from "lodash-es";
import { useMemo } from "react";
import { getMessagePathSearchItems } from "./getMessagePathSearchItems";
/**
 * トピックをFZF結果形式に変換するヘルパー関数
 * @param item - 変換するトピック
 * @returns FZF結果形式のトピック（スコア0、ハイライト位置なし）
 */
function topicToFzfResult(item) {
    return {
        item,
        score: 0,
        positions: new Set(),
 ...
 */
export function useTopicListSearch(params) {
    const { topics, datatypes, filterText } = params;
    // トピック名とスキーマ名での検索用FZFインスタンス
    const topicsAndSchemaNamesFzf = useMemo(() => new Fzf(topics, {
        selector: (item) => `${item.name}|${item.schemaName}`,
    }), [topics]);
    // メッセージパス検索項目の生成
    const messagePathSearchItems = useMemo(() => getMessagePathSearchItems(topics, datatypes), [topics, datatypes]);
    // メッセージパス検索用FZFインスタンス
    const messagePathsFzf = useMemo(() => new Fzf(messagePathSearchItems.items, {
        selector: (item) => item.fullPath,
        // カスタムマッチャー: トピック名のみのマッチを除外
        match(query) {
            const results = basicMatch.call(this, query);
    ...
    // フィルタリングされたトピック結果
    const filteredTopics = useMemo(() => (filterText ? topicsAndSchemaNamesFzf.find(filterText) : topics.map(topicToFzfResult)), [filterText, topics, topicsAndSchemaNamesFzf]);
    // メッセージパス検索結果
    const messagePathResults = useMemo(() => (filterText ? messagePathsFzf.find(filterText) : []), [filterText, messagePathsFzf]);
    // 最終的なツリー項目の構築
    const treeItems = useMemo(() => {
        const results = [];
        // メッセージパス結果をトピック名でグループ化
        const messagePathResultsByTopicName = _.groupBy(messagePathResults, (item) => item.item.topic.name);
        // 表示すべき全トピックの収集（直接マッチまたはパスマッチを含む）
        const allTopicsToShowByName = new Map();
        const matchedTopicsByName = new Map();
        // 直接マッチしたトピックを追加
        ...
        for (const { item: { topic }, score, } of messagePathResults) {
