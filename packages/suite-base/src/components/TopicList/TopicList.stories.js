...
export default {
    ...
    render: (args) => (_jsx(DndProvider, { backend: HTML5Backend, children: _jsx(MockMessagePipelineProvider, { ...args, children: _jsx(TopicList, {}) }) })),
};
async function findAllByTextContent(canvasElement, str, count) {
    return await waitFor(async () => {
        const items = await within(canvasElement).findAllByText((_content, element) => element instanceof HTMLSpanElement && element.textContent === str);
...
export const EmptyChinese = {
    args: { topics: [] },
    parameters: { forceLanguage: "zh" },
};
export const EmptyJapanese = {
    args: { topics: [] },
    parameters: { forceLanguage: "ja" },
};
export const ContextMenuSingleTopic = {
...
export const ContextMenuMultipleTopics = {
    play: async ({ canvasElement }) => {
...
export const ContextMenuSinglePath = {
    play: async ({ canvasElement }) => {
...
export const ContextMenuMultiplePaths = {
    play: async ({ canvasElement }) => {
        const canvas = within(canvasElement);
        const filterInputs = await canvas.findAllByTestId("topic-filter");
        for (const input of filterInputs) {
...
export const FilterByTopicName = {
    play: async ({ canvasElement }) => {
        const canvas = within(canvasElement);
        const filterInputs = await canvas.findAllByTestId("topic-filter");
        for (const input of filterInputs) {
...
export const FilterBySchemaName = {
    play: async ({ canvasElement }) => {
        const canvas = within(canvasElement);
        const filterInputs = await canvas.findAllByTestId("topic-filter");
        for (const input of filterInputs) {
...
export const FilterByMessagePath = {
    play: async ({ canvasElement }) => {
        const canvas = within(canvasElement);
        const filterInputs = await canvas.findAllByTestId("topic-filter");
        for (const input of filterInputs) {
...
export const FilterTextCleared = {
    play: async ({ canvasElement }) => {
