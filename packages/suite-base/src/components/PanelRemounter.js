 ...
 */
export function PanelRemounter({ children, id, tabId }) {
    /**
     ...
     * @returns パネルのシーケンス番号
     */
    const selector = useCallback((store) => store.sequenceNumbers[id] ?? 0, [id]);
    /**
     * 現在のパネルのシーケンス番号
     *
     ...
     * - シーケンス番号が変更された時（設定変更）
     */
    return _jsx(Fragment, { children: children }, `${id}${tabId ?? ""}${sequenceNumber}`);
}
