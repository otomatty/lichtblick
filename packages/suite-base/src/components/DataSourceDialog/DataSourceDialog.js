 ...
 */
export function DataSourceDialog(props) {
    ...
    const backdrop = useMemo(() => {
        ...
        if (backdropAnimation === false) {
            ...
            // 12月25日以降は雪エフェクト
            return _jsx(Snow, { effect: "snow" });
        }
        else if (now < new Date(now.getFullYear(), 0, 2)) {
            // 1月2日まではコンフェッティエフェクト
            return _jsx(Snow, { effect: "confetti" });
        }
        return;
    }, [backdropAnimation]);
    /**
        ...
        switch (activeView) {
            ...
            case "connection":
                ...
                return {
                    ...
                    component: _jsx(Connection, {}),
                };
            default:
                // デフォルトはスタート画面
                return {
                    title: "Get started",
                    component: _jsx(Start, {}),
                };
        }
    }, [activeView]);
    return (_jsxs(Dialog, { "data-testid": "DataSourceDialog", open: true, onClose: onModalClose, fullWidth: true, maxWidth: "lg", slotProps: {
            backdrop: {
        ...
        }, children: [_jsx(IconButton, { className: classes.closeButton, onClick: onModalClose, edge: "end", children: _jsx(CloseIcon, { "data-testid": "CloseIcon" }) }), _jsx(Stack, { flexGrow: 1, fullHeight: true, justifyContent: "space-between", overflow: activeView === "connection" ? "hidden" : undefined, children: view.component })] }));
}
