...
describe("SidebarItems", () => {
    ...
    beforeEach(() => {
        ...
        useAppConfigurationValue.mockReturnValue([true, jest.fn()]);
        jest.clearAllMocks();
    });
    it("renders items for unauthenticated users", () => {
        useCurrentUser.mockReturnValue({ currentUserType: "unauthenticated" });
        render(_jsx(SidebarItems, { onSelectView: mockOnSelectView }));
    ...
    it("renders items for authenticated-free users", () => {
        useCurrentUser.mockReturnValue({ currentUserType: "authenticated-free" });
        render(_jsx(SidebarItems, { onSelectView: mockOnSelectView }));
    ...
    it("renders items for authenticated-team users", () => {
        useCurrentUser.mockReturnValue({ currentUserType: "authenticated-team" });
        render(_jsx(SidebarItems, { onSelectView: mockOnSelectView }));
        expect(screen.getByText("newToLichtblick")).toBeInTheDocument();
    ...
    it("opens external links correctly", () => {
        ...
        expect(windowOpenSpy).toHaveBeenCalledWith(LICHTBLICK_DOCUMENTATION_LINK, "_blank", "noopener,noreferrer");
    });
});
