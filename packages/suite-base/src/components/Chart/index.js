...
import ChartJsMux from "@lichtblick/suite-base/components/Chart/worker/ChartJsMux";
 ...
 * @returns React JSX要素
 */
const supportsOffscreenCanvas = typeof HTMLCanvasElement.prototype.transferControlToOffscreen === "function";
function Chart(props) {
    ...
    const { type, data, typedData, isBoundsReset, options, width, height, onStartRender, onFinishRender, } = props;
    const sendWrapperRef = useRef();
    const rpcSendRef = useRef();
    const hasPannedSinceMouseDown = useRef(false);
    const queuedUpdates = useRef([]);
    const isSending = useRef(false);
    const previousUpdateMessage = useRef({});
    useLayoutEffect(() => {
        ...
        const sendWrapper = async (topic, payload, transferables) => {
            return await rpc.send(topic, { id, ...payload }, transferables);
        };
    ...
    const maybeUpdateScales = useCallback((newScales, opt) => {
        if (!isMounted()) {
            return;
        }
    ...
    // more than one.
    const flushUpdates = useCallback(async (send) => {
        ...
        while (queuedUpdates.current.length > 0) {
            ...
            onStartRender?.();
            const scales = await send("update", coalesced);
    ...
    const updateChart = useCallback(async (update) => {
        ...
        onStartRender?.();
        const offscreenCanvas = typeof canvas.transferControlToOffscreen === "function"
        ...
        queueMicrotask(async () => {
            ...
            const scales = await sendWrapperRef.current("initialize", {
    ...
    useLayoutEffect(() => {
        ...
        updateChart(newUpdate).catch((err) => {
            if (isMounted()) {
                setUpdateError(err);
            }
            console.error(err);
    ...
    useLayoutEffect(() => {
        ...
        hammerManager.on("panstart", async (event) => {
            ...
            await rpcSendRef.current("panstart", {
                event: {
                    cancelable: false,
                    deltaY: event.deltaY,
                    deltaX: event.deltaX,
        ...
        hammerManager.on("panmove", async (event) => {
            ...
            const boundingRect = event.target.getBoundingClientRect();
            const scales = await rpcSendRef.current("panmove", {
                event: {
                    cancelable: false,
                    deltaY: event.deltaY,
                    deltaX: event.deltaX,
        ...
        hammerManager.on("panend", async (event) => {
            ...
            const boundingRect = event.target.getBoundingClientRect();
            const scales = await rpcSendRef.current("panend", {
                event: {
                    cancelable: false,
                    deltaY: event.deltaY,
                    deltaX: event.deltaX,
    ...
    const onWheel = useCallback(async (event) => {
        if (!zoomEnabled || !rpcSendRef.current) {
            return;
        }
        const boundingRect = event.currentTarget.getBoundingClientRect();
        const scales = await rpcSendRef.current("wheel", {
            event: {
                cancelable: false,
                deltaY: event.deltaY,
                deltaX: event.deltaX,
    ...
    const onMouseDown = useCallback(async (event) => {
        hasPannedSinceMouseDown.current = false;
        if (!rpcSendRef.current) {
            return;
        }
        const scales = await rpcSendRef.current("mousedown", {
            event: rpcMouseEvent(event),
        });
        maybeUpdateScales(scales);
    }, [maybeUpdateScales]);
    const onMouseUp = useCallback(async (event) => {
        if (!rpcSendRef.current) {
            return;
        }
    ...
    const onMouseMove = useCallback(async (event) => {
        ...
        const elements = await rpcSendRef.current("getElementsAtEvent", {
            event: rpcMouseEvent(event),
        });
    ...
    return (_jsx("div", { ref: containerRef, onWheel: onWheel, onClick: onClick, onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseLeave: onMouseLeave, onMouseEnter: onMouseEnter, onMouseUp: onMouseUp, style: { width, height, cursor: "crosshair", position: "relative" } }));
}
