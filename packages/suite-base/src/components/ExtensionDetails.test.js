...
describe("ExtensionDetails Component", () => {
    ...
    const mockExtension = ExtensionBuilder.extensionMarketplaceDetail({
        namespace: "local",
    });
    beforeEach(() => {
        useSnackbar.mockReturnValue({ enqueueSnackbar: mockEnqueueSnackbar });
        useAnalytics.mockReturnValue({ logEvent: mockLogEvent });
        useExtensionCatalog.mockImplementation((selector) => {
        ...
        useExtensionMarketplace.mockReturnValue({
    ...
    it("renders the extension details correctly", () => {
        render(_jsx(ExtensionDetails, { extension: mockExtension, onClose: () => { }, installed: false }));
    ...
    it("calls onClose when the back button is clicked", () => {
        ...
        render(_jsx(ExtensionDetails, { extension: mockExtension, onClose: mockOnClose, installed: false }));
    ...
    describe("download and install process", () => {
        it("handles the download and install process successfully on desktop app", async () => {
            ...
            await waitFor(() => {
                ...
                expect(mockEnqueueSnackbar).toHaveBeenCalledWith(`${mockExtension.name} installed successfully`, { variant: "success" });
        ...
        it("displays an error message when not on desktop app", async () => {
            ...
            fireEvent.click(installButton);
            await waitFor(() => {
                expect(mockEnqueueSnackbar).toHaveBeenCalledWith("Download the desktop app to use marketplace extensions.", { variant: "error" });
        ...
        it("displays readme correctly", async () => {
            ...
            render(_jsx(ExtensionDetails, { extension: mockExtension, onClose: () => { }, installed: true }));
        ...
        it("displays an error message when the download and install process fails on desktop app", async () => {
            ...
            fireEvent.click(installButton);
            await waitFor(() => {
                expect(mockEnqueueSnackbar).toHaveBeenCalledWith(`Failed to install extension ${mockExtension.id}. Download failed`, { variant: "error" });
            });
        });
    });
    it("handles the uninstall process successfully", async () => {
        ...
        await waitFor(() => {
            expect(mockUninstallExtension).toHaveBeenCalledWith(mockExtension.namespace, mockExtension.id);
            expect(mockEnqueueSnackbar).toHaveBeenCalledWith(`${mockExtension.name} uninstalled successfully`, { variant: "success" });
    ...
    it("displays an error message when the uninstall process fails", async () => {
        ...
        fireEvent.click(uninstallButton);
        await waitFor(() => {
            expect(mockEnqueueSnackbar).toHaveBeenCalledWith(`Failed to uninstall extension ${mockExtension.id}. Uninstall failed`, { variant: "error" });
        });
    });
});
