 ...
 */
export const VerticalBars = React.memo(function VerticalBars({ coordinator, hoverComponentId, xAxisIsPlaybackTime, }) {
    ...
    const latestCurrentTimeSinceStart = useRef();
    const latestXScale = useRef();
    const currentTimeBarRef = useRef(ReactNull);
    const hoverBarRef = useRef(ReactNull);
    const updateBars = useCallback(() => {
        if (!currentTimeBarRef.current || !hoverBarRef.current) {
            return;
        }
        const currentTimePixel = getPixelForXValue(latestXScale.current, latestCurrentTimeSinceStart.current);
        const hoverValuePixel = getPixelForXValue(latestXScale.current, latestHoverValue.current?.value);
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { "data-testid": "vertical-bar", ref: currentTimeBarRef, className: cx(classes.verticalBar, classes.playbackBar) }), _jsx("div", { "data-testid": "hover-bar", ref: hoverBarRef, className: cx(classes.verticalBar), style: {
