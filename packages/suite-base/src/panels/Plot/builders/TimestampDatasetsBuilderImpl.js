...
import { MAX_POINTS, downsampleScatter, downsampleTimeseries, } from "@lichtblick/suite-base/components/TimeBasedChart/downsample";
...
const MAX_CURRENT_DATUMS_PER_SERIES = 50000;
const compareDatum = (a, b) => a.x - b.x;
export class TimestampDatasetsBuilderImpl {
    ...
    getViewportDatasets(viewport) {
        ...
        for (const series of this.#seriesByKey.values()) {
            ...
            const xBounds = { min: Number.MAX_VALUE, max: Number.MIN_VALUE };
            const yBounds = { min: Number.MAX_VALUE, max: Number.MIN_VALUE };
            ...
            const downsampledIndices = items.length < min
    ...
    applyAction(action) {
        switch (action.type) {
            ...
            case "append-current": {
                ...
                const cullSize = Math.max(0, series.current.length + action.items.length - MAX_CURRENT_DATUMS_PER_SERIES);
                ...
                const sorted = series.config.timestampMethod === "headerStamp"
    ...
    #updateSeriesConfigAction(series) {
