...
export class CustomDatasetsBuilderImpl {
    ...
    getViewportDatasets(viewport) {
        const datasets = [];
        const pathsWithMismatchedDataLengths = new Set();
        for (const series of this.#seriesByKey.values()) {
            ...
            if (this.#xValues.full.length !== series.full.length ||
    ...
    #applyAction(action) {
        switch (action.type) {
            ...
            case "append-current-x": {
                ...
                const cullSize = Math.max(0, this.#xValues.current.length + action.items.length - MAX_CURRENT_DATUMS_PER_SERIES);
