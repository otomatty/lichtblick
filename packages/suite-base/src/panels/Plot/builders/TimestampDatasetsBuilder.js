...
import { getTimestampForMessage } from "@lichtblick/suite-base/util/time";
 ...
 * downsampled data.
 */
export class TimestampDatasetsBuilder {
    ...
    handlePlayerState(state) {
        ...
        if (msgEvents.length > 0) {
            for (const series of this.#series) {
                ...
                const pathItems = readMessagePathItems(msgEvents, series.config.parsed, series.config.timestampMethod, activeData.startTime, mathFn);
    ...
    async handleBlocks(startTime, blocks, progress) {
        ...
        do {
            ...
            for (const series of seriesArr) {
                ...
                const pathItems = readMessagePathItems(messageEvents, series.config.parsed, series.config.timestampMethod, startTime, mathFn);
...
function readMessagePathItems(events, path, timestampMethod, startTime, mathFunction) {
    const out = [];
