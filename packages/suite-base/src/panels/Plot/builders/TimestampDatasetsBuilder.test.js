...
function buildSeriesItems(paths) {
...
function buildPlayerState(activeDataOverride, blocks) {
    return {
        activeData: {
            messages: [],
...
describe("TimestampDatasetsBuilder", () => {
    it("should process current messages into a dataset", async () => {
        ...
        builder.setSeries(buildSeriesItems([
            {
                enabled: true,
        ...
        builder.handlePlayerState(buildPlayerState({
            messages: [
                {
                    topic: "/foo",
                    schemaName: "foo",
        ...
        await expect(builder.getViewportDatasets({
    ...
    it("should create a discontinuity between current and full", async () => {
        ...
        await builder.handleBlocks(unwrap(playerState.activeData?.startTime), unwrap(playerState.progress.messageCache?.blocks), async () => await Promise.resolve(false));
        await expect(builder.getViewportDatasets({
            size: { width: 1000, height: 1000 },
            bounds: {},
        })).resolves.toEqual({
    ...
    it("should cull current messages after threshold is reached", async () => {
        ...
        const messages = new Array(60000).fill(1).map((_val, idx) => {
            return {
                topic: "/foo",
                schemaName: "foo",
        ...
        {
            builder.handlePlayerState(buildPlayerState({
                messages: messages.slice(0, 40000),
            }));
            const result = await builder.getViewportDatasets({
                size: { width: 100000, height: 100000 },
                bounds: {},
            });
            expect(result.datasetsByConfigIndex[0].data.length).toEqual(40000);
            expect(result.datasetsByConfigIndex[0].data[0]).toEqual({ x: 0, y: 0, value: 0 });
            expect(result.datasetsByConfigIndex[0].data[39999]).toEqual({
                x: 39999,
                y: 39999,
                value: 39999,
            });
        ...
        {
            builder.handlePlayerState(buildPlayerState({
                messages: messages.slice(40000, 60000),
            }));
            const result = await builder.getViewportDatasets({
                size: { width: 100000, height: 100000 },
                bounds: {},
            ...
            expect(result.datasetsByConfigIndex[0].data[37499]).toEqual({
                x: 59999,
                y: 59999,
                value: 59999,
            });
