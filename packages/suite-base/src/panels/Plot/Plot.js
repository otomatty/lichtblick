...
import { useMessagePipelineGetter, useMessagePipelineSubscribe, } from "@lichtblick/suite-base/components/MessagePipeline";
...
const Plot = (props) => {
    ...
    const { paths: series, showLegend, xAxisVal: xAxisMode, legendDisplay = config.showSidebar === true ? "left" : "floating", sidebarDimension = config.sidebarWidth ?? DEFAULT_SIDEBAR_DIMENSION, } = config;
    ...
    const [canvasDiv, setCanvasDiv] = useState(ReactNull);
    ...
    const { onMouseMove, onMouseOut, onResetView, onWheel, onClick, onClickPath, focusedPath, keyDownHandlers, keyUphandlers, getPanelContextMenuItems, } = usePlotInteractionHandlers({
        config,
        coordinator,
        draggingRef,
        setActiveTooltip,
    ...
    const { colorsByDatasetIndex, labelsByDatasetIndex, datasetsBuilder } = usePlotDataHandling(config, globalVariables);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const isCanvasTarget = (entry) => entry.target === canvasDiv;
    ...
    const tooltipContent = useMemo(() => {
        return activeTooltip ? (_jsx(TimeBasedChartTooltipContent, { content: activeTooltip.data, multiDataset: numSeries > 1, colorsByConfigIndex: colorsByDatasetIndex, labelsByConfigIndex: labelsByDatasetIndex })) : undefined;
    ...
    return (_jsxs(Stack, { flex: "auto", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative", children: [_jsx(PanelToolbar, {}), _jsxs(Stack, { direction: legendDisplay === "top" ? "column" : "row", flex: "auto", fullWidth: true, style: { height: `calc(100% - ${PANEL_TOOLBAR_MIN_HEIGHT}px)` }, position: "relative", children: [legendDisplay !== "none" && (_jsx(PlotLegend, { coordinator: coordinator, legendDisplay: legendDisplay, onClickPath: onClickPath, paths: series, saveConfig: saveConfig, showLegend: showLegend, sidebarDimension: sidebarDimension, showValues: config.showPlotValuesInLegend, hoveredValuesBySeriesIndex: hoveredValuesBySeriesIndex })), _jsx(Tooltip, { arrow: false, classes: { tooltip: classes.tooltip }, open: tooltipContent != undefined, placement: "right", title: tooltipContent ?? _jsx(_Fragment, {}), disableInteractive: true, followCursor: true, TransitionComponent: Fade, TransitionProps: { timeout: 0 }, children: _jsxs("div", { className: classes.verticalBarWrapper, "data-testid": "vertical-bar-wrapper", children: [_jsx("div", { className: classes.canvasDiv, ref: setCanvasDiv, onWheel: onWheel, onMouseMove: onMouseMove, onMouseOut: onMouseOut, onClick: onClick, onDoubleClick: onResetView }), _jsx(VerticalBars, { coordinator: coordinator, hoverComponentId: subscriberId, xAxisIsPlaybackTime: xAxisMode === "timestamp" })] }) }), canReset && (_jsx("div", { className: classes.resetZoomButton, "data-testid": "plot-reset-view-button", children: _jsx(Button, { variant: "contained", color: "inherit", title: "(shortcut: double-click)", onClick: onResetView, children: t("resetView") }) })), _jsx(PanelContextMenu, { getItems: getPanelContextMenuItems })] }), _jsx(KeyListener, { global: true, keyDownHandlers: keyDownHandlers, keyUpHandlers: keyUphandlers })] }));
};
export default Plot;
