...
export function handleUpdateAction({ draft, path, value }) {
    if (path[0] === "paths") {
...
export function handleAddSeriesAction({ draft }) {
    if (draft.paths.length === 0) {
        draft.paths.push({ ...DEFAULT_PLOT_PATH });
    }
    draft.paths.push({ ...DEFAULT_PLOT_PATH });
}
export function handleDeleteSeriesAction({ draft, index }) {
    draft.paths.splice(Number(index), 1);
}
export default function usePlotPanelSettings(config, saveConfig, focusedPath) {
