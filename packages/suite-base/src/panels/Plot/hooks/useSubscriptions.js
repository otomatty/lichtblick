...
const useSubscriptions = (config, subscriberId) => {
    const { paths, xAxisVal } = config;
    const { globalVariables } = useGlobalVariables();
    const setSubscriptions = useMessagePipeline(useCallback(({ setSubscriptions: pipelineSetSubscriptions }) => pipelineSetSubscriptions, []));
    ...
    useEffect(() => {
        ...
        const subscriptions = filterMap(paths, (item) => {
            ...
            return pathToSubscribePayload(fillInGlobalVariablesInPath(parsedPath, globalVariables), preloadType);
        });
        if ((xAxisVal === "custom" || xAxisVal === "currentCustom") && config.xAxisPath?.value) {
            const parsedPath = parseMessagePath(config.xAxisPath.value);
            if (parsedPath) {
                const xAxisSub = pathToSubscribePayload(fillInGlobalVariablesInPath(parsedPath, globalVariables), preloadType);
                if (xAxisSub) {
