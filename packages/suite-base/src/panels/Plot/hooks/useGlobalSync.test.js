...
describe("useGlobalSync", () => {
    ...
    let setGlobalBounds;
    let globalBounds;
    const renderUseGlobalSync = (options, subscriberId) => {
        return renderHook(() => {
    ...
    beforeEach(() => {
        ...
        setGlobalBounds = jest.fn();
        globalBounds = {};
        useTimelineInteractionState.mockImplementation((selector) => {
    ...
    it("should call setGlobalBounds on timeseriesBounds event", () => {
        ...
        renderUseGlobalSync({ shouldSync: true }, subscriberId);
        const timeseriesBoundsHandler = coordinator.on.mock.calls.find(([event]) => event === "timeseriesBounds")[1];
