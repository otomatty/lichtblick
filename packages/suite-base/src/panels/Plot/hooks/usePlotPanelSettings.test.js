...
import usePlotPanelSettings, { handleAddSeriesAction, handleDeleteSeriesAction, handleUpdateAction, } from "./usePlotPanelSettings";
...
describe("handleUpdateAction", () => {
    ...
    it("should update xAxisPath", () => {
        ...
        handleUpdateAction(input);
        expect(input.draft.xAxisPath).toEqual(expect.objectContaining({
            value: input.value,
        }));
    });
    ...
    it.each(["minXValue", "maxXValue"])("should update followingViewWidth when path is minXValue or maxXValue", (path1) => {
        const initialConfig = PlotBuilder.config({ paths: [] });
        const input = {
            draft: _.cloneDeep(initialConfig),
...
describe("usePlotPanelSettings", () => {
    ...
    beforeEach(() => {
        ...
        usePanelSettingsTreeUpdate.mockReturnValue(updatePanelSettingsTree);
    });
    it.each([
        {
            action: "update",
    ...
    ])("should call saveConfig to update settings tree", (action) => {
