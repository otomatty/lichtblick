...
jest.mock("@lichtblick/suite-base/util/plotColors", () => ({
    getLineColor: jest.fn((color, idx) => color ?? `default-color-${idx}`),
}));
...
describe("usePlotDataHandling hook", () => {
    ...
    it("should throw error when xAxisPath is unsupported", () => {
        ...
        const config = PlotBuilder.config({
            xAxisVal: "unsupportedMode",
            xAxisPath: undefined,
        });
        expect(() => renderHook(() => usePlotDataHandling(config, globalVariables))).toThrow(`unsupported mode: ${config.xAxisVal}`);
