...
const replaceUndefinedWithEmptyDataset = (dataset) => dataset ?? { data: [] };
/**
 * PlotCoordinator interfaces commands and updates between the dataset builder and the chart
 * renderer.
 */
export class PlotCoordinator extends EventEmitter {
    renderer;
    datasetsBuilder;
    shouldSync = false;
    configBounds = { x: {}, y: {} };
    ...
    handleConfig(config, colorScheme, globalVariables) {
        if (this.isDestroyed()) {
            return;
        }
        ...
        const configYBoundsChanged = this.configBounds.y.min !== newConfigBounds.y.min ||
        ...
        const newCurrentValuesByConfigIndex = [];
        this.series = filterMap(config.paths, (path, idx) => {
            ...
            const key = `${path.timestampMethod}:${stringifyMessagePath(filledParsed)}`;
     ...
     */
    getXResetBounds() {
        const currentSecondsIfFollowMode = this.isTimeseriesPlot && this.followRange != undefined && this.currentSeconds != undefined
            ? this.currentSeconds
            : undefined;
        const xMax = currentSecondsIfFollowMode ?? this.configBounds.x.max ?? this.datasetRange?.max;
        const xMinIfFollowMode = this.isTimeseriesPlot && this.followRange != undefined && xMax != undefined
    ...
    async dispatchDatasetsRender(datasets) {
        if (this.isDestroyed()) {
            return;
        }
    ...
    async dispatchBlocks(startTime, blocks) {
