...
describe("PlotCoordinator", () => {
    ...
    beforeEach(() => {
        ...
        renderer = new OffscreenCanvasRenderer(canvas, theme);
        datasetsBuilder = new EventEmitter();
    ...
    describe("handlePlayerState", () => {
        ...
        it("should return immediately if plotCoordinator is destroyed", () => {
            const state = PlayerBuilder.playerState();
            jest.spyOn(plotCoordinator, "isDestroyed").mockReturnValue(true);
        ...
        it("should update currentSeconds if isTimeseriesPlot is true", () => {
            ...
            plotCoordinator.isTimeseriesPlot = true;
            plotCoordinator.handlePlayerState(state);
            expect(plotCoordinator["currentSeconds"]).toBe(state.activeData.currentTime.sec - state.activeData.startTime.sec);
        });
    ...
    describe("dispatchRender", () => {
        ...
        it("should emit 'timeseriesBounds' when updating limits", async () => {
            ...
            renderer.update.mockResolvedValue(bounds);
    ...
    describe("handleConfig", () => {
        ...
        it("should set isTimeseriesPlot to false when xAxisVal is not 'timestamp'", () => {
            const config = PlotBuilder.config({
                xAxisVal: BasicBuilder.sample(["index", "custom", "currentCustom"]),
                paths: [],
            });
        ...
        it("should set updateAction.yBounds if configYBoundsChanged", () => {
            ...
            jest.spyOn(plotCoordinator, "queueDispatchRender").mockImplementation(() => { });
            plotCoordinator.handleConfig(config, "light", {});
        ...
        it("should correctly process paths and generate series", () => {
            const config = PlotBuilder.config();
            parseMessagePath.mockReturnValue(BasicBuilder.string());
            fillInGlobalVariablesInPath.mockReturnValue(undefined);
            stringifyMessagePath.mockReturnValue("");
            plotCoordinator.handleConfig(config, "light", {});
        ...
        it("should dispatch render and queue downsample", async () => {
            const queueDispatchRender = jest.spyOn(plotCoordinator, "queueDispatchRender");
            const queueDispatchDownsample = jest.spyOn(plotCoordinator, "queueDispatchDownsample");
            const config = PlotBuilder.config();
            plotCoordinator.handleConfig(config, "light", {});
    ...
    describe("setGlobalBounds", () => {
        ...
        it("should reset Y bounds when globalBounds is undefined", () => {
            const queueDispatchRenderSpy = jest
                .spyOn(plotCoordinator, "queueDispatchRender")
                .mockImplementation(() => { });
    ...
    describe("setShouldSync", () => {
        ...
        it.each([true, false])("should update shouldSync property", (shouldSync) => {
            plotCoordinator.setShouldSync({ shouldSync });
            expect(plotCoordinator["shouldSync"]).toBe(shouldSync);
        });
    });
    describe("setZoomMode", () => {
        it.each(["x", "xy", "y"])("should update zoomMode in updateAction", (mode) => {
            const queueDispatchRenderSpy = jest
                .spyOn(plotCoordinator, "queueDispatchRender")
                .mockImplementation(() => { });
            plotCoordinator.setZoomMode(mode);
    ...
    describe("setSize", () => {
        it("should update viewport size", () => {
            ...
            const queueDispatchDownsampleSpy = jest
                .spyOn(plotCoordinator, "queueDispatchDownsample")
                .mockImplementation(() => { });
