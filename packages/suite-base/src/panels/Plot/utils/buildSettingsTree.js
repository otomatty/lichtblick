...
import { plotPathDisplayName, } from "@lichtblick/suite-base/panels/Plot/utils/config";
...
const makeSeriesNode = memoizeWeak(({ canDelete, index, path, t }) => {
    return {
        actions: canDelete
            ? [
                {
...
const makeRootSeriesNode = memoizeWeak(({ paths, t }) => {
...
export function buildSettingsTree(config, t) {
    const maxYError = _.isNumber(config.minYValue) &&
        _.isNumber(config.maxYValue) &&
        config.minYValue >= config.maxYValue
        ? t("maxYError")
        : undefined;
    const maxXError = _.isNumber(config.minXValue) &&
    ...
    return {
        ...
        xAxis: {
            ...
            fields: {
                ...
                xAxisPath: config.xAxisVal === "currentCustom" || config.xAxisVal === "custom"
                    ? {
                        label: t("messagePath"),
                        input: "messagepath",
