...
describe("getPixelForXValue", () => {
    const scale = { left: 0, right: 100, min: 0, max: 50 };
    ...
    it("returns correct pixel value for valid xValue and scale", () => {
        ...
        const expected = scale.left + ((xValue - scale.min) / (scale.max - scale.min)) * (scale.right - scale.left);
    ...
    it("returns undefined if pixelRange is less than or equal to 0", () => {
        const zeroPixelRangeScale = { left: 100, right: 100, min: 0, max: 100 };
