...
describe("isReferenceLinePlotPathType", () => {
    it.each(["0", "1.2", "1e6"])("returns true for '%s'", (value) => {
        expect(isReferenceLinePlotPathType({ value, enabled: true, timestampMethod: "receiveTime" })).toBe(true);
    });
    it.each([
        "",
        "x",
        ...
        expect(isReferenceLinePlotPathType({ value, enabled: true, timestampMethod: "receiveTime" })).toBe(false);
    });
});
