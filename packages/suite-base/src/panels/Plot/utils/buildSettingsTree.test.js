...
describe("buildSettingsTree", () => {
    ...
    it("should build the settings tree", () => {
        ...
        const config = PlotBuilder.config({ paths });
        ...
        expect(tree.paths.actions[0].id).toEqual("add-series");
        // paths.children[0]
        const children0 = tree.paths?.children["0"];
        ...
        expect((children0?.actions[0]).id).toEqual("delete-series");
        ...
        expect(children0?.fields["value"]).toEqual(expect.objectContaining({
            supportsMathModifiers: true,
            validTypes: PLOTABLE_ROS_TYPES,
            value: config.paths[0]?.value,
        }));
        expect(children0?.fields["label"]).toEqual(expect.objectContaining({
            value: config.paths[0]?.label,
        }));
        expect(children0?.fields["color"]).toEqual(expect.objectContaining({
            value: config.paths[0]?.color,
        }));
        expect(children0?.fields["lineSize"]).toEqual(expect.objectContaining({ value: config.paths[0]?.lineSize }));
        expect(children0?.fields["showLine"]).toEqual(expect.objectContaining({ value: config.paths[0]?.showLine }));
        expect(children0?.fields["timestampMethod"]).toEqual(expect.objectContaining({ value: config.paths[0]?.timestampMethod }));
        // paths.children[1]
        const children1 = tree.paths?.children["1"];
        expect(children1?.fields["showLine"]).toEqual(expect.objectContaining({ value: true }));
        expect(children1?.fields["color"]).toEqual(expect.objectContaining({ value: lineColors[1 % lineColors.length] }));
    });
