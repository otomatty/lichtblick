...
export class ChartRenderer {
    ...
    constructor(args) {
        ...
        ZoomPlugin.start = (chartInstance, startArgs, pluginOptions) => {
            // swap the canvas with our fake dom node canvas to support zoom plugin addEventListener
            const ctx = chartInstance.ctx;
            chartInstance.ctx = {
                canvas: fakeNode,
            };
            const res = origZoomStart?.(chartInstance, startArgs, pluginOptions);
    ...
    update(action) {
        ...
        if (action.referenceLines) {
            ...
            const newAnnotations = action.referenceLines.map((config) => {
                return {
                    ...DEFAULT_ANNOTATION,
    ...
    #applyInteractionEvent(event) {
        ...
        switch (type) {
            case "wheel":
                ...
                break;
            case "panstart":
                maybeCast(this.#chartInstance)?.$zoom.panStartHandler({
                    center: event.center,
                    deltaX: event.deltaX,
                    deltaY: event.deltaY,
                    target: {
            ...
            case "panmove":
                ...
                break;
            case "panend":
                maybeCast(this.#chartInstance)?.$zoom.panEndHandler();
                break;
        }
    }
    /**
