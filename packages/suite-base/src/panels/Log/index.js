...
function FilterBar(props) {
    return (_jsx(FilterTagInput, { items: [...props.searchTerms], suggestions: [], onChange: (items) => {
...
const LogPanel = React.memo(({ config, saveConfig }) => {
    ...
    const availableTopics = useMemo(() => topics.filter((topic) => topic.schemaName != undefined && SUPPORTED_DATATYPES.includes(topic.schemaName)), [topics]);
    ...
    const seenNodeNamesCache = useRef(new Set());
    ...
    const actionHandler = useCallback((action) => {
        if (action.action === "update") {
            ...
            if (path[0] === "nameFilter") {
                saveConfig(produce((draft) => _.set(draft, path, value)));
            }
            else {
            ...
            saveConfig(produce((draft) => {
                const newNameFilter = Object.fromEntries(Object.entries(draft.nameFilter ?? {}).map(([k, _v]) => [k, { visible }]));
    ...
    useEffect(() => {
        updatePanelSettingsTree({
            actionHandler,
            enableFilter: true,
            nodes: buildSettingsTree(topicToRender, minLogLevel, nameFilter ?? {}, availableTopics, Array.from(seenNodeNames), t),
        });
    }, [
        actionHandler,
        availableTopics,
    ...
    const filteredMessages = useMemo(() => filterMessages(messages, {
        minLogLevel,
        searchTerms,
        nameFilter: nameFilter ?? {},
    }), [messages, minLogLevel, searchTerms, nameFilter]);
    const normalizedMessages = useMemo(() => filteredMessages.map((logMessage) => normalizedLogMessage(logMessage.schemaName, logMessage["message"])), [filteredMessages]);
    const { timeZone } = useAppTimeFormat();
    const handleCopy = useCallback(async () => {
        const messagesToCopy = formatMessages(normalizedMessages, timeZone);
    ...
    const copyLogIcon = (_jsx(ToolbarIconButton, { title: t("copyLogs"), onClick: handleCopy, children: _jsx(Copy20Filled, {}) }));
    return (_jsxs(Stack, { fullHeight: true, children: [_jsx(PanelToolbar, { additionalIcons: copyLogIcon }), _jsx(Stack, { flexGrow: 0, padding: 0.5, children: _jsx(FilterBar, { searchTerms: searchTermsSet, minLogLevel: minLogLevel, onFilterChange: onFilterChange }) }), _jsx(Divider, {}), _jsx(Stack, { flexGrow: 1, children: _jsx(LogList, { items: normalizedMessages }) })] }));
});
