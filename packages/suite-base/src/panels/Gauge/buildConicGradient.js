...
import { ColorModeConfig, ColorMapConfig, } from "@lichtblick/suite-base/panels/Gauge/types";
import { turboColorString } from "@lichtblick/suite-base/util/colorUtils";
export const buildConicGradient = ({ config, gaugeAngle, height, width, }) => {
    ...
    const angleAndPosition = `from ${-Math.PI / 2 + gaugeAngle}rad at 50% ${100 * (width / 2 / height)}%`;
    const angularColorStop = colorStops.length > 0
        ? colorStops
