...
import { ColorMapConfig, ColorModeConfig, } from "@lichtblick/suite-base/panels/Gauge/types";
...
describe("buildConicGradient", () => {
    function setup(configOverride = {}, propsOverride = {}) {
        const config = {
    ...
    it.each([ColorMapConfig.RED_YELLOW_GREEN, ColorMapConfig.RAINBOW])("should generate a gradient for the RED_YELLOW_GREEN and RAINBOW colormaps", (colorMap) => {
    ...
    it("should generate a gradient for the TURBO colormap", () => {
        ...
        const result = buildConicGradient(props);
        const numStops = 20;
        const expectedColors = Array.from({ length: numStops }, (_, i) => turboColorString(i / (numStops - 1)));
