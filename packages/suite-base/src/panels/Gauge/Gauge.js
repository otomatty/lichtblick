...
export function Gauge({ context }) {
    ...
    const [state, dispatch] = useReducer(stateReducer, config, ({ path }) => ({
    ...
    const rawValue = typeof state.latestMatchingQueriedData === "number" ||
        typeof state.latestMatchingQueriedData === "string"
        ? Number(state.latestMatchingQueriedData)
        : NaN;
    const { minValue, maxValue } = config;
    const scaledValue = (Math.max(minValue, Math.min(rawValue, maxValue)) - minValue) / (maxValue - minValue);
    ...
    const height = Math.max(centerY - radius * Math.sin(gaugeAngle), centerY - innerRadius * Math.sin(gaugeAngle)) + padding;
    const needleThickness = 8;
    const needleExtraLength = 0.05;
    const [clipPathId] = useState(() => `gauge-clip-path-${uuidv4()}`);
    return (_jsx("div", { className: classes.root, children: _jsxs("div", { className: classes.gaugeContainer, children: [_jsxs("div", { className: classes.gaugeWrapper, style: { aspectRatio: `${width} / ${height}` }, children: [_jsx("div", { className: classes.conicGradient, style: {
                                background: buildConicGradient({ config, width, height, gaugeAngle }),
                                clipPath: `url(#${clipPathId})`,
                                opacity: state.latestMatchingQueriedData == undefined ? 0.5 : 1,
                            } }), _jsx("div", { className: classes.needle, style: {
                                ...
                                transform: [
                                    `scaleZ(1)`,
                                    `rotate(${-Math.PI / 2 + gaugeAngle + scaledValue * 2 * (Math.PI / 2 - gaugeAngle)}rad)`,
                                    `translateX(${-needleThickness / 2}px)`,
                                    `translateY(${needleThickness / 2}px)`,
                                ].join(" "),
                                width: needleThickness,
                            } })] }), _jsx("svg", { style: { position: "absolute" }, children: _jsx("clipPath", { id: clipPathId, clipPathUnits: "objectBoundingBox", children: _jsx("path", { transform: `scale(${1 / width}, ${1 / height})`, d: [
                                `M ${centerX - radius * Math.cos(gaugeAngle)},${centerY - radius * Math.sin(gaugeAngle)}`,
                                `A 0.5,0.5 0 ${gaugeAngle < 0 ? 1 : 0} 1 ${centerX + radius * Math.cos(gaugeAngle)},${centerY - radius * Math.sin(gaugeAngle)}`,
                                `L ${centerX + innerRadius * Math.cos(gaugeAngle)},${centerY - innerRadius * Math.sin(gaugeAngle)}`,
                                `A ${innerRadius},${innerRadius} 0 ${gaugeAngle < 0 ? 1 : 0} 0 ${centerX - innerRadius * Math.cos(gaugeAngle)},${centerY - innerRadius * Math.sin(gaugeAngle)}`,
                                `Z`,
                            ].join(" ") }) }) })] }) }));
}
