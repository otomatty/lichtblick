...
function buildSettingsTree(config, topics) {
    const general = {
        label: "General",
        fields: {
...
function TeleopPanel(props) {
    ...
    const [config, setConfig] = useState(() => {
        const partialConfig = context.initialState;
        const { topic, publishRate = 1, upButton: { field: upField = "linear-x", value: upValue = 1 } = {}, downButton: { field: downField = "linear-x", value: downValue = -1 } = {}, leftButton: { field: leftField = "angular-z", value: leftValue = 1 } = {}, rightButton: { field: rightField = "angular-z", value: rightValue = -1 } = {}, } = partialConfig;
        return {
            topic,
            publishRate,
    ...
    return (_jsx(ThemeProvider, { isDark: colorScheme === "dark", children: _jsxs(Stack, { fullHeight: true, justifyContent: "center", alignItems: "center", style: { padding: "min(5%, 8px)", textAlign: "center" }, children: [!canPublish && _jsx(EmptyState, { children: "Connect to a data source that supports publishing" }), canPublish && !hasTopic && (_jsx(EmptyState, { children: "Select a publish topic in the panel settings" })), enabled && _jsx(DirectionalPad, { onAction: setCurrentAction, disabled: !enabled })] }) }));
}
