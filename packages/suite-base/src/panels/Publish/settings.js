...
function datatypeError(schemaNames, datatype) {
    if (!datatype) {
...
function topicError(topicName) {
    if (!topicName) {
        return "Topic cannot be empty";
    }
    return undefined;
}
const buildSettingsTree = (config, schemaNames, topics) => ({
    general: {
        fields: {
            topicName: {
                label: "Topic",
...
const getSampleMessage = (datatypes, datatype) => {
...
export function usePublishPanelSettings(config, saveConfig, topics, datatypes) {
