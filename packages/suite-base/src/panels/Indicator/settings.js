...
export function settingsActionReducer(prevConfig, action) {
        ...
        switch (action.action) {
            case "perform-node-action":
                if (action.payload.path[0] === "rules") {
                    ...
                    else if (action.payload.id === "add-rule" ||
...
const memoizedCreateRuleNode = memoizeWeak((rule, i, rules) => {
    const actions = [
...
export function useSettingsTree(config, pathParseError, error) {
    ...
    const ruleSettings = useMemo(() => ({
        ...
        children: Object.fromEntries(rules
