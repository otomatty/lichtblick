...
import { TabDndContext, } from "@lichtblick/suite-base/panels/Tab/TabDndContext";
...
function Tab({ config, saveConfig }) {
    ...
    const [draggingTabState, setDraggingTabState] = useState({
        item: undefined,
        isOver: false,
    });
    ...
    const actions = useMemo(() => ({ addTab, removeTab, selectTab, setTabTitle }), [addTab, removeTab, selectTab, setTabTitle]);
    ...
    const preventTabDrop = !!draggingTab &&
        draggingTab.panelId === panelId &&
        draggingTab.tabIndex === activeTabIdx &&
        !draggingTabState.isOver;
    return (_jsxs(Stack, { flex: "auto", overflow: "hidden", children: [_jsx(TabbedToolbar, { panelId: panelId, tabs: tabs, actions: actions, activeTabIdx: activeTabIdx, setDraggingTabState: setDraggingTabState }), _jsxs(Stack, { direction: "row", flex: "auto", overflow: "hidden", position: "relative", children: [activeLayout != undefined ? (_jsx(TabDndContext.Provider, { value: { preventTabDrop }, children: _jsx(UnconnectedPanelLayout, { layout: activeLayout, onChange: onChangeLayout, tabId: panelId }) })) : (_jsx(EmptyPanelLayout, { tabId: panelId })), preventTabDrop && _jsx("div", { className: classes.panelCover })] })] }));
}
