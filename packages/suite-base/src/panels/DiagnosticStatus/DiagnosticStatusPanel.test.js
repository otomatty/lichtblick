...
describe("DiagnosticStatusPanel", () => {
    ...
    const setup = (configOverride = {}) => {
        ...
        const ui = (_jsx(MockPanelContextProvider, { children: _jsx(PanelSetup, { children: _jsx(DiagnosticStatusPanel, { ...props }) }) }));
        return {
            ...render(ui),
            ...props,
        };
    ...
    it("should render the empty state when there is a selected display name", () => {
        ...
        expect(screen.getByText(new RegExp(`${config.selectedHardwareId}: ${config.selectedName}`, "i"))).toBeInTheDocument();
    });
