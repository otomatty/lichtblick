...
export function Stats() {
    const [div, setDiv] = useState(ReactNull);
...
class Panel {
    ...
    update(value, maxValue) {
        const PR = Math.round(window.devicePixelRatio);
        const WIDTH = 80 * PR, TEXT_X = 3 * PR, TEXT_Y = 2 * PR, GRAPH_X = 3 * PR, GRAPH_Y = 15 * PR, GRAPH_WIDTH = 74 * PR, GRAPH_HEIGHT = 30 * PR;
