...
export const DEFAULT_MESH_UP_AXIS = "y_up";
...
export class ModelCache {
    ...
    #edgeMaterial;
    #fetchAsset;
    #colladaTextureObjectUrls = new Map();
    #dracoLoader;
    constructor(options) {
    ...
    async load(url, opts, reportError) {
    ...
    async #loadModel(url, options, reportError) {
        ...
        if (GLB_MAGIC === view.getUint32(0, false) ||
            ...
            return await this.#loadGltf(url, buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength), reportError);
        }
        // Check if this is a STL file based on content-type or file extension
        if (STL_MIME_TYPES.includes(contentType) || /\.stl$/i.test(url)) {
            // Create a copy of the array buffer to respect the `byteOffset` and `byteLength` value as
            // the underlying three.js STLLoader only accepts an ArrayBuffer instance.
            return this.#loadSTL(url, buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength), this.options.meshUpAxis);
        }
    ...
    async #loadGltf(url, buffer, reportError) {
    ...
    ignoreUpAxis, reportError) {
        ...
        for (const node of xml.querySelectorAll("init_from")) {
            ...
            try {
                ...
                const objectUrl = URL.createObjectURL(new Blob([textureAsset.data], { type: textureAsset.mediaType }));
    ...
    #getDracoLoader(manager) {
        ...
        if (!dracoLoader) {
            ...
            dracoLoader["_loadLibrary"] = async function (url, responseType) {
...
function addEdges(model, edgeMaterial) {
    const edgesToAdd = [];
...
function fixDaeMaterials(model) {
    model.traverse((child) => {
        if (!(child instanceof THREE.Mesh)) {
            return;
        }
...
function fixObjMaterials(model) {
    model.traverse((child) => {
        if (!(child instanceof THREE.Mesh)) {
            return;
        }
...
function toStandard(material) {
    ...
    const shininess = material.shininess ?? 0; // [0-100]
