...
import { PublishRos1Datatypes, PublishRos2Datatypes, makePointMessage, makePoseEstimateMessage, makePoseMessage, } from "./publish";
...
export function ThreeDeeRender(props) {
    ...
    const { initialState, saveState, unstable_fetchAsset: fetchAsset, unstable_setMessagePathDropConfig: setMessagePathDropConfig, } = context;
    ...
    const [canvas, setCanvas] = useState(ReactNull);
    const [renderer, setRenderer] = useState(undefined);
    const rendererRef = useRef(undefined);
    const { enqueueSnackbar } = useSnackbar();
    const displayTemporaryError = useCallback((errorString) => {
    ...
    useEffect(() => {
        const newRenderer = canvas
            ? new Renderer({
                ...
                interfaceMode,
                fetchAsset,
                sceneExtensionConfig: _.merge({}, DEFAULT_SCENE_EXTENSION_CONFIG, customSceneExtensions ?? {}),
    ...
    const schemaSubscriptions = useRendererProperty("schemaSubscriptions", "schemaSubscriptionsChanged", () => new Map(), renderer);
    const topicSubscriptions = useRendererProperty("topicSubscriptions", "topicSubscriptionsChanged", () => new Map(), renderer);
    // Config cameraState
    useEffect(() => {
        const listener = () => {
            if (renderer) {
    ...
    const [settingsTree, setSettingsTree] = useState(undefined);
    const updateSettingsTree = useCallback((curRenderer) => {
    ...
    const updateConfig = useCallback((curRenderer) => {
    ...
    const updateSelectedRenderable = useCallback((selection) => {
    ...
    const [focusedSettingsPath, setFocusedSettingsPath] = useState();
    const onShowTopicSettings = useCallback((topic) => {
    ...
    const throttledSave = useDebouncedCallback((newConfig) => {
        saveState(newConfig);
    ...
    const [topicsToSubscribe, setTopicsToSubscribe] = useState(undefined);
    useEffect(() => {
        if (!topics) {
        ...
        const addSubscription = (topic, rendererSubscription, convertTo) => {
    ...
    useEffect(() => {
        ...
        if (sharedPanelState.followMode !== config.followMode) {
            renderer.setCameraSyncError(`Follow mode must be ${sharedPanelState.followMode} to sync camera.`);
        }
        else if (sharedPanelState.followTf !== renderer.followFrameId) {
            renderer.setCameraSyncError(`Display frame must be ${sharedPanelState.followTf} to sync camera.`);
        }
        else {
    ...
    const addPanel = useCallback((params) => {
        ...
        const onSubmit = (event) => {
            ...
            try {
                switch (event.publishClickType) {
                    ...
                    case "pose_estimate": {
                        const message = makePoseEstimateMessage(event.pose, frameId, latestPublishConfig.current.poseEstimateXDeviation, latestPublishConfig.current.poseEstimateYDeviation, latestPublishConfig.current.poseEstimateThetaDeviation);
    ...
    return (_jsx(ThemeProvider, { isDark: colorScheme === "dark", children: _jsxs("div", { style: PANEL_STYLE, onKeyDown: onKeyDown, children: [_jsx("canvas", { ref: setCanvas, style: {
                        position: "absolute",
                        top: 0,
                        left: 0,
                        ...((measureActive || publishActive) && { cursor: "crosshair" }),
                    } }), _jsx(RendererContext.Provider, { value: renderer, children: _jsx(RendererOverlay, { interfaceMode: interfaceMode, canvas: canvas, addPanel: addPanel, enableStats: config.scene.enableStats ?? false, perspective: config.cameraState.perspective, onTogglePerspective: onTogglePerspective, measureActive: measureActive, onClickMeasure: onClickMeasure, canPublish: canPublish, publishActive: publishActive, onClickPublish: onClickPublish, onShowTopicSettings: onShowTopicSettings, publishClickType: renderer?.publishClickTool.publishClickType ?? "point", onChangePublishClickType: (type) => {
