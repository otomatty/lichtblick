...
import { IMAGE_MODE_HUD_GROUP_ID, IMAGE_TOPIC_PATH, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/ImageMode/constants";
import { IMAGE_RENDERABLE_DEFAULT_SETTINGS, ImageRenderable, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/Images/ImageRenderable";
import { getFrameIdFromImage, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/Images/ImageTypes";
import { IMAGE_DEFAULT_COLOR_MODE_SETTINGS } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/Images/decodeImage";
import { cameraInfosEqual, normalizeCameraInfo, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/projections";
...
import { BOTH_TOPICS_DO_NOT_EXIST_HUD_ITEM_ID, IMAGE_TOPIC_DOES_NOT_EXIST_HUD_ITEM_ID, CALIBRATION_TOPIC_DOES_NOT_EXIST_HUD_ITEM_ID, } from "./constants";
import { SceneExtension } from "../../SceneExtension";
import { CAMERA_CALIBRATION_DATATYPES, COMPRESSED_IMAGE_DATATYPES, COMPRESSED_VIDEO_DATATYPES, RAW_IMAGE_DATATYPES, } from "../../foxglove";
import { IMAGE_DATATYPES as ROS_IMAGE_DATATYPES, COMPRESSED_IMAGE_DATATYPES as ROS_COMPRESSED_IMAGE_DATATYPES, CAMERA_INFO_DATATYPES, } from "../../ros";
...
const NO_IMAGE_TOPICS_HUD_ITEM = {
    id: "NO_IMAGE_TOPICS",
...
export class ImageMode extends SceneExtension {
    static extensionId = "foxglove.ImageMode";
    #camera;
    #cameraModel;
    #annotations;
    imageRenderable;
    #removeImageTimeout;
    messageHandler;
    supportedImageSchemas = ALL_SUPPORTED_IMAGE_SCHEMAS;
    ...
    constructor(renderer, name = ImageMode.extensionId) {
        ...
        this.#annotations = new ImageAnnotations({
            ...
            addSettingsError(path, errorId, errorMessage) {
                renderer.settings.errors.add(path, errorId, errorMessage);
            },
            removeSettingsError(path, errorId) {
        ...
        renderer.input.on("mousedown", (mouseDownCursorCoords) => {
            ...
            renderer.input.trackDrag((mouseMoveCursorCoords) => {
                this.#camera.setPanOffset(mouseMoveCursorCoords
                    .clone()
        ...
        renderer.input.on("wheel", (cursorCoords, _worldSpaceCursorCoords, event) => {
            ...
            1 - 0.01 * THREE.MathUtils.clamp(event.deltaY, -30, 30), cursorCoords);
    ...
    #handleTopicsChanged = () => {
        ...
        const imageTopic = this.renderer.topics?.find((topic) => topicIsConvertibleToSchema(topic, this.supportedImageSchemas));
    ...
    #getMatchingCalibrationTopic(imageTopic) {
        const prefix = getTopicMatchPrefix(imageTopic);
        if (prefix == undefined) {
            return undefined;
        }
        return this.renderer.topics?.find((topic) => topicIsConvertibleToSchema(topic, ALL_SUPPORTED_CALIBRATION_SCHEMAS) &&
            topic.name.startsWith(prefix));
    }
    settingsNodes() {
        const handler = this.handleSettingsAction;
        const settings = this.getImageModeSettings();
        const { imageTopic: imageTopicName, calibrationTopic, synchronize, flipHorizontal, flipVertical, rotation, } = settings;
        ...
        const imageTopicExists = !imageTopicName || imageTopics.some((topic) => topic.value === imageTopicName);
        this.renderer.settings.errors.errorIfFalse(imageTopicExists, IMAGE_TOPIC_PATH, IMAGE_TOPIC_UNAVAILABLE, `${imageTopicName} is not available`);
        const calibrationTopicExists = !(calibrationTopic && !calibrationTopics.some((topic) => topic.value === calibrationTopic));
        this.renderer.settings.errors.errorIfFalse(calibrationTopicExists, CALIBRATION_TOPIC_PATH, CALIBRATION_TOPIC_UNAVAILABLE, `${calibrationTopic} is not available`);
        ...
        const calibrationTopicError = this.renderer.settings.errors.errors.errorAtPath(CALIBRATION_TOPIC_PATH);
        const fields = {};
        fields.imageTopic = {
            label: t3D("topic"),
            input: "select",
        ...
        const imageTopic = imageTopicName != undefined ? this.renderer.topicsByName?.get(imageTopicName) : undefined;
        const isRawImageTopic = imageTopic != undefined &&
    ...
    handleSettingsAction = (action) => {
        ...
        const config = this.getImageModeSettings();
        const calibrationTopicChanged = config.calibrationTopic !== prevImageModeConfig.calibrationTopic;
    ...
    updateConfigForDropPath = (draft, path) => {
    ...
    #handleCameraInfoChange = (cameraInfo) => {
     ...
     */
    #updateFallbackCameraModel(renderable) {
    ...
    #getImageRenderable(topicName, receiveTime, image, frameId) {
    ...
    initRenderable(topicName, userData) {
    ...
    #getCurrentFrameId() {
        ...
        if (imageFrameId != undefined) {
            if (imageFrameId !== cameraInfoFrameId) {
                this.renderer.settings.errors.add(IMAGE_TOPIC_PATH, IMAGE_TOPIC_DIFFERENT_FRAME, `Image topic's frame id (${imageFrameId}) does not match camera info's frame id (${cameraInfoFrameId})`);
            }
            else {
    ...
    getImageModeSettings() {
        ...
        const colorMode = config.colorMode === "rgba-fields"
     ...
     */
    #updateViewAndRenderables() {
        const cameraInfo = this.#cameraModel?.info;
        if (!this.#fallbackCameraModelActive() && !cameraInfo) {
            this.renderer.settings.errors.add(CALIBRATION_TOPIC_PATH, MISSING_CAMERA_INFO, "Missing camera info for topic");
            return;
        }
        else {
     ...
     */
    #updateCameraModel(newCameraInfo) {
    ...
    handleResize(width, height, _pixelRatio) {
        this.#camera.setCanvasSize(width, height);
        this.#updateAnnotationsScale();
    }
    #updateAnnotationsScale() {
        this.#annotations.updateScale(this.#camera.getEffectiveScale(), this.renderer.input.canvasSize.width, this.renderer.input.canvasSize.height, this.renderer.getPixelRatio());
    }
    setCameraState() {
        this.#updateViewAndRenderables();
    }
    ...
    #getDownloadImageCallback = () => {
        return async () => {
            ...
            try {
                const width = rotation === 90 || rotation === 270 ? currentImage.height : currentImage.width;
                const height = rotation === 90 || rotation === 270 ? currentImage.width : currentImage.height;
                ...
                const bitmap = currentImage instanceof ImageData ? await createImageBitmap(currentImage) : currentImage;
