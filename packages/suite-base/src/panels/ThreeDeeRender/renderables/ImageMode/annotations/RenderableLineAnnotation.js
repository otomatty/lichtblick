...
const FALLBACK_COLOR = { r: 0, g: 0, b: 0, a: 0 };
/**
 * Handles rendering of 2D annotations (line list, line strip, and line loop/polygon).
 */
export class RenderableLineAnnotation extends Renderable {
    #geometry;
    #linePrepass;
    #linePrepassMaterial;
    #line;
    ...
    setScale(scale, canvasWidth, canvasHeight, _pixelRatio) {
    ...
    setAnnotationFromCircle(annotation, originalMessage) {
        ...
        if (this.#annotationNeedsUpdate || this.#cameraModelNeedsUpdate) {
            ...
            if (this.#numPoints == undefined ||
                ...
                if (useVertexColors) {
                    ...
                    // [rgba, rgba]
                    const instanceColorBuffer = new THREE.InstancedInterleavedBuffer(this.#colorBuffer, isLineList ? 8 : 4 /* stride */, 1);
                    this.#geometry.setAttribute("instanceColorStart", new THREE.InterleavedBufferAttribute(instanceColorBuffer, 4, 0, true));
                    this.#geometry.setAttribute("instanceColorEnd", new THREE.InterleavedBufferAttribute(instanceColorBuffer, 4, isLineList ? 4 : 0 /* offset */, true));
                }
                this.#linePrepass.geometry = this.#geometry;
                this.#line.geometry = this.#geometry;
            }
            const hasExactColors = isLineList && outlineColors.length === pointsLength / 2;
            const shapeFillColor = (isPolygon || isLineStrip) && fillColor != undefined && fillColor.a > 0
                ? fillColor
                : undefined;
...
function makePointsAnnotationFromCircle(circle) {
    const segments = 32;
    const { position: { x: cx, y: cy }, radius, } = circle;
    return {
        type: "points",
        stamp: circle.stamp,
        style: "polygon",
