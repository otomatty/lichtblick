...
import { MessageHandler, WAITING_FOR_BOTH_HUD_ITEM, WAITING_FOR_CALIBRATION_HUD_ITEM, WAITING_FOR_IMAGE_EMPTY_HUD_ITEM, WAITING_FOR_IMAGE_NOTICE_HUD_ITEM, WAITING_FOR_SYNC_EMPTY_HUD_ITEM, WAITING_FOR_SYNC_NOTICE_HUD_ITEM, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/ImageMode/MessageHandler";
function wrapInMessageEvent(topic, schema, time, message) {
    return {
        message: (message ?? {}),
        topic,
        schemaName: schema,
...
describe("MessageHandler: synchronized = false", () => {
    ...
    it("should have camera info if handled camera info", () => {
        ...
        const messageHandler = new MessageHandler(initConfig, hud);
        const cameraInfo = wrapInMessageEvent("calibration", "foxglove.CameraCalibration", 0n);
    ...
    it("should have image if handled image", () => {
        const hud = new HUDItemManager(() => { });
        const messageHandler = new MessageHandler({ synchronize: false, calibrationTopic: "info" }, hud);
        const image = wrapInMessageEvent("image", "foxglove.RawImage", 0n);
        messageHandler.handleRawImage(image);
        const state = messageHandler.getRenderStateAndUpdateHUD();
        expect(state.image).not.toBeUndefined();
        expect(hud.getHUDItems().find((item) => item.id === WAITING_FOR_CALIBRATION_HUD_ITEM.id)).toBeTruthy();
    });
    it("should have annotations if handled annotations", () => {
        ...
        const annotationMessage = wrapInMessageEvent("annotations", "foxglove.ImageAnnotations", 0n, annotation);
        messageHandler.handleAnnotations(annotationMessage);
        const state = messageHandler.getRenderStateAndUpdateHUD();
    ...
    it("clears image if image topic changed", () => {
        ...
        const messageHandler = new MessageHandler(initConfig, hud);
        const image = wrapInMessageEvent("image1", "foxglove.RawImage", 0n);
        messageHandler.handleRawImage(image);
    ...
    it("clears cameraInfo if calibration topic changed", () => {
        ...
        const messageHandler = new MessageHandler(initConfig, hud);
        const cameraInfo = wrapInMessageEvent("calibration1", "foxglove.CameraCalibration", 0n);
        messageHandler.handleCameraInfo(cameraInfo);
    ...
    it("clears specific annotations if annotations subscriptions change", () => {
        ...
        const messageHandler = new MessageHandler(initConfig, hud);
        const annotation = createCircleAnnotations([0n]);
        const annotation1Message = wrapInMessageEvent("annotations1", "foxglove.ImageAnnotations", 0n, annotation);
        const annotation2Message = wrapInMessageEvent("annotations2", "foxglove.ImageAnnotations", 0n, annotation);
        messageHandler.handleAnnotations(annotation1Message);
        messageHandler.handleAnnotations(annotation2Message);
...
describe("MessageHandler: synchronized = true", () => {
    it("handles and shows camera info in state", () => {
        const hud = new HUDItemManager(() => { });
        const messageHandler = new MessageHandler({ synchronize: true, calibrationTopic: "calibration" }, hud);
        const cameraInfo = wrapInMessageEvent("calibration", "foxglove.CameraCalibration", 0n);
        messageHandler.handleCameraInfo(cameraInfo);
        const state = messageHandler.getRenderStateAndUpdateHUD();
        expect(state.cameraInfo).not.toBeUndefined();
    ...
    it("shows state without image and annotations if they have different header timestamps", () => {
        ...
        const annotationMessage1 = wrapInMessageEvent("annotations1", "foxglove.ImageAnnotations", 0n, annotation1);
        const annotation2 = createCircleAnnotations([time + 1n]);
        const annotationMessage2 = wrapInMessageEvent("annotations2", "foxglove.ImageAnnotations", 0n, annotation2);
        messageHandler.handleRawImage(image);
        messageHandler.handleAnnotations(annotationMessage1);
        messageHandler.handleAnnotations(annotationMessage2);
        const state = messageHandler.getRenderStateAndUpdateHUD();
        expect(state.image).toBeUndefined();
    ...
    it("shows most recent image and annotations with same timestamps", () => {
        ...
        let time = 2n;
        let image = wrapInMessageEvent("image", "foxglove.RawImage", 0n, {
            timestamp: fromNanoSec(time),
        });
        let annotation = createCircleAnnotations([time]);
        let annotationMessage = wrapInMessageEvent("annotations", "foxglove.ImageAnnotations", 0n, annotation);
        messageHandler.handleRawImage(image);
        messageHandler.handleAnnotations(annotationMessage);
        time = 4n;
        image = wrapInMessageEvent("image", "foxglove.RawImage", 1n, {
            timestamp: fromNanoSec(time),
        });
        annotation = createCircleAnnotations([time]);
        annotationMessage = wrapInMessageEvent("annotations", "foxglove.ImageAnnotations", 1n, annotation);
        messageHandler.handleRawImage(image);
        messageHandler.handleAnnotations(annotationMessage);
        const state = messageHandler.getRenderStateAndUpdateHUD();
        expect((state.image?.message).timestamp).toEqual(fromNanoSec(time));
        expect(state.annotationsByTopic?.get("annotations")?.annotations[0]?.stamp).toEqual(fromNanoSec(time));
    });
    ...
    it("does not show image in state if it hasn't received requisite annotations at same timestamp", () => {
        ...
        const annotation = createCircleAnnotations([time]);
        const annotationMessage = wrapInMessageEvent("annotations1", "foxglove.ImageAnnotations", 0n, annotation);
        messageHandler.handleRawImage(image);
        messageHandler.handleAnnotations(annotationMessage);
        const state = messageHandler.getRenderStateAndUpdateHUD();
        expect(state.image).toBeUndefined();
...
describe("MessageHandler: hud item display", () => {
    ...
    it("init: waiting for both if calibration topic specified", () => {
        const hud = new HUDItemManager(() => { });
        const messageHandler = new MessageHandler({ synchronize: false, calibrationTopic: "exists" }, hud);
        messageHandler.getRenderStateAndUpdateHUD();
        const hudItems = _.sortBy(hud.getHUDItems(), "id");
    ...
    it("shows waiting for Both if calibration topic changed after only receiving calibration message", () => {
        ...
        messageHandler.setConfig({ ...initConfig, calibrationTopic: "calibration2" });
        messageHandler.getRenderStateAndUpdateHUD();
        const hudItems = _.sortBy(hud.getHUDItems(), "id").filter((item) => item.displayType === "empty");
    ...
    it("displays waiting for image after calibration is set to undefined", () => {
        ...
        messageHandler.setConfig({ ...initConfig, calibrationTopic: undefined });
        messageHandler.getRenderStateAndUpdateHUD();
        const hudItems = _.sortBy(hud.getHUDItems(), "id").filter(({ displayType }) => displayType === "empty");
        expect(hudItems).toEqual([WAITING_FOR_IMAGE_EMPTY_HUD_ITEM]);
    });
    it("displays no info after receiving calibration and image", () => {
        const hud = new HUDItemManager(() => { });
        const messageHandler = new MessageHandler({ synchronize: false, calibrationTopic: "calibration" }, hud);
        const cameraInfo = wrapInMessageEvent("calibration", "foxglove.CameraCalibration", 0n);
        messageHandler.handleCameraInfo(cameraInfo);
        const image = wrapInMessageEvent("image1", "foxglove.RawImage", 0n);
        messageHandler.handleRawImage(image);
    ...
    it("displays no info after receiving image when calibration topic is undefined", () => {
        const hud = new HUDItemManager(() => { });
        const messageHandler = new MessageHandler({ synchronize: false, calibrationTopic: undefined }, hud);
        const image = wrapInMessageEvent("image1", "foxglove.RawImage", 0n);
        messageHandler.handleRawImage(image);
        messageHandler.getRenderStateAndUpdateHUD();
        const hudItems = _.sortBy(hud.getHUDItems(), "id");
...
function createCircleAnnotations(atTimes) {
    return {
