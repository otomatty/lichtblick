...
import { createStixelMaterial, PointCloudHistoryRenderable, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/PointClouds";
import { Calibration, PointCloud, PointFieldDataType, RawPacket, Transformer, } from "@lichtblick/velodyne-cloud";
import { autoSelectColorSettings } from "./colorMode";
import { createInstancePickingMaterial, createPickingMaterial, DEFAULT_POINT_SETTINGS, pointSettingsNode, pointCloudMaterial, POINT_CLOUD_REQUIRED_FIELDS, } from "./pointExtensionUtils";
...
function pointFieldDataTypeToNumericType(type) {
    switch (type) {
...
export class VelodyneScans extends SceneExtension {
    static extensionId = "foxglove.VelodyneScans";
    #pointCloudFieldsByTopic = new Map();
    #velodyneCloudConverter = new VelodyneCloudConverter();
    constructor(renderer, name = VelodyneScans.extensionId) {
        super(name, renderer);
    }
    getSubscriptions() {
        return [
    ...
    settingsNodes() {
        ...
        for (const topic of this.renderer.topics ?? []) {
            ...
            const messageFields = this.#pointCloudFieldsByTopic.get(topic.name) ?? POINT_CLOUD_REQUIRED_FIELDS;
    ...
    #handleVelodyneScan = (messageEvent) => {
        ...
        renderable.updatePointCloud(pointCloud, messageEvent.message, renderable.userData.settings, receiveTime);
    };
}
