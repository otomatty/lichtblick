...
import { makeLineMaterial, makeLinePrepassMaterial, makeLinePickingMaterial, markerHasTransparency, } from "./materials";
...
export class RenderableLineStrip extends RenderableMarker {
    ...
    dispose() {
        ...
        const pickingMaterial = this.#line.userData.pickingMaterial;
    ...
    update(newMarker, receiveTime) {
        ...
        const matLinePrepass = this.#linePrepass.material;
        matLinePrepass.lineWidth = lineWidth;
        const matLine = this.#line.material;
    ...
    #setColors(marker, pointsLength) {
        ...
        if (8 * pointsLength > this.#colorBuffer.length) {
            ...
            this.#geometry.setAttribute("instanceColorEnd", new THREE.InterleavedBufferAttribute(instanceColorBuffer, 4, 4, true));
        }
        else {
