...
import { arrowHeadSubdivisions, arrowShaftSubdivisions } from "../../lod";
...
export class RenderableArrow extends RenderableMarker {
    ...
    constructor(topic, marker, receiveTime, renderer) {
        ...
        const shaftGeometry = this.renderer.sharedGeometry.getGeometry(`${this.constructor.name}-shaft-${renderer.maxLod}`, () => createShaftGeometry(renderer.maxLod));
        const headGeometry = this.renderer.sharedGeometry.getGeometry(`${this.constructor.name}-head-${renderer.maxLod}`, () => createHeadGeometry(renderer.maxLod));
        const shaftEdgesGeometry = this.renderer.sharedGeometry.getGeometry(`${this.constructor.name}-shaftedges-${renderer.maxLod}`, () => createShaftEdgesGeometry(shaftGeometry));
        const headEdgesGeometry = this.renderer.sharedGeometry.getGeometry(`${this.constructor.name}-headedges-${renderer.maxLod}`, () => createHeadEdgesGeometry(headGeometry));
        // Shaft mesh
...
function createShaftGeometry(lod) {
...
function createHeadGeometry(lod) {
...
function createShaftEdgesGeometry(shaftGeometry) {
...
function createHeadEdgesGeometry(headGeometry) {
