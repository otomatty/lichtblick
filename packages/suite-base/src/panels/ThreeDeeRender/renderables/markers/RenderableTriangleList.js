...
export class RenderableTriangleList extends RenderableMarker {
    ...
    constructor(topic, marker, receiveTime, renderer) {
        ...
        this.#mesh = new THREE.Mesh(new DynamicBufferGeometry(), makeStandardVertexColorMaterial(marker));
    ...
    update(newMarker, receiveTime) {
        ...
        if (vertexCount === 0) {
            this.renderer.settings.errors.addToTopic(this.userData.topic, EMPTY_ERR, `TRIANGLE_LIST: points is empty`);
            this.#mesh.geometry.resize(0);
            return;
        }
        if (vertexCount % 3 !== 0) {
            const markerId = `${marker.ns.length > 0 ? marker.ns + ":" : ""}${marker.id}`;
            this.renderer.settings.errors.addToTopic(this.userData.topic, NOT_DIVISIBLE_ERR, `TRIANGLE_LIST: points.length ${vertexCount} is not divisible by 3 for marker ${markerId}`);
            vertexCount = Math.floor(vertexCount / 3) * 3;
        }
        if (marker.colors.length !== 0 && marker.colors.length !== vertexCount) {
            const markerId = `${marker.ns.length > 0 ? marker.ns + ":" : ""}${marker.id}`;
            this.renderer.settings.errors.addToTopic(this.userData.topic, COLORS_MISMATCH_ERR, `TRIANGLE_LIST: colors.length ${marker.colors.length} != points.length ${vertexCount} for marker ${markerId}`);
        ...
        for (let i = 0; i < vertexCount; i++) {
            ...
            if (!isPointValid(point)) {
                this.renderer.settings.errors.addToTopic(this.userData.topic, INVALID_POINT_ERR, `TRIANGLE_LIST: point at index ${i} is not finite`);
                continue;
            }
            dataChanged =
                dataChanged ||
