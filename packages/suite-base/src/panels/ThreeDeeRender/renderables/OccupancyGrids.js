...
import { normalizeHeader, normalizePose, normalizeInt8Array, normalizeTime, } from "../normalizeMessages";
import { OCCUPANCY_GRID_DATATYPES } from "../ros";
...
export class OccupancyGridRenderable extends Renderable {
    dispose() {
...
export class OccupancyGrids extends SceneExtension {
    static extensionId = "foxglove.OccupancyGrids";
    constructor(renderer, name = OccupancyGrids.extensionId) {
        super(name, renderer);
    }
    getSubscriptions() {
        return [
    ...
    handleSettingsAction = (action) => {
        ...
        if (renderable) {
            ...
            this.#updateOccupancyGridRenderable(renderable, renderable.userData.occupancyGrid, renderable.userData.receiveTime);
        }
    };
    #handleOccupancyGrid = (messageEvent) => {
        ...
        if (!renderable) {
            ...
            const geometry = this.renderer.sharedGeometry.getGeometry(this.constructor.name, createGeometry);
    ...
    #updateOccupancyGridRenderable(renderable, occupancyGrid, receiveTime) {
...
function invalidOccupancyGridError(renderer, renderable, message) {
    renderer.settings.errors.addToTopic(renderable.userData.topic, INVALID_OCCUPANCY_GRID, message);
}
function createTexture(occupancyGrid) {
    const width = occupancyGrid.info.width;
    const height = occupancyGrid.info.height;
    const size = width * height;
    const rgba = new Uint8ClampedArray(size * 4);
    const texture = new THREE.DataTexture(rgba, width, height, THREE.RGBAFormat, THREE.UnsignedByteType, THREE.UVMapping, THREE.ClampToEdgeWrapping, THREE.ClampToEdgeWrapping, THREE.NearestFilter, THREE.LinearFilter, 1, THREE.LinearSRGBColorSpace);
    texture.generateMipmaps = false;
    return texture;
}
function createMesh(topic, geometry, texture, settings) {
...
function updateTexture(texture, occupancyGrid, settings) {
    const size = occupancyGrid.info.width * occupancyGrid.info.height;
...
function createMaterial(texture, topic, settings) {
...
function createPickingMaterial(texture) {
...
function occupancyGridHasTransparency(settings) {
    if (settings.colorMode === "custom") {
        stringToRgba(tempMinColor, settings.minColor);
        stringToRgba(tempMaxColor, settings.maxColor);
        stringToRgba(tempUnknownColor, settings.unknownColor);
        stringToRgba(tempInvalidColor, settings.invalidColor);
        return (tempMinColor.a < 1 || tempMaxColor.a < 1 || tempInvalidColor.a < 1 || tempUnknownColor.a < 1);
    }
    else {
        return true;
    }
}
function srgbToLinearUint8(color) {
...
function normalizeOccupancyGrid(message) {
 ...
 */
function paletteColorCached(output, value, paletteColorMode) {
