...
import { ALL_CAMERA_INFO_SCHEMAS } from "./Images/ImageTypes";
import { normalizeCompressedImage, normalizeCompressedVideo, normalizeRawImage, normalizeRosCompressedImage, normalizeRosImage, } from "./Images/imageNormalizers";
...
import { CAMERA_INFO_DATATYPES, COMPRESSED_IMAGE_DATATYPES as ROS_COMPRESSED_IMAGE_DATATYPES, IMAGE_DATATYPES as ROS_IMAGE_DATATYPES, } from "../ros";
...
export class Images extends SceneExtension {
    static extensionId = "foxglove.Images";
    /* All known camera info topics */
    #cameraInfoTopics = new Set();
    /**
     ...
     */
    #cameraInfoToImageTopics = new MultiMap();
    /**
     ...
     */
    #cameraInfoByTopic = new Map();
    customCameraModels;
    supportedImageSchemas = ALL_SUPPORTED_IMAGE_SCHEMAS;
    constructor(renderer, name = Images.extensionId) {
        super(name, renderer);
    ...
    #handleTopicsChanged = () => {
        ...
        for (const topic of this.renderer.topics ?? []) {
            if (topicIsConvertibleToSchema(topic, CAMERA_INFO_DATATYPES) ||
    ...
    settingsNodes() {
        ...
        for (const topic of this.renderer.topics ?? []) {
            ...
            const config = (configTopics[imageTopic] ?? {});
    ...
    handleSettingsAction = (action) => {
        ...
        if (!cameraInfo) {
            this.renderer.settings.errors.addToTopic(imageTopic, NO_CAMERA_INFO_ERR, `No CameraInfo received on ${cameraInfoTopic}`);
            return;
        }
        this.#recomputeCameraModel(renderable, cameraInfo);
        renderable.update();
    };
    #cameraInfoShouldSubscribe = (cameraInfoTopic) => {
        ...
        for (const topicConfig of Object.values(this.renderer.config.topics)) {
            const maybeImageConfig = topicConfig;
    ...
    handleImage = (messageEvent, image) => {
        ...
        if (settings.cameraInfoTopic == undefined) {
            ...
            const newCameraInfoTopic = prefix != undefined
                ? filterMap(this.#cameraInfoTopics, (topic) => topic.startsWith(prefix) ? topic : undefined).sort()[0]
                : undefined;
            ...
            if (newCameraInfoTopic == undefined) {
                this.renderer.settings.errors.addToTopic(imageTopic, NO_CAMERA_INFO_ERR, "No CameraInfo topic found");
                return;
            }
        ...
        if (!cameraInfo) {
            this.renderer.settings.errors.addToTopic(imageTopic, NO_CAMERA_INFO_ERR, `No CameraInfo received on ${settings.cameraInfoTopic}`);
        }
        else {
            this.#recomputeCameraModel(renderable, cameraInfo);
        }
     ...
     */
    #recomputeCameraModel(renderable, newCameraInfo) {
    ...
    #getImageRenderable(imageTopic, receiveTime, image, frameId) {
