...
import { FS_SRGB_TO_LINEAR, NEEDS_MIN_MAX, autoSelectColorSettings, colorModeSettingsFields, getColorConverter, } from "./colorMode";
...
import { getReader } from "./pointClouds/fieldReaders";
function zeroReader() {
    return 0;
}
const floatTextureColorModes = ["gradient", "colormap"];
...
function getTextureColorSpace(settings) {
...
export class FoxgloveGridRenderable extends Renderable {
    dispose() {
    ...
    #getRgbaFieldReaders(out, foxgloveGrid) {
    ...
    #getColorByFieldReader(foxgloveGrid, settings) {
        const { cell_stride } = foxgloveGrid;
        for (const field of foxgloveGrid.fields) {
    ...
    updateUniforms(foxgloveGrid, settings) {
        const { material } = this.userData;
        const { uniforms: { colorMode, colorMap, colorMapOpacity, minValue, maxValue, minGradientColorLinear, maxGradientColorLinear, }, } = material;
        ...
        minMaxColorValues(tempMinMaxColor, settings, foxgloveGrid.fields.find((field) => settings.colorField === field.name)?.type ??
            NumericType.UNKNOWN);
    ...
    updateTexture(foxgloveGrid, settings) {
        let texture = this.userData.texture;
        const fieldReader = this.#getColorByFieldReader(foxgloveGrid, settings);
        if (!fieldReader) {
            return;
        }
        const view = new DataView(foxgloveGrid.data.buffer, foxgloveGrid.data.byteOffset, foxgloveGrid.data.byteLength);
        ...
        if (floatMode) {
            ...
            const valueData = texture.image.data;
...
export class FoxgloveGrid extends SceneExtension {
    static extensionId = "foxglove.Grid";
    #fieldsByTopic = new Map();
    constructor(renderer, name = FoxgloveGrid.extensionId) {
        super(name, renderer);
    }
    getSubscriptions() {
        return [
    ...
    handleSettingsAction = (action) => {
        ...
        if (renderable) {
            ...
            if (action.payload.path[2] === "colorMode" ||
    ...
    #handleFoxgloveGrid = (messageEvent) => {
        ...
        if (renderable) {
            ...
            const geometry = this.renderer.sharedGeometry.getGeometry(this.constructor.name, createGridGeometry);
        ...
        this.#updateFoxgloveGridRenderable(renderable, foxgloveGrid, receiveTime, renderable.userData.settings);
    };
    #validateFoxgloveGrid(foxgloveGrid, topic) {
        ...
        if (foxgloveGrid.fields.length === 0) {
            invalidFoxgloveGridError(this.renderer, topic, `Grid has no fields. At least one field is required for the Grid to be displayed.`);
            return false;
        }
        ...
        if (cell_stride * cols > row_stride) {
            const message = `Grid row_stride (${row_stride}) does not allow for requisite column_count (${cols}) with cell stride (${cell_stride}). Minimum requisite bytes in row_stride needed: (${cols * cell_stride}) `;
            invalidFoxgloveGridError(this.renderer, topic, message);
            return false;
        }
        ...
        if (minBytesPerCell > cell_stride) {
            ...
            if (maxField) {
                message += ` required by “${maxField.name}” field, type=${numericTypeName(maxField.type)}, offset=${maxField.offset}`;
            }
            invalidFoxgloveGridError(this.renderer, topic, message);
            return false;
        }
    ...
    #updateFoxgloveGridRenderable(renderable, foxgloveGrid, receiveTime, settings) {
...
function invalidFoxgloveGridError(renderer, topic, message) {
    renderer.settings.errors.addToTopic(topic, INVALID_LICHTBLICK_GRID, message);
}
function createTexture(foxgloveGrid, settings) {
    ...
    const texture = new THREE.DataTexture(data, cols, rows, format, type, THREE.UVMapping, THREE.ClampToEdgeWrapping, THREE.ClampToEdgeWrapping, THREE.NearestFilter, THREE.LinearFilter, 1, getTextureColorSpace(settings));
    texture.generateMipmaps = false;
    return texture;
}
function createMesh(topic, texture, geometry) {
...
function numericTypeWidth(type) {
    switch (type) {
...
function createMaterial(texture, topic) {
...
function createPickingMaterial(originalMaterial) {
...
function normalizeFoxgloveGrid(message) {
    return {
...
function minMaxColorValues(output, settings, numericType) {
...
const NumericTypeMinMaxValueMap = {
