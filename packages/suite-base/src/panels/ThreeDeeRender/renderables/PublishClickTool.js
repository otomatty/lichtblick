...
export class PublishClickTool extends SceneExtension {
    ...
    constructor(renderer) {
        ...
        this.#arrow = new RenderableArrow("", makeArrowMarker(this.publishClickType), undefined, this.renderer);
    ...
    #handleMouseMove = (_cursorCoords, worldSpaceCursorCoords, _event) => {
    ...
    #handleClick = (_cursorCoords, worldSpaceCursorCoords, _event) => {
        if (!worldSpaceCursorCoords) {
            return;
        }
        switch (this.state) {
            ...
            case "place-second-point":
                ...
                if (this.#point1 && this.publishClickType !== "point") {
                    ...
                    const q = new THREE.Quaternion().setFromUnitVectors(UNIT_X, tempVec3.subVectors(this.#point2, this.#point1).normalize());
                    this.dispatchEvent({
                        type: "foxglove.publish-submit",
                        publishClickType: this.publishClickType,
                        pose: {
    ...
    #render() {
        if (this.publishClickType === "point") {
            ...
            if (this.#point1) {
                ...
                if (this.#point2) {
                    this.#arrow.quaternion.setFromUnitVectors(UNIT_X, tempVec3.subVectors(this.#point2, this.#point1).normalize());
                }
                else {
