...
import { decodeBGR8, decodeBGRA8, decodeBayerBGGR8, decodeBayerGBRG8, decodeBayerGRBG8, decodeBayerRGGB8, decodeFloat1c, decodeMono16, decodeMono8, decodeRGB8, decodeRGBA8, decodeUYVY, decodeYUYV, } from "@lichtblick/den/image";
...
export function isVideoKeyframe(frameMsg) {
...
export function getVideoDecoderConfig(frameMsg) {
    switch (frameMsg.format) {
        case "h264": {
...
export async function decodeCompressedVideoToBitmap(frameMsg, videoPlayer, firstMessageTime, resizeWidth) {
    ...
    const videoFrame = await videoPlayer.decode(frameMsg.data, timestampMicros, isVideoKeyframe(frameMsg) ? "key" : "delta");
    if (videoFrame) {
...
export const IMAGE_DEFAULT_COLOR_MODE_SETTINGS = {
    colorMode: "gradient",
    flatColor: "#ffffff",
 ...
 */
export function decodeRawImage(image, options, output) {
...
export function emptyVideoFrame(videoPlayer, resizeWidth) {
