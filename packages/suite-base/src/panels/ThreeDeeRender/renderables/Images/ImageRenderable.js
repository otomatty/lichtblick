...
import { decodeCompressedImageToBitmap, decodeCompressedVideoToBitmap, emptyVideoFrame, getVideoDecoderConfig, } from "./decodeImage";
import { DECODE_IMAGE_ERR_KEY, IMAGE_TOPIC_PATH } from "../ImageMode/constants";
const log = Logger.getLogger(__filename);
const DEFAULT_DISTANCE = 1;
const DEFAULT_PLANAR_PROJECTION_FACTOR = 0;
export const IMAGE_RENDERABLE_DEFAULT_SETTINGS = {
    visible: false,
    frameLocked: true,
...
export class ImageRenderable extends Renderable {
    ...
    updateHeaderInfo() {
        ...
        const rawFrameId = this.userData.cameraInfo?.header.frame_id ??
    ...
    setSettings(newSettings) {
        ...
        if (prevSettings.distance !== newSettings.distance ||
        ...
        if (prevSettings.colorMode !== newSettings.colorMode ||
    ...
    setImage(image, resizeWidth, onDecoded) {
    ...
    async #setErrorImage(seq, onDecoded) {
    ...
    async decodeImage(image, resizeWidth) {
        if ("format" in image) {
            if (VIDEO_FORMATS.has(image.format)) {
                const frameMsg = image;
                ...
                return await decodeCompressedVideoToBitmap(frameMsg, videoPlayer, this.userData.firstMessageTime, resizeWidth);
            }
    ...
    #updateTexture() {
        assert(this.#decodedImage, "Decoded image must be set before texture can be updated or created");
        ...
        if (decodedImage instanceof ImageBitmap) {
            ...
            if (canvasTexture == undefined ||
                ...
                !bitmapDimensionsEqual(decodedImage, canvasTexture.image)) {
...
function createCanvasTexture(bitmap) {
    const texture = new THREE.CanvasTexture(bitmap, THREE.UVMapping, THREE.ClampToEdgeWrapping, THREE.ClampToEdgeWrapping, THREE.NearestFilter, THREE.LinearFilter, THREE.RGBAFormat, THREE.UnsignedByteType);
    texture.generateMipmaps = false;
    texture.colorSpace = THREE.SRGBColorSpace;
    return texture;
}
function createDataTexture(imageData) {
    const dataTexture = new THREE.DataTexture(imageData.data, imageData.width, imageData.height, THREE.RGBAFormat, THREE.UnsignedByteType, THREE.UVMapping, THREE.ClampToEdgeWrapping, THREE.ClampToEdgeWrapping, THREE.NearestFilter, THREE.LinearFilter, 1, THREE.SRGBColorSpace);
    dataTexture.needsUpdate = true; // ensure initial image data is displayed
    return dataTexture;
}
function createGeometry(cameraModel, settings) {
...
const bitmapDimensionsEqual = (a, b) => a?.width === b?.width && a?.height === b?.height;
async function getErrorImage(width, height) {
