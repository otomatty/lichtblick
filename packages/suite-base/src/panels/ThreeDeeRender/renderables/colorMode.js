...
export function getColorConverter(settings, minValue, maxValue) {
...
export const RGBA_PACKED_FIELDS = new Set(["rgb", "rgba"]);
export const INTENSITY_FIELDS = new Set(["intensity", "i"]);
/**
 ...
 */
export function autoSelectColorSettings(output, fields, { supportsPackedRgbModes, supportsRgbaFieldsMode, }) {
...
export function colorModeSettingsFields({ msgFields, config, defaults, modifiers: { supportsPackedRgbModes, supportsRgbaFieldsMode, hideFlatColor, hideExplicitAlpha }, }) {
    ...
    if (msgFields && msgFields.length > 0) {
        if (supportsPackedRgbModes) {
            colorModeOptions.push({ label: t("threeDee:colorModeBgrPacked"), value: "rgb" }, { label: t("threeDee:colorModeBgraPacked"), value: "rgba" });
        }
...
const tempColor = { r: 0, g: 0, b: 0, a: 0 };
export function colorHasTransparency(settings) {
    switch (settings.colorMode) {
        case "flat":
            return stringToRgba(tempColor, settings.flatColor).a < 1.0;
        case "gradient":
            return (stringToRgba(tempColor, settings.gradient[0]).a < 1.0 ||
