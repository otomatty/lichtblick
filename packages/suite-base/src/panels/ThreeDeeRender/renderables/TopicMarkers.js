...
import { getMarkerId } from "./markers/RenderableMarker";
import { missingTransformMessage, MISSING_TRANSFORM } from "./transforms";
import { Renderable } from "../Renderable";
import { MarkerAction, MarkerType } from "../ros";
...
const DEFAULT_NAMESPACE_SETTINGS = {
    visible: true,
};
export class MarkersNamespace {
    ...
    markersById = new Map();
    settings;
    constructor(topic, namespace, renderer) {
...
export class TopicMarkers extends Renderable {
    pickable = false;
    namespaces = new Map();
    dispose() {
    ...
    addMarkerMessage(marker, receiveTime) {
        switch (marker.action) {
            ...
            default:
                // Unknown action
                this.renderer.settings.errors.addToTopic(this.topic, INVALID_MARKER_ACTION, `Invalid marker action ${marker.action}`);
        }
    }
    update() {
    ...
    #addOrUpdateMarker(marker, receiveTime) {
    ...
    #createMarkerRenderable(marker, receiveTime) {
        ...
        switch (marker.type) {
            ...
            case MarkerType.LINE_STRIP:
                if (marker.points.length === 0) {
                    ...
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_LINE_STRIP, `LINE_STRIP marker ${markerId} has no points`);
                    return undefined;
                }
                else if (marker.points.length === 1) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_LINE_STRIP, `LINE_STRIP marker ${markerId} only has one point`);
                    return undefined;
                }
            ...
            case MarkerType.LINE_LIST:
                if (marker.points.length === 0) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_LINE_LIST, `LINE_LIST marker ${markerId} has no points`);
                    return undefined;
                }
                else if (marker.points.length === 1) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_LINE_LIST, `LINE_LIST marker ${markerId} only has one point`);
                    return undefined;
                }
                else if (marker.points.length % 2 !== 0) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_LINE_LIST, `LINE_LIST marker ${markerId} has an odd number of points (${marker.points.length})`);
            ...
            case MarkerType.CUBE_LIST:
                if (marker.points.length === 0) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_CUBE_LIST, `CUBE_LIST marker ${markerId} has no points`);
                    return undefined;
                }
            ...
            case MarkerType.SPHERE_LIST:
                if (marker.points.length === 0) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_SPHERE_LIST, `SPHERE_LIST marker ${markerId} has no points`);
                    return undefined;
                }
            ...
            case MarkerType.POINTS:
                if (marker.points.length === 0) {
                    const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                    this.renderer.settings.errors.addToTopic(this.topic, INVALID_POINTS_LIST, `POINTS marker ${markerId} has no points`);
                    return undefined;
                }
            ...
            case MarkerType.MESH_RESOURCE: {
                ...
                renderable.update(marker, receiveTime, true);
                return renderable;
            }
            ...
            default: {
                const markerId = getMarkerId(this.topic, marker.ns, marker.id);
                this.renderer.settings.errors.addToTopic(this.topic, INVALID_MARKER_TYPE, `Marker ${markerId} has invalid type ${marker.type}`);
                return undefined;
            }
        }
    }
