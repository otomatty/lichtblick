...
import { makePose, MAX_DURATION, Transform } from "../transforms";
...
class FrameAxisRenderable extends Renderable {
    dispose() {
...
export class FrameAxes extends SceneExtension {
    ...
    #lineGeometry;
    #defaultRenderableSettings;
    constructor(renderer, defaultRenderableSettings, name = FrameAxes.extensionId) {
        super(name, renderer);
        const linewidth = this.renderer.config.scene.transforms?.lineWidth ?? DEFAULT_LINE_WIDTH_PX;
        const color = stringToRgb(new THREE.Color(), this.renderer.config.scene.transforms?.lineColor ?? DEFAULT_LINE_COLOR_STR);
        ...
        this.#lineGeometry = this.renderer.sharedGeometry.getGeometry(this.constructor.name, createLineGeometry);
        ...
        const children = {
            settings: {
                ...
                fields: {
                    ...
                    axisScale: fieldSize(t("threeDee:axisScale"), config.scene.transforms?.axisScale, DEFAULT_AXIS_SCALE),
                    lineWidth: {
    ...
    setColorScheme(colorScheme, backgroundColor) {
        ...
        for (const renderable of this.renderables.values()) {
            renderable.userData.label.setColor(this.#labelForegroundColor, this.#labelForegroundColor, this.#labelForegroundColor);
            renderable.userData.label.setBackgroundColor(this.#labelBackgroundColor.r, this.#labelBackgroundColor.g, this.#labelBackgroundColor.b);
        }
    }
    #setLabelSize(size) {
        for (const renderable of this.renderables.values()) {
    ...
    handleSettingsAction = (action) => {
        ...
        const toggleFrameVisibility = (value) => {
            for (const renderable of this.renderables.values()) {
    ...
    #getRenderableSettingsWithDefaults(partialDefinedSettings) {
    ...
    #addFrameAxis(frameId) {
        ...
        label.setBackgroundColor(this.#labelBackgroundColor.r, this.#labelBackgroundColor.g, this.#labelBackgroundColor.b);
...
function buildSettingsFields(frame, currentTime, config) {
    ...
    if (config.scene.transforms?.editable ?? DEFAULT_EDITABLE) {
        let xyzOffsetValue = config.transforms[frameKey]?.xyzOffset;
        let rpyCoefficient = config.transforms[frameKey]?.rpyCoefficient;
...
function vec3IsZero(v, eps = 1e-6) {
    return Math.abs(v[0]) < eps && Math.abs(v[1]) < eps && Math.abs(v[2]) < eps;
}
function getOffset(maybeOffset) {
    if (!maybeOffset) {
        return undefined;
    }
