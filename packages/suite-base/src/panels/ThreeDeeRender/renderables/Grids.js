...
import { MarkerAction, MarkerType, TIME_ZERO } from "../ros";
...
export class Grids extends SceneExtension {
    static extensionId = "foxglove.Grids";
    constructor(renderer, name = Grids.extensionId) {
        super(name, renderer);
        ...
        for (const [instanceId, entry] of Object.entries(renderer.config.layers)) {
            if (entry?.layerId === LAYER_ID) {
                this.#updateGrid(instanceId, entry);
            }
        }
    }
    dispose() {
    ...
    #updateGrid(instanceId, settings) {
        ...
        const markersEqual = newSettings.size === prevSettings.size &&
        ...
        if (!vec3TupleApproxEquals(newSettings.position, prevSettings.position) ||
            !vec3TupleApproxEquals(newSettings.rotation, prevSettings.rotation)) {
            renderable.userData.pose = xyzrpyToPose(newSettings.position, newSettings.rotation);
        }
    }
    #createRenderable(instanceId, settings) {
        const marker = createMarker(settings);
        const lineListId = `${instanceId}:LINE_LIST`;
        const lineList = new RenderableLineList(lineListId, marker, undefined, this.renderer, LINE_OPTIONS);
