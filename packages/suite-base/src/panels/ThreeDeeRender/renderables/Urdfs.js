...
import { parseRobot, } from "@lichtblick/den/urdf";
...
import { JOINTSTATE_DATATYPES, MarkerAction, MarkerType, } from "../ros";
...
const XYZ_LABEL = ["X", "Y", "Z"];
const RPY_LABEL = ["R", "P", "Y"];
...
const URDF_TOPIC_SCHEMAS = new Set(["std_msgs/String", "std_msgs/msg/String"]);
const tempVec3a = new THREE.Vector3();
const tempVec3b = new THREE.Vector3();
const tempQuaternion1 = new THREE.Quaternion();
const tempQuaternion2 = new THREE.Quaternion();
const tempEuler = new THREE.Euler();
var EmbeddedMaterialUsage;
(function (EmbeddedMaterialUsage) {
...
export class UrdfRenderable extends Renderable {
    dispose() {
        this.removeChildren();
...
export class Urdfs extends SceneExtension {
    static extensionId = "foxglove.Urdfs";
    #framesByInstanceId = new Map();
    #transformsByInstanceId = new Map();
    #jointStates = new Map();
    #textDecoder = new TextDecoder();
    #urdfsByTopic = new Map();
    constructor(renderer, name = Urdfs.extensionId) {
        super(name, renderer);
    ...
    settingsNodes() {
        ...
        if (topic != undefined) {
            const config = (this.renderer.config.topics[TOPIC_NAME] ?? {});
            const fields = {
                displayMode: {
            ...
            entries.push({
                ...
                node: {
                    ...
                    children: urdfChildren(this.#transformsByInstanceId.get(TOPIC_NAME), this.renderer.transformTree, this.#jointStates),
                },
            });
        }
        ...
        if (parameter != undefined) {
            const config = (this.renderer.config.topics[PARAM_KEY] ?? {});
            const fields = {
                displayMode: {
                    ...baseDisplayModeField,
                    value: config.displayMode ?? DEFAULT_SETTINGS.displayMode,
            ...
            entries.push({
                ...
                node: {
                    ...
                    visible: config.visible ?? DEFAULT_SETTINGS.visible,
                    handler: this.#handleTopicSettingsAction,
                    children: urdfChildren(this.#transformsByInstanceId.get(PARAM_KEY), this.renderer.transformTree, this.#jointStates),
                },
            });
        }
        ...
        for (const [instanceId, layerConfig] of Object.entries(this.renderer.config.layers)) {
            if (layerConfig?.layerId === LAYER_ID) {
                ...
                const fields = {
                    ...
                    url: (config.sourceType ?? DEFAULT_CUSTOM_SETTINGS.sourceType) === "url"
                        ? {
                            label: "URL",
                            input: "string",
                            placeholder: "package://",
                        ...
                        : undefined,
                    filePath: config.sourceType === "filePath"
                        ? {
                            label: "File path",
                            input: "string",
                        ...
                        : undefined,
                    topic: config.sourceType === "topic"
                        ? {
                            label: "Topic",
                            input: "autocomplete",
                            value: config.topic ?? DEFAULT_CUSTOM_SETTINGS.topic,
                            items: filterMap(this.renderer.topics ?? [], (_topic) => URDF_TOPIC_SCHEMAS.has(_topic.schemaName) ? _topic.name : undefined),
                        }
                        : undefined,
                    parameter: config.sourceType === "param"
                        ? {
                            label: "Parameter",
                            input: "autocomplete",
                            value: config.parameter ?? DEFAULT_CUSTOM_SETTINGS.parameter,
                            items: filterMap(this.renderer.parameters ?? [], ([paramName, value]) => typeof value === "string" ? paramName : undefined),
                        }
                        : undefined,
                    label: {
                        label: "Label",
                ...
                entries.push({
                    ...
                    node: {
                        ...
                        children: urdfChildren(this.#transformsByInstanceId.get(instanceId), this.renderer.transformTree, this.#jointStates),
                    },
                });
            }
        }
    ...
    startFrame(currentTime, renderFrameId, fixedFrameId) {
        for (const renderable of this.renderables.values()) {
            ...
            for (const childRenderable of renderable.userData.renderables.values()) {
                ...
                const updated = updatePose(childRenderable, this.renderer.transformTree, renderFrameId, fixedFrameId, frameId, currentTime, srcTime);
                if (!updated) {
    ...
    #handleTopicSettingsAction = (action) => {
        if (action.action === "update") {
            this.#handleSettingsUpdate(action);
            return;
        }
    };
    #handleLayerSettingsAction = (action) => {
    ...
    #handleSettingsUpdate = (action) => {
        ...
        if (path.length === 5 && path[2] === "joints") {
            ...
            if (isAngular) {
                ...
                const scale = action.payload.value;
            ...
            const [_layers, instanceId, field] = path;
            const renderable = this.renderables.get(instanceId);
            ...
            if (field === "url" || field === "filePath") {
                ...
                urdf = this.renderer.parameters?.get(action.payload.value);
                ...
                const sourceType = action.payload.value;
                if (sourceType === "topic") {
                    ...
                    urdf = renderable?.userData.parameter
                        ? this.renderer.parameters?.get(renderable.userData.parameter)
                        : undefined;
                }
                else {
                    urdf = undefined;
                ...
                urdf = this.#urdfsByTopic.get(action.payload.value);
    ...
    #handleRobotDescription = (messageEvent) => {
        ...
        const subscribedInstanceIds = filterMap(this.renderables, ([instanceId, renderable]) => renderable.userData.sourceType === "topic" && renderable.userData.topic === topic
            ? instanceId
            : undefined);
    ...
    #shouldSubscribe = (topic) => {
        return Array.from(this.renderables.values()).some((renderable) => renderable.userData.sourceType === "topic" && renderable.userData.topic === topic);
    };
    #handleJointState = (messageEvent) => {
    ...
    #handleParametersChange = (parameters) => {
        ...
        for (const [instanceId, renderable] of this.renderables.entries()) {
            const sourceType = renderable.userData.settings
                .sourceType;
            const paramName = renderable.userData.settings
                .parameter;
    ...
    #handleAddUrdf = (instanceId) => {
        ...
        const config = { ...DEFAULT_CUSTOM_SETTINGS, instanceId };
    ...
    #loadUrdf(args) {
        ...
        const label = settings.label ?? DEFAULT_CUSTOM_SETTINGS.label;
        ...
        parseUrdf(urdf, async (uri) => await this.#getFileFetch(uri, baseUrl), framePrefix)
            ...
            this.renderer.settings.errors.remove(loadedRenderable.userData.settingsPath, PARSE_URDF_ERR);
    ...
    #loadRobot(renderable, { robot, frames, transforms }, baseUrl) {
        ...
        renderable.removeChildren();
        const createChild = (frameId, i, visual) => {
        ...
        for (const link of robot.links.values()) {
            ...
            const renderCollision = displayMode === "collision" || (displayMode === "auto" && link.visuals.length === 0);
            if (renderVisual) {
    ...
    #loadTransforms(instanceId, transforms) {
...
async function parseUrdf(text, getFileContents, framePrefix) {
    ...
    try {
        ...
        if (framePrefix) {
            robot.links = new Map([...robot.links].map(([name, link]) => [
                applyFramePrefix(name),
                { ...link, name: applyFramePrefix(link.name) },
            ]));
            robot.joints = new Map([...robot.joints].map(([name, joint]) => [
                name,
                {
                    ...joint,
...
function getColor(visual, robot) {
    if (!visual.material) {
        return undefined;
    }
...
function createMarker(frameId, type, pose, scale, color) {
    return {
...
function createMeshMarker(frameId, pose, embeddedMaterialUsage, mesh, baseUrl, color) {
...
function urdfChildren(transforms, transformTree, jointStates) {
    if (!transforms) {
        return {};
    }
    const jointChildren = {};
...
function signedAngleAroundAxis(rotation, axis) {
