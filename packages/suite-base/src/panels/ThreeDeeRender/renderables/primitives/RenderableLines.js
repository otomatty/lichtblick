...
import { LineType } from "@foxglove/schemas";
...
export class RenderableLines extends RenderablePrimitive {
    ...
    #updateLines(lines) {
        ...
        for (idx; idx < lines.length; idx++) {
            ...
            lineRenderable.position.set(primitive.pose.position.x, primitive.pose.position.y, primitive.pose.position.z);
            lineRenderable.quaternion.set(primitive.pose.orientation.x, primitive.pose.orientation.y, primitive.pose.orientation.z, primitive.pose.orientation.w);
...
class LinePrimitiveRenderable extends THREE.Object3D {
    ...
    #color;
    #lineType;
    constructor(primitive, canvasSize) {
        super();
    ...
    update() {
        ...
        const numVertices = (useIndices ? this.#primitive.indices.length : this.#primitive.points.length) +
            (isLoop ? 1 : 0);
        const necessaryPositionBufferSize = numVertices * 3;
        if (this.#primitiveChanged) {
            const geometryNeedsRecreated = this.#geometry == undefined ||
        ...
        if (this.#colorChanged || this.#primitiveChanged) {
            ...
            if (singleColor == undefined) {
                ...
                this.#material.color.setRGB(1, 1, 1); // any non-white color will tint the vertex colors
                ...
                const instanceColorBuffer = new THREE.InstancedInterleavedBuffer(this.#colorBuffer, isSegments ? 8 : 4, 1);
                this.#geometry.setAttribute("instanceColorStart", new THREE.InterleavedBufferAttribute(instanceColorBuffer, 4, 0));
                this.#geometry.setAttribute("instanceColorEnd", new THREE.InterleavedBufferAttribute(instanceColorBuffer, 4, 4));
            }
            else {
...
function serializePositions(positionsOut, primitive) {
    let i = 0;
    assert(positionsOut.length >= primitive.points.length * 3, `positionsOut must have a length (${positionsOut.length})  >= to primitive.points.length (${primitive.points.length}) * 3`);
...
function serializePositionsWithIndices(positionsOut, primitive) {
    ...
    assert(positionsOut.length >= primitive.indices.length * 3, `positionsOut must have a length (${positionsOut.length})  >= to primitive.indices.length (${primitive.indices.length}) * 3`);
    let i = 0;
...
function serializeColors(colorsOut, primitive) {
    assert(colorsOut.length >= primitive.colors.length * 4, `colorsOut buffer must have a length (${colorsOut.length}) >= to the primitive.colors.length (${primitive.colors.length}) * 4`);
...
function serializeColorsWithIndices(colorsOut, primitive) {
    ...
    assert(colorsOut.length >= indices.length * 4, `colorsOut buffer must have a length (${colorsOut.length}) >= to primitive.indices.length (${primitive.indices.length}) * 4`);
    assert(primitive.colors.length > 0, "Invariant: expected not to be using vertex colors");
    let i = 0;
    for (const idx of indices) {
