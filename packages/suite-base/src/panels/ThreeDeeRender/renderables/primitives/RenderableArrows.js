...
export class RenderableArrows extends RenderablePrimitive {
    ...
    constructor(renderer) {
        ...
        const shaftEdgesGeometry = renderer.sharedGeometry.getGeometry(`${this.constructor.name}-shaftedges`, () => createShaftEdgesGeometry(this.#shaftGeometry));
        this.#shaftOutlineGeometry = new THREE.InstancedBufferGeometry();
        this.#shaftOutlineGeometry.copy(shaftEdgesGeometry);
        this.#shaftOutlineGeometry.setAttribute("instanceMatrix", this.#shaftMesh.instanceMatrix);
        this.#shaftOutline = new THREE.LineSegments(this.#shaftOutlineGeometry, renderer.instancedOutlineMaterial);
        this.#shaftOutline.frustumCulled = false;
        this.#shaftOutline.userData.picking = false;
        this.add(this.#shaftOutline);
        const headEdgesGeometry = renderer.sharedGeometry.getGeometry(`${this.constructor.name}-headedges`, () => createHeadEdgesGeometry(this.#headGeometry));
        this.#headOutlineGeometry = new THREE.InstancedBufferGeometry();
        this.#headOutlineGeometry.copy(headEdgesGeometry);
        this.#headOutlineGeometry.setAttribute("instanceMatrix", this.#headMesh.instanceMatrix);
        this.#headOutline = new THREE.LineSegments(this.#headOutlineGeometry, renderer.instancedOutlineMaterial);
        this.#headOutline.frustumCulled = false;
        this.#headOutline.userData.picking = false;
        this.add(this.#headOutline);
    }
    #ensureCapacity(numArrows) {
        if (numArrows > this.#maxInstances) {
            ...
            const shaftEdgesGeometry = this.renderer.sharedGeometry.getGeometry(`${this.constructor.name}-shaftedges`, () => createShaftEdgesGeometry(this.#shaftGeometry));
            this.#shaftOutlineGeometry = new THREE.InstancedBufferGeometry();
            this.#shaftOutlineGeometry.copy(shaftEdgesGeometry);
            ...
            const headEdgesGeometry = this.renderer.sharedGeometry.getGeometry(`${this.constructor.name}-headedges`, () => createHeadEdgesGeometry(this.#headGeometry));
            this.#headOutlineGeometry = new THREE.InstancedBufferGeometry();
            this.#headOutlineGeometry.copy(headEdgesGeometry);
    ...
    #updateMesh(arrows) {
        ...
        for (const arrow of arrows) {
            ...
            tempQuat.set(arrow.pose.orientation.x, arrow.pose.orientation.y, arrow.pose.orientation.z, arrow.pose.orientation.w);
            this.#shaftMesh.setMatrixAt(i, tempMat4.compose(tempVec3.set(arrow.pose.position.x, arrow.pose.position.y, arrow.pose.position.z), tempQuat, tempVec3_2.set(arrow.shaft_length, arrow.shaft_diameter, arrow.shaft_diameter)));
            ...
            this.#headMesh.setMatrixAt(i, tempMat4.compose(tempVec3, tempQuat, tempVec3_2.set(arrow.head_length, arrow.head_diameter, arrow.head_diameter)));
            i++;
        }
...
function createShaftEdgesGeometry(geometry) {
...
function createHeadEdgesGeometry(geometry) {
