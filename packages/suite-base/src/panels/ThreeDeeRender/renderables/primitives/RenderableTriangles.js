        ...
        for (const primitive of tris) {
            ...
            for (let i = 0; i < primitive.points.length; i++) {
                ...
                if (!isPointValid(point)) {
                    this.addError(`${this.name}-${INVALID_POINT_ERROR_ID}`, `Entity: ${this.userData.entity?.id}.triangles[${triMeshIdx}](1st index) - Point definition at index ${i} is not finite`);
                    continue;
                }
                vertChanged =
                    vertChanged ||
                ...
                if (!singleColor && colors && primitive.colors.length > 0) {
                    ...
                    if (i === primitive.points.length - 1 && color === missingColor) {
                        ...
                        this.addError(`${this.name}-${COLOR_LENGTH_ERROR_ID}`, `Entity: ${this.userData.entity?.id}.triangles[${triMeshIdx}](1st index) - Colors array should be same size as points array, showing #00ff00 instead`);
                    }
            ...
            const vertexColorChanged = !material.vertexColors && !singleColor && primitive.colors.length > 0;
            if (vertexColorChanged) {
                ...
                const materialNeedsUpdate = material.vertexColors ||
            ...
            mesh.position.set(primitive.pose.position.x, primitive.pose.position.y, primitive.pose.position.z);
            mesh.quaternion.set(primitive.pose.orientation.x, primitive.pose.orientation.y, primitive.pose.orientation.z, primitive.pose.orientation.w);
            this.add(mesh);
            triMeshIdx++;
        }
    }
...
function makeTriangleMesh() {
    return new THREE.Mesh(new DynamicBufferGeometry(), new THREE.MeshStandardMaterial({
        metalness: 0,
        roughness: 1,
        flatShading: true,
