...
import { MISSING_TRANSFORM, missingTransformMessage, } from "@lichtblick/suite-base/panels/ThreeDeeRender/renderables/transforms";
...
import { colorHasTransparency, colorModeSettingsFields, FS_SRGB_TO_LINEAR, } from "./colorMode";
 ...
 */
export function pointSettingsNode(topic, messageFields, config, defaultSettings = DEFAULT_POINT_SETTINGS) {
    const pointSize = config.pointSize;
    const pointShape = config.pointShape ?? "circle";
    const decayTime = config.decayTime;
    const checkColorsForDatatypes = (pointCloudDatatypes) => {
        return (pointCloudDatatypes.has(topic.schemaName) ||
 ...
 * @returns
 */
export function createGeometry(topic, usage) {
...
export function pointCloudColorEncoding(settings) {
...
export class PointsRenderable extends Renderable {
    #points;
    geometry;
    pickableInstances = true;
    constructor(name, userData, geometry, material, pickingMaterial, instancePickingMaterial) {
...
`;
export function pointCloudMaterial(settings) {
...
export function createPickingMaterial(settings) {
...
export function createInstancePickingMaterial(settings) {
    const MIN_PICKING_POINT_SIZE = 8;
    // Use a custom shader for picking that sets a minimum point size to make
    // individual points easier to click on
    const pointSize = Math.max(settings.pointSize, MIN_PICKING_POINT_SIZE);
 ...
 */
export class RenderObjectHistory {
    #history;
    #parentRenderable;
    #renderer;
    ...
    updateHistoryFromCurrentTime(currentTime) {
        ...
        const expireTime = decayTime > 0 ? currentTime - BigInt(Math.round(decayTime * 1e9)) : MAX_DURATION;
    ...
    updatePoses(currentTime, renderFrameId, fixedFrameId) {
        ...
        for (const entry of this.#history) {
            ...
            const updated = updatePose(entry.renderable, this.#renderer.transformTree, renderFrameId, fixedFrameId, frameId, currentTime, srcTime);
            if (!updated && !hadTfError) {
                const message = missingTransformMessage(renderFrameId, fixedFrameId, frameId);
                this.#renderer.settings.errors.add(this.#parentRenderable.userData.settingsPath, MISSING_TRANSFORM, message);
                hadTfError = true;
            }
        }
    }
