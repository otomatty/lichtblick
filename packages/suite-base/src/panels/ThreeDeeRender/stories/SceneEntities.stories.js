...
const primitives = {
    ...
    lines: [LineType.LINE_STRIP, LineType.LINE_LOOP, LineType.LINE_LIST].flatMap((type, typeIndex) => [
        {
        ...
        },
        {
            ...
            thickness: 0.05,
            scale_invariant: false,
            points: rearrange(new Array(10).fill(0).map((_, i, { length }) => ({
                x: 0.25 * Math.cos((2 * Math.PI * i) / length),
                y: 0.25 * Math.sin((2 * Math.PI * i) / length),
                z: 0,
            }))),
        ...
        },
        {
            ...
            color: makeColor("#7995fb", 0.8),
            colors: rearrange(new Array(10).fill(0).map((_, i, { length }) => {
                const { r, g, b, a } = tinycolor.fromRatio({ h: i / (length - 1), s: 1, v: 1 }).toRgb();
                return { r: r / 255, g: g / 255, b: b / 255, a };
            })),
            // Should make a flat-pointed star, LINE_LOOP should have a line across it
...
function makeStoryScene({ topic, frameId, }) {
    return {
        topic,
...
export const UpdatedLineLoopsDontHaveExtraLines = {
    parameters: {
        useReadySignal: true,
        colorScheme: "dark",
    },
...
function makeMultiEntityScene({ topic, frameId, }) {
    const entityData = {
        timestamp: { sec: 0, nsec: 0 },
        frame_id: frameId,
        lifetime: { sec: 0, nsec: 0 },
    ...
    const entities = [];
    const allPrimitivesAsEmpty = Object.keys(primitives).reduce((acc, primitiveType) => ({ ...acc, [primitiveType]: [] }), {});
    for (const primitiveType of Object.keys(primitives)) {
        const primitiveArray = primitives[primitiveType];
        let i = 0;
...
function CheckVisibleAfterSeek() {
    const readySignal = useReadySignal();
    const frames = useMemo(() => {
...
// that we correctly handle the case where the `#positionBuffer` in `RenderableLines/LinePrimitiveRenderable` is bigger than
// the number of points after a `SceneUpdate`
export const ProcessesSameSceneTwice = {
    parameters: {
        useReadySignal: true,
        colorScheme: "dark",
    },
