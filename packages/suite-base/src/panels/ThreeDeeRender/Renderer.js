...
import { fromNanoSec, isLessThan, toNanoSec } from "@lichtblick/rostime";
...
import { normalizeFrameTransform, normalizeFrameTransforms, normalizeTFMessage, normalizeTransformStamped, } from "./normalizeMessages";
...
import { AddTransformResult, CoordinateFrame, DEFAULT_MAX_CAPACITY_PER_FRAME, TransformTree, Transform, } from "./transforms";
...
export class Renderer extends EventEmitter {
    ...
    transformTree = new TransformTree(this.#transformPool);
    coordinateFrameList = [];
    currentTime = 0n;
    fixedFrameId;
    followFrameId;
    labelPool = new LabelPool({ fontFamily: fontMonospace });
    ...
    constructor(args) {
        ...
        for (const extensionItem of Object.values(extensionsById)) {
            if (extensionItem.supportedInterfaceModes == undefined ||
                extensionItem.supportedInterfaceModes.includes(interfaceMode)) {
                this.#addSceneExtension(extensionItem.init(this));
            }
        }
        log.debug(`Renderer initialized with scene extensions ${Array.from(this.sceneExtensions.keys()).join(", ")}`);
    ...
    #watchDevicePixelRatio() {
        this.#devicePixelRatioMediaQuery = window.matchMedia(`(resolution: ${window.devicePixelRatio}dppx)`);
     ...
     */
    clear({ clearTransforms, resetAllFramesCursor, clearImageModeExtension = true, } = {
     ...
     */
    handleAllFramesMessages(allFrames) {
        ...
        if (this.#allFramesCursor.lastReadMessage != undefined &&
    ...
    #addSubscriptionsFromSceneExtensions(filterFn) {
    ...
    #addSchemaSubscriptions(schemaNames, subscription) {
    ...
    #addTopicSubscription(topic, subscription) {
    ...
    enableImageOnlySubscriptionMode = () => {
        assert(this.#imageModeExtension, "Image mode extension should be defined when calling enable Image only mode");
        this.clear({
        ...
        this.#addSubscriptionsFromSceneExtensions((extension) => extension === this.#imageModeExtension);
    ...
    #imageOnlyModeTopicSettingsValidator = (entry, errors) => {
        const { path, node } = entry;
        if (path[0] === "topics") {
            if (node.visible === true) {
                errors.addToTopic(path[1], "IMAGE_ONLY_TOPIC", "Camera calibration information is required to display 3D topics");
            }
            else {
    ...
    #updateTopicsAndCustomLayerSettingsNodes() {
        this.settings.setNodesForKey(RENDERER_ID, [
            this.#getTopicsSettingsEntry(),
            this.#getCustomLayersSettingsEntry(),
        ]);
    }
    #getTopicsSettingsEntry() {
    ...
    setColorScheme(colorScheme, backgroundColor) {
    ...
    setSelectedRenderable(selection) {
        ...
        if (selection) {
            ...
            log.debug(`Selected ${selection.renderable.id} (${selection.renderable.name}) (instance=${selection.instanceIndex})`, selection.renderable);
        }
    ...
    addMessageEvent(messageEvent) {
        ...
        const maybeHasHeader = message;
        const maybeHasMarkers = message;
        const maybeHasEntities = message;
        const maybeHasFrameId = message;
    ...
    #addFrameTransform(transform) {
        ...
        try {
            ...
            const err = e;
            this.settings.errors.add(["transforms"], ADD_TRANSFORM_ERROR, `Error adding transform for frame ${childId}: ${err.message}`);
        }
    }
    #addTransformMessage(tf) {
        ...
        try {
            ...
            const err = e;
            this.settings.errors.add(["transforms"], ADD_TRANSFORM_ERROR, `Error adding transform for frame ${normalizedChildId}: ${err.message}`);
        }
    }
    // Create a new transform and add it to the renderer's TransformTree
    addTransform(parentFrameId, childFrameId, stamp, translation, rotation, errorSettingsPath) {
        const t = translation;
        const q = rotation;
        tempVec3[0] = t.x;
        tempVec3[1] = t.y;
        ...
        if (status === AddTransformResult.CYCLE_DETECTED) {
            this.settings.errors.add(["transforms", `frame:${childFrameId}`], CYCLE_DETECTED, `Transform tree cycle detected: Received transform with parent "${parentFrameId}" and child "${childFrameId}", but "${childFrameId}" is already an ancestor of "${parentFrameId}". Transform message dropped.`);
            if (errorSettingsPath) {
                this.settings.errors.add(errorSettingsPath, CYCLE_DETECTED, `Attempted to add cyclical transform: Frame "${parentFrameId}" cannot be the parent of frame "${childFrameId}". Transform message dropped.`);
            }
        }
        ...
        if (frame.transformsSize() === frame.maxCapacity) {
            this.settings.errors.add(["transforms", `frame:${childFrameId}`], TF_OVERFLOW, `[Warning] Transform history is at capacity (${frame.maxCapacity}), old TFs will be dropped`);
        }
    }
    ...
    #frameHandler = (currentTime) => {
        ...
        const renderFrameId = this.followFrameId && this.transformTree.frame(this.followFrameId)
            ? this.followFrameId
    ...
    #updateFixedFrameId() {
        const frame = this.followFrameId != undefined ? this.transformTree.frame(this.followFrameId) : undefined;
        if (frame == undefined) {
    ...
    #clickHandler = (cursorCoords) => {
        ...
        const selections = [];
        let curSelection;
        while ((curSelection = this.#pickSingleObject(cursorCoords)) &&
    ...
    #handleCustomLayersAction = (action) => {
        const path = action.payload.path;
    ...
    #pickSingleObject(cursorCoords) {
        ...
        const objectId = this.#picker.pick(cursorCoords.x, cursorCoords.y, this.cameraHandler.getActiveCamera(), { debug: this.debugPicking, disableSetViewOffset: this.interfaceMode === "image" });
        if (objectId === -1) {
        ...
        let renderable;
        let maybeRenderable = pickedObject;
        ...
        if (!renderable) {
            log.warn(`No Renderable found for objectId ${objectId} (name="${pickedObject?.name}" uuid=${pickedObject?.uuid})`);
            return undefined;
        }
        log.debug(`Picking pass returned ${renderable.id} (${renderable.name})`, renderable);
        let instanceIndex;
        if (renderable.pickableInstances) {
            instanceIndex = this.#picker.pickInstance(cursorCoords.x, cursorCoords.y, this.cameraHandler.getActiveCamera(), renderable, { debug: this.debugPicking, disableSetViewOffset: this.interfaceMode === "image" });
    ...
    #updateFrameErrors() {
        if (this.followFrameId == undefined) {
            // No frames available
            this.settings.errors.add(FOLLOW_TF_PATH, NO_FRAME_SELECTED, i18next.t("threeDee:noCoordinateFramesFound"));
            return;
        }
        ...
        if (!frame) {
            this.settings.errors.add(FOLLOW_TF_PATH, FOLLOW_FRAME_NOT_FOUND, i18next.t("threeDee:frameNotFound", {
    ...
    getContextMenuItems = () => {
        return Array.from(this.sceneExtensions.values()).flatMap((extension) => extension.getContextMenuItems());
    };
    #updateResolution() {
 ...
 */
function baseSettingsTree(interfaceMode) {
    const keys = [];
