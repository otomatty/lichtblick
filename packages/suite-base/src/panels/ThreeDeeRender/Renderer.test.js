...
function createTFMessageEvent(parentId, childId, receiveTime, headerStamps, topic = "/tf") {
...
const fetchAsset = async (uri, options) => {
...
describe("3D Renderer", () => {
    ...
    it("does not set a unfollow pose snapshot  when in follow-pose mode", () => {
        ...
        const cameraState = renderer.sceneExtensions.get("foxglove.CameraStateSettings");
...
describe("Renderer.handleAllFramesMessages behavior", () => {
    ...
    it("resets cursor if messages were added before the cursor index", () => {
        ...
        renderer.setCurrentTime(5n);
        renderer.handleAllFramesMessages(msgs);
        const numMessagesBeforeTime = msgs.filter((msg) => toNanoSec(msg.message.transforms[0].header.stamp) <= 5n).length;
