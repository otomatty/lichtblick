...
export function getDiagnosticId(hardwareId, name) {
...
export function computeDiagnosticInfo(status, stamp) {
    ...
    if (status.values.some(({ value }) => value.length > MAX_STRING_LENGTH)) {
        validatedStatus = {
            ...status,
            values: status.values.map((kv) => kv.value.length > MAX_STRING_LENGTH
...
export function getDiagnosticsByLevel(diagnosticsByHardwareId) {
    const ret = new Map();
...
export const filterAndSortDiagnostics = (nodes, hardwareIdFilter, pinnedIds) => {
