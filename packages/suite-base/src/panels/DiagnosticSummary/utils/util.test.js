...
import { LEVELS, MAX_STRING_LENGTH, } from "@lichtblick/suite-base/panels/DiagnosticSummary/constants";
import { getDiagnosticId, getDiagnosticsByLevel, filterAndSortDiagnostics, computeDiagnosticInfo, } from "./util";
...
const cameraStatus = {
    status: {
        level: 1,
        name: "status",
        message: "Ground rendering using lidar_aligned frame. This is okay for playing back an old bag but NOT okay on the car.",
...
const usrrStatus = {
    status: {
        level: 2,
        name: "status",
        message: "Error processing raw radar from: USRR Segmentation Node: car velocity exceeds threshold for Usrr Segmentation.",
...
const diagnosticsByHardwareId = new Map([
    ...okMap,
    ...warnMap,
    ...errorMap,
]);
describe("diagnostics", () => {
    ...
    describe("getDiagnosticsByLevel", () => {
        it("groups diagnostics by level", () => {
            expect(getDiagnosticsByLevel(diagnosticsByHardwareId)).toStrictEqual(new Map([
