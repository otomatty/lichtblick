...
import { CircleMarker, FeatureGroup, geoJSON, LayerGroup, Map as LeafMap, TileLayer, } from "leaflet";
...
import FilteredPointLayer, { POINT_MARKER_RADIUS, } from "@lichtblick/suite-base/panels/Map/FilteredPointLayer";
import ThemeProvider from "@lichtblick/suite-base/theme/ThemeProvider";
import { darkColor, lightColor, lineColors } from "@lichtblick/suite-base/util/plotColors";
import { buildSettingsTree, validateCustomUrl } from "./config";
import { hasFix, isGeoJSONMessage, isSupportedSchema, isValidMapMessage, parseGeoJSON, } from "./support";
const memoizedFilterMessages = memoizeWeak((msgs) => msgs.filter(isValidMapMessage));
function MapPanel(props) {
    const { context } = props;
    const [colorScheme, setColorScheme] = useState("light");
    const mapContainerRef = useRef(ReactNull);
    const [config, setConfig] = useState(() => {
        const initialConfig = props.context.initialState;
        return {
    ...
    const [tileLayer] = useState(new TileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors',
        maxNativeZoom: 18,
        maxZoom: 24,
    }));
    const [satelliteLayer] = useState(new TileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", {
        attribution: "&copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community",
        maxNativeZoom: 18,
        maxZoom: 24,
    }));
    const [customLayer] = useState(new TileLayer("https://example.com/{z}/{y}/{x}", {
        attribution: "",
        maxNativeZoom: 18,
        maxZoom: 24,
    }));
    ...
    const [allMapMessages, setAllMapMessages] = useState([]);
    const [currentMapMessages, setCurrentMapMessages] = useState([]);
    const [allGeoMessages, allNavMessages] = useMemo(() => _.partition(allMapMessages, isGeoJSONMessage), [allMapMessages]);
    const [currentGeoMessages, currentNavMessages] = useMemo(() => _.partition(currentMapMessages, isGeoJSONMessage), [currentMapMessages]);
    ...
    const { width: panelWidth, height: panelHeight, ref: sizeRef, } = useResizeDetector({
        refreshRate: 0,
    ...
    useLayoutEffect(() => {
        ...
        map.setView(config.center ? [config.center.lat, config.center.lon] : [0, 0], config.zoomLevel ?? 10);
        setCurrentMap(map);
    ...
    const onHover = useCallback((messageEvent) => {
        context.setPreviewTime(messageEvent == undefined ? undefined : toSec(messageEvent.receiveTime));
    }, [context]);
    const onClick = useCallback((messageEvent) => {
    ...
    const addGeoFeatureEventHandlers = useCallback((feature, message, layer) => {
    ...
    const addGeoJsonMessage = useCallback((message, group) => {
    ...
    useEffect(() => {
        ...
        // preview time.
        const prevNavMessages = allNavMessages.filter((message) => toSec(message.receiveTime) <= previewTime);
    ...
    const moveHandler = useDebouncedCallback((map) => {
    ...
    return (_jsx(ThemeProvider, { isDark: colorScheme === "dark", children: _jsxs(Stack, { ref: sizeRef, fullHeight: true, fullWidth: true, position: "relative", children: [!center && _jsx(EmptyState, { children: "Waiting for first GPS point..." }), _jsx(Stack, { position: "absolute", ref: mapContainerRef, style: {
                        inset: 0,
                        cursor: "auto",
