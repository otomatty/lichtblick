...
import { SeriesActionId, } from "../types";
...
export function setSeriesAction({ label, icon, id }) {
    return {
        display: "inline",
        icon,
        id,
...
export const makeSeriesNode = memoizeWeak((index, { path, canDelete, isArray }, t) => {
...
export const makeRootSeriesNode = memoizeWeak((paths, t) => {
    const children = Object.fromEntries(paths.length === 0
        ? [
            [
                "0",
                makeSeriesNode(0, {
...
export function buildSettingsTree({ isSynced, xAxisMaxValue, xAxisMinValue, xAxisRange, showPoints }, paths, t) {
    const maxXError = _.isNumber(xAxisMinValue) && _.isNumber(xAxisMaxValue) && xAxisMinValue >= xAxisMaxValue
        ? t("maxXError")
        : undefined;
    function setAxis({ value, label, error = undefined }) {
        return {
            label,
            input: "number",
            value,
...
export function usePanelSettings(config, saveConfig, paths, focusedPath) {
    const updatePanelSettingsTree = usePanelSettingsTreeUpdate();
    const { t } = useTranslation("stateTransitions");
    const actionHandler = useCallback(({ action, payload }) => {
        if (action === "update") {
