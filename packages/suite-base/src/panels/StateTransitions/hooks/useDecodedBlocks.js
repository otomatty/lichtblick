...
export function useDecodedBlocks(paths) {
    ...
    const subscriptions = useMemo(() => filterMap(paths, (path) => {
    ...
    const decodedBlocks = useMemo(() => blocks.map(decodeMessagePathsForMessagesByTopic), [blocks, decodeMessagePathsForMessagesByTopic]);
