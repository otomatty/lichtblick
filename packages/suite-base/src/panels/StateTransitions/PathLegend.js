...
export const PathLegend = React.memo(function PathLegend(props) {
    ...
    const handleDeletePath = useCallback((event, index) => {
    ...
    const handleEditTopic = useCallback((index) => {
        setSelectedPanelIds([panelId]);
        openPanelSettings();
        setFocusedPath(["paths", String(index)]);
    }, [openPanelSettings, panelId, setFocusedPath, setSelectedPanelIds]);
    return (_jsx(Stack, { className: classes.chartOverlay, position: "absolute", paddingTop: 0.5, children: (paths.length === 0 ? [DEFAULT_STATE_TRANSITION_PATH] : paths).map((path, index) => (_jsx("div", { "data-testid": `row-${index}`, className: classes.row, style: { height: heightPerTopic }, children: _jsxs(ButtonGroup, { size: "small", color: "inherit", variant: "contained", className: classes.buttonGroup, children: [_jsx(Button, { "data-testid": `edit-topic-button-${index}`, endIcon: paths.length === 0 && _jsx(Add16Regular, {}), onClick: () => {
                            handleEditTopic(index);
                        }, children: paths.length === 0
                            ? t("addSeriesButton")
                            : stateTransitionPathDisplayName(path, index) }), paths.length > 0 && (_jsx(Button, { "data-testid": `delete-topic-button-${index}`, className: classes.dismissIcon, size: "small", onClick: (event) => {
                            handleDeletePath(event, index);
                        }, children: _jsx(Dismiss12Regular, {}) }))] }) }, index))) }));
});
