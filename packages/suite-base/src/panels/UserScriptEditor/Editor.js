...
const gotoSelection = (editor, selection) => {
    if (selection) {
...
const Editor = ({ autoFormatOnSave, script, setScriptCode, save, setScriptOverride, rosLib, typesLib, }) => {
    const editorRef = React.useRef(ReactNull);
    ...
    React.useEffect(() => {
        const disposable = monacoApi.languages.typescript.typescriptDefaults.addExtraLib(rosLib, `file:///node_modules/@types/${projectConfig.rosLib.fileName}`);
        return () => {
            disposable.dispose();
        };
    }, [rosLib]);
    React.useEffect(() => {
        ...
        const model = monacoApi.editor.getModel(filePath) ??
    ...
    */
    useEffect(() => {
        const disposable = codeEditorService.registerCodeEditorOpenHandler(async (
    ...
    const willMount = React.useCallback((monaco) => {
        ...
        projectConfig.declarations.forEach((lib) => {
            if (lib.fileName.startsWith("@foxglove/schemas")) {
                monaco.languages.typescript.typescriptDefaults.addExtraLib(lib.sourceCode, `file:///node_modules/${lib.fileName}`);
            }
            else {
                monaco.languages.typescript.typescriptDefaults.addExtraLib(lib.sourceCode, `file:///node_modules/@types/${lib.fileName}`);
            }
        });
        ...
        projectConfig.utilityFiles.forEach((sourceFile) => {
            ...
            const model = monaco.editor.getModel(filePath) ??
    ...
    const onChange = useCallback((srcCode) => {
    ...
    return (_jsx("div", { ref: sizeRef, style: { width: "100%", height: "100%" }, children: _jsx(ErrorBoundary, { children: _jsx(Suspense, { fallback: _jsx("p", { children: "Loading user script editor" }), children: _jsx(MonacoEditor, { language: "typescript", theme: editorTheme, editorWillMount: willMount, editorDidMount: didMount, options: options, onChange: onChange }) }) }) }));
};
