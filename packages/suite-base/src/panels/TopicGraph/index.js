...
import { ArrowBidirectionalUpDown20Regular, PageFit20Regular, Diamond20Filled, RectangleLandscape20Regular, } from "@fluentui/react-icons";
import { FormControlLabel, IconButton, Paper, Radio, RadioGroup, iconButtonClasses, } from "@mui/material";
...
const topicVisibilityToLabelMap = {
    all: "All topics",
    none: "No topics",
...
function unionInto(dest, ...iterables) {
...
function TopicGraph() {
    ...
    const publishedTopics = useMessagePipeline(useCallback((ctx) => ctx.playerState.activeData?.publishedTopics, []));
    const subscribedTopics = useMessagePipeline(useCallback((ctx) => ctx.playerState.activeData?.subscribedTopics, []));
    const services = useMessagePipeline(useCallback((ctx) => ctx.playerState.activeData?.services, []));
    const [lrOrientation, setLROrientation] = useState(false);
    const [showServices, setShowServices] = useState(true);
    const [topicVisibility, setTopicVisibility] = useState("all");
    const textMeasure = useMemo(() => textMetrics.init({
        fontFamily: "Arial",
        fontSize: "16px",
    }), []);
    const topicPassesConditions = useCallback(({ topicIdWithSubscriptions, topic, }) => {
    ...
    const elements = useMemo(() => {
        ...
        const topicIdWithSubscriptions = new Set();
    ...
    return (_jsxs(_Fragment, { children: [_jsx(PanelToolbar, {}), _jsxs("div", { className: classes.root, children: [_jsxs(Paper, { square: false, elevation: 4, className: classes.paper, children: [_jsx(IconButton, { size: "small", title: "Zoom fit", onClick: onZoomFit, children: _jsx(PageFit20Regular, {}) }), _jsx(IconButton, { size: "small", title: "Orientation", onClick: toggleOrientation, children: _jsx(ArrowBidirectionalUpDown20Regular, { style: { transform: `rotate(${lrOrientation ? 90 : 0}deg)` } }) }), _jsx(IconButton, { size: "small", color: showServices ? "info" : "inherit", title: showServices ? "Showing services" : "Hiding services", onClick: toggleShowServices, children: _jsx(RectangleLandscape20Regular, {}) })] }), _jsx(ExpandingToolbar, { checked: topicVisibility !== "none", dataTest: "set-topic-visibility", tooltip: topicVisibilityTooltip, icon: _jsx(Diamond20Filled, {}), selectedTab: selectedTab, onSelectTab: (newSelectedTab) => {
                            setSelectedTab(newSelectedTab);
                        }, children: _jsx(ToolGroup, { name: "Topics", children: _jsx("div", { className: classes.toolbarContent, children: _jsx(RadioGroup, { defaultValue: topicVisibility, onChange: (_event, value) => {
                                        graph.current?.resetUserPanZoom();
                                        setTopicVisibility(value);
                                    }, children: Object.entries(topicVisibilityToLabelMap).map(([id, label]) => (_jsx(FormControlLabel, { value: id, control: _jsx(Radio, {}), label: label }, id))) }) }) }) })] }), _jsx(Graph, { style: STYLESHEET, elements: elements, rankDir: lrOrientation ? "LR" : "TB", graphRef: graph })] }));
}
