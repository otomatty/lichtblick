...
export default class VirtualLRUBuffer {
    byteLength; // How many bytes does this buffer represent.
    #blocks = []; // Actual `Buffer` for each block.
    ...
    #blockSize = Math.trunc(kMaxLength / 2);
    #numberOfBlocks = Infinity; // How many blocks are we allowed to have at any time.
    #lastAccessedBlockIndices = []; // Indexes of blocks, from least to most recently accessed.
    #rangesWithData = []; // Ranges for which we have data copied in (and have not been evicted).
    constructor(options) {
    ...
    copyFrom(source, targetStart) {
        ...
        while (position < range.end) {
            const { blockIndex, positionInBlock, remainingBytesInBlock } = this.#calculatePosition(position);
    ...
    #getBlock(index) {
        ...
        if (this.#lastAccessedBlockIndices.length > this.#numberOfBlocks) {
            ...
            if (deleteIndex != undefined) {
                ...
                this.#rangesWithData = simplify(substract(this.#rangesWithData, [
