...
export const expandedLineColors = _.uniq(_.range(0, colorExpansion[0].length)
    .map((i) => colorExpansion.map((colors) => colors[i]))
    .flat());
export const lightColor = _.memoize((color) => tinycolor(color).brighten(15).toString());
export const darkColor = _.memoize((color) => tinycolor(color).darken(30).toString());
export function getLineColor(color, index) {
    return color ?? lineColors[index % lineColors.length];
}
export function getContrastColor(colorScheme, color) {
