...
import { toDate, fromDate } from "@lichtblick/rostime";
...
export function format(stamp, timezone) {
    return `${formatDate(stamp, timezone)} ${formatTime(stamp, timezone)}`;
}
export function formatDate(stamp, timezone) {
...
export function formatTime(stamp, timezone) {
    if (stamp.sec < 0 || stamp.nsec < 0) {
        console.error("Times are not allowed to be negative");
        return "(invalid negative time)";
    }
...
export function formatDuration(stamp) {
    return moment
        .duration(Math.round(stamp.sec * 1000 + stamp.nsec / 1e6))
        .format("h:mm:ss.SSS", { trim: false });
}
export function parseTimeStr(str, timezone) {
    const newMomentTimeObj = timezone != undefined
...
export const getValidatedTimeAndMethodFromString = ({ text, timezone, }) => {
