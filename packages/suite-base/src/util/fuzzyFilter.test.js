...
describe("fuzzyFilter", () => {
    ...
    it("allows disabling sorting", () => {
        expect(fuzzyFilter({ options: ["abbc", "abc"], filter: "abc", getText: (x) => x, sort: false })).toEqual(["abbc", "abc"]);
        expect(fuzzyFilter({ options: ["abb", "ab"], filter: "ab", getText: (x) => x, sort: false })).toEqual(["abb", "ab"]);
    });
    ...
    it("supports custom objects", () => {
        expect(fuzzyFilter({ options: [{ x: "abc" }, { x: "def" }], filter: "a", getText: ({ x }) => x })).toEqual([{ x: "abc" }]);
    });
});
