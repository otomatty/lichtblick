...
import { updateAppURLState, parseAppURLState, } from "@lichtblick/suite-base/util/appURLState";
import isDesktopApp from "@lichtblick/suite-base/util/isDesktopApp";
jest.mock("@lichtblick/suite-base/util/isDesktopApp", () => ({
    __esModule: true,
    default: jest.fn(),
}));
const mockIsDesktop = isDesktopApp;
describe("app state url parser", () => {
    ...
    describe.each([
        ...
        it("parses data platform state urls", () => {
            const now = { sec: new Date().getTime(), nsec: 0 };
...
describe("updateAppURLState", () => {
    ...
    it("encodes rosbag urls", () => {
        ...
        expect(decodeURIComponent(result.href)).toEqual(`${baseURL.origin}/?ds=${urlState.ds}&ds.url=${url}`);
    });
    it("should encode multiple remote files urls", () => {
        ...
        const result = updateAppURLState(baseURL, urlState);
        expect(decodeURIComponent(result.href)).toEqual(`${baseURL.origin}/?ds=${urlState.ds}&ds.url=${urls[0]}&ds.url=${urls[1]}`);
    });
    it("appends 'ds.' + key when keyMap entry doesn't have a substitute for that key", () => {
        const key = BasicBuilder.string();
        const paramArray = BasicBuilder.strings({ count: 2 });
        const urlState = {
            time: undefined,
            ds: "remote-file",
        ...
        const result = updateAppURLState(baseURL, urlState);
        expect(result.href).toEqual(`${baseURL.origin}/?ds=${urlState.ds}&ds.${key}=${paramArray[0]}&ds.${key}=${paramArray[1]}`);
    });
    describe("url states", () => {
        ...
        it.each([
            ...
            expect(result.href).toEqual(`${baseURL.origin}/?ds=${state.ds}&ds.eventId=${eventId}&ds.url=${encodedURLFile}`);
        });
    });
});
