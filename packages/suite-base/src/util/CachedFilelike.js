...
export default class CachedFilelike {
    #fileReader;
    ...
    #setConnection(range) {
        ...
        if (this.#currentConnection) {
            ...
            this.#log.debug(`Destroyed current connection @ ${rangeToString(currentConnection.remainingRange)}`);
        }
        ...
        stream.on("error", (error) => {
            ...
            if (this.#keepReconnectingCallback) {
                ...
                if (lastErrorTime != undefined && Date.now() - lastErrorTime < 100) {
                    this.#log.error(`Connection @ ${rangeToString(range)} threw another error; closing: ${error.toString()}`);
                    this.#closed = true;
            ...
            this.#log.info(`Connection @ ${rangeToString(range)} threw error; trying to continue: ${error.toString()}`);
        ...
        stream.on("data", (chunk) => {
            ...
            if (bytesRead - lastReportedBytesRead > LOGGING_INTERVAL_IN_BYTES) {
                ...
                this.#log.debug(`Connection @ ${rangeToString(currentConnection.remainingRange)} downloading at ${speed} MiB/s`);
            }
            if (this.#virtualBuffer.hasData(range.start, range.end)) {
