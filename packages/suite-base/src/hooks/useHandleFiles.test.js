...
import { FILE_ACCEPT_TYPE, FILE_JSON_TYPE, } from "@lichtblick/suite-base/context/Workspace/constants";
...
jest.mock("@lichtblick/suite-base/hooks/useInstallingExtensionsStore", () => ({
    useInstallingExtensionsStore: (selector) => selector({
...
describe("useHandleFiles", () => {
    ...
    function fileBuilder(extension, type) {
        return new File([BasicBuilder.string()], `${BasicBuilder.string()}.${extension}`, {
            type,
        });
    }
    function setup({ filesOverride, isLayout } = { isLayout: false }) {
        let files = [];
        if (isLayout) {
            ...
            files.forEach((file) => {
                file.arrayBuffer = async () => await Promise.resolve(new Uint8Array(new TextEncoder().encode(BasicBuilder.string()).buffer));
            });
        }
    ...
    beforeEach(() => {
        useLayoutManager.mockReturnValue(mockLayoutManager);
        useInstallingExtensionsState.mockReturnValue({
            installFoxeExtensions: installFoxeExtensionsMock,
        });
        useLayoutTransfer.mockReturnValue({
    ...
    it("logs error if reading a file fails", async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(`Error reading file ${brokenFile.name}`, expect.objectContaining({ message: "Read failed" }));
        logSpy.mockRestore();
    });
