...
export function useLayoutNavigation(menuClose) {
    ...
    const [layouts] = useAsyncFn(async () => {
    ...
    const promptForUnsavedChanges = useCallback(async () => {
        const currentLayout = currentLayoutId != undefined ? await layoutManager.getLayout(currentLayoutId) : undefined;
    ...
    const onSelectLayout = useCallbackWithToast(async (item, { selectedViaClick = false, event } = {}) => {
        if (selectedViaClick) {
