...
describe("useSeekTimeFromCLI", () => {
    ...
    const mockUseAppParameters = useAppParameters;
    const mockParseTimestampStr = parseTimestampStr;
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    it("should log an error if time parsing fails", () => {
        ...
        expect(mockSeekPlayback).not.toHaveBeenCalled();
        expect(enqueueSnackbar).toHaveBeenCalledWith("Invalid time format using 'time' parameter on CLI. Please check and try again.", {
            variant: "warning",
        });
    });
});
