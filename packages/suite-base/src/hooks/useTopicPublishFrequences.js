...
import { subtract as subtractTimes, areEqual, fromMillis, toSec } from "@lichtblick/rostime";
import { useMessagePipeline, } from "@lichtblick/suite-base/components/MessagePipeline";
import { PLAYER_CAPABILITIES } from "@lichtblick/suite-base/players/constants";
const EMPTY_TOPIC_STATS = new Map();
// Empirically this seems to strike a good balance between stable values
// and reasonably quick reactions to change.
function smoothValues(oldValue, newValue) {
    return 0.7 * (oldValue ?? newValue) + 0.3 * newValue;
}
function calculateStaticItemFrequency(numMessages, firstMessageTime, lastMessageTime, duration) {
...
function calculateLiveItemFrequency(numMessages, duration) {
    const durationSec = toSec(duration);
    return durationSec > 0 ? numMessages / durationSec : undefined;
}
const selectCurrentTime = ({ playerState }) => playerState.activeData?.currentTime;
const selectStartTime = ({ playerState }) => playerState.activeData?.startTime;
const selectEndTime = ({ playerState }) => playerState.activeData?.endTime;
const selectTopicStats = (ctx) => ctx.playerState.activeData?.topicStats ?? EMPTY_TOPIC_STATS;
const selectPlayerCapabilities = (ctx) => ctx.playerState.capabilities;
const EMPTY_FREQUENCIES = {};
/**
 ...
 */
export function useTopicPublishFrequencies() {
    ...
    const duration = useMemo(() => (endTime && startTime ? subtractTimes(endTime, startTime) : { sec: 0, nsec: 0 }), [endTime, startTime]);
    const samplesByTopic = useRef({});
    const playerIsStaticSource = useMemo(() => playerCapabilities.includes(PLAYER_CAPABILITIES.playbackControl), [playerCapabilities]);
    const frequencies = useMemo(() => {
        ...
        for (const [topic, stat] of topicStats) {
            if (playerIsStaticSource) {
                ...
                const frequency = calculateStaticItemFrequency(stat.numMessages, stat.firstMessageTime, stat.lastMessageTime, duration);
