...
jest.mock("@lichtblick/suite-base/hooks/useInstallingExtensionsStore", () => ({
    useInstallingExtensionsStore: (selector) => selector(mockStore),
}));
jest.mock("@lichtblick/suite-base/context/ExtensionCatalogContext", () => ({
    useExtensionCatalog: (selector) => selector({
...
describe("useInstallingExtensionsState", () => {
    ...
    it("installs extensions and updates progress", async () => {
        ...
        const { result } = renderHook(() => useInstallingExtensionsState({
            isPlaying: true,
        ...
        expect(enqueueSnackbar).toHaveBeenCalledWith(expect.stringContaining(`Successfully installed all ${extensionsData.length} extensions.`), expect.objectContaining({
            variant: "success",
    ...
    it("calls installExtensions and shows error snackbar on failure", async () => {
        ...
        expect(enqueueSnackbar).toHaveBeenCalledWith(expect.stringContaining(`An error occurred during extension installation: ${errorValue.message}`), expect.objectContaining({ variant: "error" }));
    });
    it("shows progress snackbar when installation starts", () => {
        const extensionsToBeInstalled = BasicBuilder.number();
        const { rerender } = renderHook(() => useInstallingExtensionsState({
            isPlaying: false,
            playerEvents: { play: playMock },
        }));
        ...
        rerender();
        expect(enqueueSnackbar).toHaveBeenCalledWith(`Installing ${extensionsToBeInstalled} extensions...`, expect.objectContaining({
            key: expect.anything(),
            variant: "info",
            persist: true,
            preventDuplicate: true,
