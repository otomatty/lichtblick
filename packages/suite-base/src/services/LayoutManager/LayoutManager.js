...
import { layoutAppearsDeleted, layoutIsShared, layoutPermissionIsShared, } from "@lichtblick/suite-base/services/ILayoutStorage";
 ...
 */
async function updateOrFetchLayout(remote, params) {
...
export default class LayoutManager {
    static LOCAL_STORAGE_NAMESPACE = "local";
    static REMOTE_STORAGE_NAMESPACE_PREFIX = "remote-";
    /**
     ...
     */
    static #withBusyStatus(_prototype, _propertyKey, descriptor) {
    ...
    constructor({ local, remote, }) {
        this.#local = new MutexLocked(new NamespacedLayoutStorage(new WriteThroughLayoutCache(local), remote
    ...
    async saveNewLayout({ name, data: unmigratedData, permission, from, }) {
        ...
        if (layoutPermissionIsShared(permission)) {
            ...
            const result = await this.#local.runExclusive(async (local) => await local.put({
                id: newLayout.id,
                name: newLayout.name,
        ...
        const newLayout = await this.#local.runExclusive(async (local) => await local.put({
            id: uuidv4(),
            name,
            from,
            permission,
            baseline: { data, savedAt: new Date().toISOString() },
            working: undefined,
    ...
    async updateLayout({ id, name, data, }) {
        ...
        const newWorking = data == undefined
            ? localLayout.working
        ...
        if (name != undefined && layoutIsShared(localLayout)) {
            ...
            const isRename = this.#remote != undefined &&
                name != undefined &&
    ...
    async overwriteLayout({ id }) {
        ...
        if (layoutIsShared(localLayout)) {
            ...
            const result = await this.#local.runExclusive(async (local) => await local.put({
                ...
                working: undefined,
                syncInfo: this.#remote && localLayout.syncInfo?.status !== "new"
                    ? { status: "updated", lastRemoteSavedAt: localLayout.syncInfo?.lastRemoteSavedAt }
                    : localLayout.syncInfo,
            }));
            this.#notifyChangeListeners({ type: "change", updatedLayout: result });
    ...
    async makePersonalCopy({ id, name }) {
        const now = new Date().toISOString();
        const result = await this.#local.runExclusive(async (local) => {
            const layout = await local.get(id);
            if (!layout) {
     ...
     * storage, or both.
     */
    async syncWithRemote(abortSignal) {
        if (this.#currentSync) {
    ...
    async #syncWithRemoteImpl(abortSignal) {
        ...
        const [localOps, remoteOps] = _.partition(syncOperations, (op) => op.local);
        await Promise.all([
            this.#performLocalSyncOperations(localOps, abortSignal),
            this.#performRemoteSyncOperations(remoteOps, abortSignal),
        ]);
    }
    async #performLocalSyncOperations(operations, abortSignal) {
        await this.#local.runExclusive(async (local) => {
            for (const operation of operations) {
                ...
                switch (operation.type) {
                    ...
                    case "delete-local":
                        log.debug(`Deleting local layout ${operation.localLayout.id}, whose sync status was ${operation.localLayout.syncInfo?.status}`);
    ...
    async #performRemoteSyncOperations(operations, abortSignal) {
        const remote = this.#remote;
        if (!remote) {
            return;
        }
        const cleanups = await Promise.all(operations.map(async (operation) => {
            switch (operation.type) {
                case "delete-remote": {
                    const { localLayout } = operation;
                ...
                case "upload-new": {
                    ...
                    const newBaseline = await remote.saveNewLayout({
                        ...
                        savedAt: localLayout.baseline.savedAt ?? new Date().toISOString(),
                    });
                    return async (local) => {
    ...
    static {
        ...
        ], LayoutManager.prototype, "saveNewLayout", null);
        __decorate([
            LayoutManager.#withBusyStatus
        ], LayoutManager.prototype, "updateLayout", null);
        __decorate([
            LayoutManager.#withBusyStatus
        ], LayoutManager.prototype, "deleteLayout", null);
        __decorate([
            LayoutManager.#withBusyStatus
        ], LayoutManager.prototype, "overwriteLayout", null);
        __decorate([
            LayoutManager.#withBusyStatus
        ], LayoutManager.prototype, "revertLayout", null);
        __decorate([
            LayoutManager.#withBusyStatus
        ], LayoutManager.prototype, "makePersonalCopy", null);
        __decorate([
            LayoutManager.#withBusyStatus
        ], LayoutManager.prototype, "syncWithRemote", null);
    }
}
