...
export default function computeLayoutSyncOperations(localLayouts, remoteLayouts) {
    const ops = [];
    ...
    for (const localLayout of localLayouts) {
        ...
        if (remoteLayout) {
            ...
            switch (localLayout.syncInfo?.status) {
                case undefined:
                case "new":
                    log.warn(`Remote layout is present but local has sync status: ${localLayout.syncInfo?.status}`);
                ...
                case "remotely-deleted":
                    log.warn(`Remote layout is present but cache is marked as remotely deleted: ${localLayout.id}`);
                    break;
            }
        }
        else {
