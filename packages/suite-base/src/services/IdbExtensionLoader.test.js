...
import { EXTENSION_STORE_NAME, METADATA_STORE_NAME, } from "@lichtblick/suite-base/services/IdbExtensionStorage";
...
const expectedChangelog = "# studio-extension-turtlesim version history\n\n## 0.0.0\n\n- Alpha testing\n";
...
describe("IdbExtensionLoader", () => {
    ...
    describe("loadExtension", () => {
        ...
        it("should throw an error if the extension is not found", async () => {
            ...
            await expect(loader.loadExtension(BasicBuilder.string())).rejects.toThrow("Extension not found");
        });
        ...
        it("should throw an error if the main extension script is missing", async () => {
            ...
            mockGet.mockReturnValueOnce(extension);
            await expect(loader.loadExtension(extension.info.id)).rejects.toThrow(`Extension is corrupted: missing ${ALLOWED_FILES.EXTENSION}`);
        });
    });
    ...
    describe("validatePackageInfo", () => {
        ...
        ])("should throw an error if the name is missing", (extensionInfo) => {
        ...
        it("should throw an error if the publisher is missing", () => {
            ...
            const input = { name: nonStandardName };
