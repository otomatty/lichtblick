 ...
 */
export function useMessagePathDrag({ item, selected }) {
    ...
    const overDropTargets = useRef(new Set());
    const [{ isDragging, draggedItemCount: actualDraggedItemCount }, connectDragSource, connectDragPreview,] = useDrag({
    ...
    const connectDragPreviewWithCaptureDraggingState = useCallback((el) => connectDragPreview(el, { captureDraggingState: true }), [connectDragPreview]);
    return {
        connectDragSource,
...
export function useMessagePathDrop() {
    ...
    const [{ isDragging, isOver, isValidTarget, message }, connectDropTarget] = useDrop({
        ...
        collect(monitor) {
            ...
            const dragObject = monitor.getItem();
