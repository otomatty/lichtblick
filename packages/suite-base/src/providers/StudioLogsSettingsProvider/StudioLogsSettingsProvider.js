 ...
 * ```
 */
function StudioLogsSettingsProvider(props) {
    // LocalStorageからのログ設定状態の管理
    // "blick.logs-settings"キーで永続化される
    const [studioLogsSettingsSavedState, setStudioLogsSettingsSavedState] = useLocalStorage("blick.logs-settings", {});
    // Zustandストアの状態管理
    // 初期化時にLocalStorageの状態を使用
    const [studioLogsSettingsStore, setStudioLogsSettingsStore] = useState(() => createStudioLogsSettingsStore(studioLogsSettingsSavedState));
    // LocalStorage状態の参照を最適化
    // useEffectの依存配列での不要な再実行を防ぐためuseRefを使用
    const savedStateRef = useRef(studioLogsSettingsSavedState);
    useEffect(() => {
    ...
    return (_jsx(StudioLogsSettingsContext.Provider, { value: studioLogsSettingsStore, children: props.children }));
}
