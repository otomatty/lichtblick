 ...
 * ```
 */
function createExtensionRegistryStore(loaders, mockMessageConverters) {
    return createStore((set, get) => {
         ...
         * @param extensionId - 拡張機能ID
         */
        const markExtensionAsInstalled = (extensionId) => {
         ...
         * @param extensionId - 拡張機能ID
         */
        const unMarkExtensionAsInstalled = (extensionId) => {
            const updatedExtensions = new Set(get().loadedExtensions);
         ...
         * @returns インストール結果の配列
         */
        const installExtensions = async (namespace, data) => {
         ...
         * @returns インストール結果の配列
         */
        async function promisesInBatch(batch, loader) {
            return await Promise.all(batch.map(async (extensionData) => {
                try {
         ...
         */
        const mergeState = (info, { messageConverters, panelSettings, panels, topicAliasFunctions, cameraModels, }) => {
            set((state) => ({
         ...
         */
        async function loadInBatch({ batch, loader, installedExtensions, contributionPoints, }) {
            await Promise.all(batch.map(async (extension) => {
                try {
                    installedExtensions.push(extension);
                    const { messageConverters, panelSettings, panels, topicAliasFunctions, cameraModels } = contributionPoints;
        ...
        const refreshAllExtensions = async () => {
             ...
             */
            const processLoader = async (loader) => {
                try {
            ...
            log.info(`Loaded ${installedExtensions.length} extensions in ${(performance.now() - start).toFixed(1)}ms`);
            // ストアを更新
            set({
                installedExtensions,
         ...
         * @returns 更新されたストア状態
         */
        function removeExtensionData({ id, // deleted extension id
        state, }) {
            const { installedExtensions, installedPanels, installedMessageConverters, installedTopicAliasFunctions, installedCameraModels, } = state;
            return {
                installedExtensions: installedExtensions?.filter(({ id: extensionId }) => extensionId !== id),
                installedPanels: _.pickBy(installedPanels, ({ extensionId }) => extensionId !== id),
                installedMessageConverters: installedMessageConverters?.filter(({ extensionId }) => extensionId !== id),
                installedTopicAliasFunctions: installedTopicAliasFunctions?.filter(({ extensionId }) => extensionId !== id),
                installedCameraModels: new Map([...installedCameraModels].filter(([, { extensionId }]) => extensionId !== id)),
            };
        }
        /**
         * 拡張機能をアンインストール
         ...
         * @param id - 拡張機能ID
         */
        const uninstallExtension = async (namespace, id) => {
            const namespaceLoader = loaders.find((loader) => loader.namespace === namespace);
            if (namespaceLoader == undefined) {
                throw new Error("No extension loader found for namespace " + namespace);
            }
        ...
        return {
            ...
            panelSettings: _.merge({}, ...(mockMessageConverters ?? []).map(({ fromSchemaName, panelSettings }) => _.mapValues(panelSettings, (settings) => ({ [fromSchemaName]: settings })))),
        };
    });
}
/**
 ...
 * ```
 */
export default function ExtensionCatalogProvider({ children, loaders, mockMessageConverters, }) {
    ...
    useEffect(() => {
        refreshAllExtensions().catch((err) => {
            log.error(err);
        });
    }, [refreshAllExtensions]);
    return (_jsx(ExtensionCatalogContext.Provider, { value: store, children: children }));
}
