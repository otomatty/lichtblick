...
import { getAllPanelIds, getParentTabPanelByPanelId, isTabPanel, } from "@lichtblick/suite-base/util/layout";
/**
 * パネル選択状態の切り替え処理
 *
 ...
 */
export default function toggleSelectedPanel(panelId, containingTabId, configById, selectedPanelIds) {
    const panelIdsToDeselect = [];
    // タブパネルが選択された場合、その子パネルを選択解除
    const savedConfig = configById[panelId];
    if (isTabPanel(panelId) && savedConfig) {
        const { activeTabIdx, tabs } = savedConfig;
