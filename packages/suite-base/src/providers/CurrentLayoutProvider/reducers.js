...
import { createDragToUpdates, createHideUpdate, createRemoveUpdate, getLeaves, getNodeAtPath, updateTree, } from "react-mosaic-component";
import { filterMap } from "@lichtblick/den/collection";
import { TAB_PANEL_TYPE } from "@lichtblick/suite-base/util/globalConstants";
import { DEFAULT_TAB_PANEL_CONFIG, addPanelToTab, createAddUpdates, getAllPanelIds, getConfigsForNestedPanelsInsideTab, getPanelIdForType, getPanelIdsInsideTabPanels, getPanelTypeFromId, getPathFromNode, getSaveConfigsPayloadForAddedPanel, inlineTabPanelLayouts, moveTabBetweenTabPanels, removePanelFromTabPanel, reorderTabWithinTabPanel, replaceAndRemovePanels, updateTabPanelLayout, } from "@lichtblick/suite-base/util/layout";
 ...
 */
export const defaultPlaybackConfig = {
    speed: 1.0,
};
/**
 * パネルレイアウト変更処理
 ...
 */
function changePanelLayout(state, { layout, trimConfigById = true }) {
    // 新しいレイアウトから全パネルIDを取得（空文字列は除外）
    const panelIds = getLeaves(layout ?? ReactNull).filter((panelId) => !_.isEmpty(panelId));
 ...
 * @returns 更新されたレイアウト状態
 */
function savePanelConfigs(state, payload) {
    const { configs } = payload;
    const prevConfigById = state.configById;
    // 新しい設定を既存設定にマージ
    // 注意: 設定が変更されていない場合、同じオブジェクト参照を維持
    const newConfigById = configs.reduce((currentSavedProps, { id, config, defaultConfig = {}, override = false }) => {
        if (override) {
            // 完全上書きモード
 ...
 * @returns 更新されたレイアウト状態
 */
function saveFullPanelConfig(state, payload) {
 ...
 * @returns 更新されたレイアウト状態
 */
const closePanel = (panelsState, { tabId, root, path }) => {
    if (tabId != undefined) {
        // タブパネル内のパネルを削除
        const config = panelsState.configById[tabId];
 ...
 * @returns 更新されたレイアウト状態
 */
const splitPanel = (panelsState, { id, tabId, direction, config, root, path }) => {
    ...
    // タブパネル内での分割処理
    if (tabId != undefined) {
        const prevConfig = configById[tabId];
    ...
    newPanelsState = savePanelConfigs(newPanelsState, getSaveConfigsPayloadForAddedPanel({ id: newId, config, savedProps: configById }));
 ...
 * @returns 更新されたレイアウト状態
 */
const swapPanel = (panelsState, { tabId, originalId, type, config, root, path }) => {
    const newId = getPanelIdForType(type);
    let newPanelsState = { ...panelsState };
    // タブパネル内でのパネル交換
    ...
    // 新パネルの設定を保存
    newPanelsState = savePanelConfigs(newPanelsState, getSaveConfigsPayloadForAddedPanel({
        id: newId,
        config,
 ...
 * @returns 更新されたレイアウト状態
 */
const createTabPanelWithSingleTab = (panelsState, { idToReplace, layout, idsToRemove }) => {
    ...
    const panelIdsNotInNewTab = getAllPanelIds(layout, configById).filter((leaf) => !idsToRemove.includes(leaf));
    // タブ内レイアウトから不要なパネルを除去
    ...
    // ネストパネルの設定を取得
    const nestedPanelConfigs = getConfigsForNestedPanelsInsideTab(idToReplace, newId, idsToRemove, configById);
    // 設定を一括保存
    newPanelsState = savePanelConfigs(newPanelsState, {
 ...
 * @returns 更新されたレイアウト状態
 */
const createTabPanelWithMultipleTabs = (panelsState, { idToReplace, layout, idsToRemove }) => {
    const { configById } = panelsState;
    const newId = getPanelIdForType(TAB_PANEL_TYPE);
    // メインレイアウトを更新
    const newLayout = replaceAndRemovePanels({ originalId: idToReplace, newId, idsToRemove }, layout);
 ...
 * @returns 更新されたレイアウト状態
 */
const moveTab = (panelsState, { source, target }) => {
    const saveConfigsPayload = source.panelId === target.panelId
        ? // 同一タブパネル内での順序変更
 ...
 * @returns 更新されたレイアウト状態
 */
const addPanel = (panelsState, { tabId, id, config }) => {
    let newPanelsState = { ...panelsState };
    ...
    // 新しいレイアウトを構築
    const fixedLayout = _.isEmpty(layout)
 ...
 * @returns 更新されたレイアウト状態
 */
const dropPanel = (panelsState, { newPanelType, destinationPath = [], position, tabId, config }) => {
    const id = getPanelIdForType(newPanelType);
    const configs = [];
    // タブパネル内へのドロップ処理
    if (tabId != undefined) {
        const { configs: newConfigs } = addPanelToTab(id, destinationPath, position, panelsState.configById[tabId], tabId);
        configs.push(...newConfigs);
    }
    // レイアウトを更新
    const newLayout = tabId != undefined
        ? panelsState.layout // タブ内ドロップの場合、メインレイアウトは変更なし
        : updateTree(panelsState.layout, createAddUpdates(panelsState.layout, id, destinationPath, position ?? "left"));
    // 新パネルの設定を準備
    if (config) {
...
const dragWithinSameTab = (panelsState, { originalLayout, sourceTabId, position, destinationPath, ownPath, sourceTabConfig, sourceTabChildConfigs, }) => {
    ...
    if (typeof currentTabLayout === "string") {
        ...
        newPanelsState = savePanelConfigs(newPanelsState, {
            configs: [
                {
                    id: sourceTabId,
                    config: updateTabPanelLayout(newTree, panelsState.configById[sourceTabId]),
                },
                ...sourceTabChildConfigs,
            ],
        });
...
const dragToMainFromTab = (panelsState, { originalLayout, sourceTabId, position, destinationPath, ownPath, sourceTabConfig, sourceTabChildConfigs, }) => {
    const currentTabLayout = sourceTabConfig.tabs[sourceTabConfig.activeTabIdx]?.layout;
    if (currentTabLayout == undefined) {
        return panelsState;
    }
    // Remove panel from tab layout
    const saveConfigsPayload = removePanelFromTabPanel(ownPath, panelsState.configById[sourceTabId], sourceTabId);
    const panelConfigs = {
        ...saveConfigsPayload,
    ...
    const newLayout = updateTree(originalLayout, createAddUpdates(originalLayout, currentNode, destinationPath, position));
    let newPanelsState = changePanelLayout(panelsState, { layout: newLayout, trimConfigById: false });
    newPanelsState = savePanelConfigs(newPanelsState, panelConfigs);
    return newPanelsState;
};
const dragToTabFromMain = (panelsState, { originalLayout, panelId, targetTabId, position, destinationPath, ownPath, targetTabConfig, sourceTabChildConfigs, }) => {
...
const dragToTabFromTab = (panelsState, { originalLayout, panelId, sourceTabId, targetTabId, position, destinationPath, ownPath, targetTabConfig, sourceTabConfig, sourceTabChildConfigs, }) => {
    // Remove panel from tab layout
 ...
 * @returns 更新されたレイアウト状態
 */
const startDrag = (panelsState, { path, sourceTabId }) => {
    // パネルを一時的に非表示にしてドラッグ中の視覚的フィードバックを提供
    if (path.length > 0) {
        if (sourceTabId != undefined) {
            // タブパネル内のパネルドラッグ
            const tabConfig = panelsState.configById[sourceTabId];
        ...
        const sourceTabConfig = panelsState.configById[sourceTabId];
        return savePanelConfigs(panelsState, {
 ...
 * @returns 更新されたレイアウト状態
 */
const endDrag = (panelsState, dragPayload) => {
    const { originalLayout, originalSavedProps, panelId, sourceTabId, targetTabId, position, destinationPath, ownPath, } = dragPayload;
    // ドラッグパターンの判定
    ...
    // 元の設定を取得
    const sourceTabConfig = sourceTabId != undefined ? originalSavedProps[sourceTabId] : undefined;
    const targetTabConfig = targetTabId != undefined ? originalSavedProps[targetTabId] : undefined;
    // ソースタブ内のネストパネル設定を取得
    const panelIdsInsideTabPanels = sourceTabId != undefined ? getPanelIdsInsideTabPanels([sourceTabId], originalSavedProps) : [];
 ...
 * ```
 */
export default function (panelsState, action) {
