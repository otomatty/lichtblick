...
describe("LayoutManagerProvider", () => {
    ...
    useNetworkState.mockReturnValue({ online: true });
    useVisibilityState.mockReturnValue("visible");
    useLayoutStorage.mockReturnValue({});
    useRemoteLayoutStorage.mockReturnValue({});
    beforeEach(() => {
        jest.clearAllMocks();
    });
    it("should call layoutManager.setOnline accordingly with useNetworkState", async () => {
        useNetworkState.mockResolvedValueOnce({ online: false });
        // 1 render with true and another with false.
        render(_jsx(LayoutManagerProvider, {}));
        render(_jsx(LayoutManagerProvider, {}));
        await waitFor(() => {
    ...
    it("should not call layoutManager.syncWithRemote if offline", async () => {
        useNetworkState.mockReturnValueOnce({ online: false });
        render(_jsx(LayoutManagerProvider, {}));
        await waitFor(() => {
    ...
    it("should not call layoutManager.syncWithRemote if not visible", async () => {
        useVisibilityState.mockReturnValueOnce("invisible");
        render(_jsx(LayoutManagerProvider, {}));
        await waitFor(() => {
            expect(mockLayoutManager.syncWithRemote).toHaveBeenCalledTimes(0);
        });
    ...
    it("should not call layoutManager.syncWithRemote if there is not remote storage", async () => {
        useRemoteLayoutStorage.mockReturnValueOnce(undefined);
        render(_jsx(LayoutManagerProvider, {}));
        await waitFor(() => {
            expect(mockLayoutManager.syncWithRemote).toHaveBeenCalledTimes(0);
        });
