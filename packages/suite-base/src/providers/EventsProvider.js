...
import { EventsContext, } from "@lichtblick/suite-base/context/EventsContext";
/** イベントが存在しない場合のデフォルト値 */
const NO_EVENTS = [];
/**
 * イベントストアを作成する関数
...
function createEventsStore() {
    return createStore((set) => ({
        ...
        setEventsSupported: (eventsSupported) => {
            set({ eventsSupported });
        },
        /**
         * デバイスIDを設定する
...
export default function EventsProvider({ children }) {
    ...
    return _jsx(EventsContext.Provider, { value: store, children: children });
}
