 ...
 * ```
 */
export function buildContributionPoints(extension, unwrappedExtensionSource) {
    ...
    const extensionMode = process.env.NODE_ENV === "production"
        ? "production"
    ...
    const ctx = {
         ...
         */
        registerMessageConverter: (messageConverter) => {
            log.debug(`Extension ${extension.qualifiedName} registering message converter from: ${messageConverter.fromSchemaName} to: ${messageConverter.toSchemaName}`);
    ...
    try {
        ...
        fn(module, require, {});
        const wrappedExtensionModule = module.exports;
