...
describe("ExtensionCatalogProvider", () => {
    function setup({ loadersOverride } = {}) {
        ...
        return {
            ...renderHook(() => useExtensionCatalog((state) => state), {
                initialProps: {},
                wrapper: ({ children }) => (_jsx(ExtensionCatalogProvider, { loaders: loaders, children: children })),
            }),
            extensionInfo,
            loaders,
            loadExtension,
    ...
    `;
        ...
        const { result } = renderHook(() => useExtensionCatalog((state) => state), {
            initialProps: {},
            wrapper: ({ children }) => (_jsx(ExtensionCatalogProvider, { loaders: [loader], children: children })),
        });
        await waitFor(() => {
            expect(loadExtension).toHaveBeenCalledTimes(1);
        });
    ...
    it("should register a default config", async () => {
        ...
        function getDummyPanel(updatedConfig, childId) {
        ...
        const generatePanelSettings = (obj) => obj;
        render(_jsx(PanelSetup, { fixture: {
            ...
            }, children: _jsx(DummyPanel, { childId: childId }) }));
        await waitFor(() => {
    ...
    describe("installExtensions", () => {
        ...
        it("should throw an error when install with no registered loader to the namespace", async () => {
            const invalidNamespace = BasicBuilder.string();
            const { result } = setup();
    ...
    describe("uninstallExtension", () => {
        it("should uninstall an extension", async () => {
            ...
            const namespace = loaders[0]?.namespace;
            await act(async () => {
    ...
    describe("mergeState", () => {
        it("should merge state correctly using mergeState", async () => {
            ...
            expect(result.current.installedExtensions).toContainEqual(expect.objectContaining({ id: extensionInfo.id }));
