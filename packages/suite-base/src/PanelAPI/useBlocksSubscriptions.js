...
const filterBlockByTopics = memoizeWeak((block, topics) => {
    if (!block) {
...
const useSubscriptionsForBlocks = (subscriptions) => {
...
export function useBlocksSubscriptions(subscriptions) {
    const requestedTopics = useMemo(() => subscriptions.map((sub) => sub.topic), [subscriptions]);
    useSubscriptionsForBlocks(subscriptions);
    const allBlocks = useMessagePipeline(useCallback((ctx) => ctx.playerState.progress.messageCache?.blocks, []));
