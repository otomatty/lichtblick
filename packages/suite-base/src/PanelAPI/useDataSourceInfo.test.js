...
describe("useDataSourceInfo", () => {
    const topics = [{ name: "/foo", schemaName: "Foo" }];
    const messages = [
        {
            topic: "/foo",
    ...
    it("returns data from MessagePipelineContext", () => {
        const { result } = renderHook(() => PanelAPI.useDataSourceInfo(), {
            wrapper: ({ children }) => (_jsx(MockMessagePipelineProvider, { topics: topics, datatypes: datatypes, capabilities: ["hello"], messages: messages.slice(0, 1), startTime: { sec: 0, nsec: 1 }, children: children })),
        });
    ...
    it("doesn't change when messages change", () => {
        ...
        const { result, rerender } = renderHook(() => PanelAPI.useDataSourceInfo(), {
            wrapper: ({ children }) => (_jsx(MockMessagePipelineProvider, { topics: currentTopics, datatypes: datatypes, capabilities: capabilities, messages: currentMessages, startTime: startTime, children: children })),
        });
        expect(result.current).toEqual({
            topics: [{ name: "/foo", schemaName: "Foo" }],
            datatypes: new Map(Object.entries({ Foo: { definitions: [] } })),
