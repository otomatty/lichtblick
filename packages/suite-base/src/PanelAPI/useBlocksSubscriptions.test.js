...
describe("useBlocksSubscriptions", () => {
    ...
    it("returns no messagesByTopic when the player does not provide blocks", async () => {
        const { result } = renderHook(({ topics }) => PanelAPI.useBlocksSubscriptions(topics), {
            ...
            wrapper: ({ children }) => (_jsx(MockMessagePipelineProvider, { activeData: {}, children: children })),
        });
    ...
    it("handles uninitialized block states", async () => {
        ...
        const { result } = renderHook(({ topics }) => PanelAPI.useBlocksSubscriptions(topics), {
            initialProps: { topics: [{ topic: "/topic1" }] },
            wrapper: ({ children }) => (_jsx(MockMessagePipelineProvider, { activeData: activeData, progress: progress, children: children })),
        });
    ...
    it("handles sliced subscriptions", async () => {
        ...
        const stableSubscriptions = [{ topic: "/topic", fields: ["a"] }];
        const { result } = renderHook(({ subscriptions }) => PanelAPI.useBlocksSubscriptions(subscriptions), {
    ...
    it("maintains block identity across repeated renders", async () => {
        ...
        const stableSubscriptions = [{ topic: "/topic" }];
        const { result, rerender } = renderHook(({ subscriptions }) => PanelAPI.useBlocksSubscriptions(subscriptions), {
            initialProps: { subscriptions: stableSubscriptions },
            wrapper: ({ children }) => (_jsx(MockMessagePipelineProvider, { activeData: activeData, progress: progress, children: children })),
        });
