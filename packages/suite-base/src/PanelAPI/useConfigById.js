 ...
 */
export default function useConfigById(panelId) {
    ...
    const customSettingsByTopic = useMemo(() => {
        if (!panelId) {
            return {};
        }
        const panelType = getPanelTypeFromId(panelId);
        return _.merge({}, ...sortedTopics.map(({ name: topic, schemaName }) => {
    ...
    const configSelector = useCallback((state) => {
        if (panelId == undefined) {
            return undefined;
        }
        const stateConfig = maybeCast(state.selectedLayout?.data?.configById?.[panelId]);
        const customSettings = _.merge({}, customSettingsByTopic, stateConfig?.topics);
        // Avoid returning a new object if nothing has changed
        const hasCustomSettingsChanged = Object.keys(customSettings).length !== 0 && !_.isEqual(customSettings, stateConfig?.topics);
        ...
        return maybeCast({ ...stateConfig, topics: customSettings });
    ...
    const saveConfig = useCallback((newConfig) => {
        ...
        if (typeof newConfig === "function") {
            ...
            const currentConfig = getCurrentLayoutState().selectedLayout?.data?.configById[panelId];
            if (currentConfig) {
                savePanelConfigs({
