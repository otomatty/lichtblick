...
import { useCurrentLayoutActions, useCurrentLayoutSelector } from "./index";
/**
...
describe("useCurrentLayoutSelector", () => {
    ...
    it("updates when layout changes", () => {
        const { result } = renderHook(({ selector }) => ({
            ...
            initialProps: {
                selector: (layoutState) => layoutState.selectedLayout?.data?.configById["foo"],
            },
            wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { initialState: { configById: { foo: { value: 42 } } }, children: children }));
            },
        });
        // 初期値の確認
    ...
    it("updates when selector changes", () => {
        const { result, rerender } = renderHook(({ selector }) => ({
            actions: useCurrentLayoutActions(),
            value: useCurrentLayoutSelector(selector),
        }), {
            initialProps: {
            ...
            wrapper({ children }) {
                return (_jsx(MockCurrentLayoutProvider, { initialState: {
                        configById: {
                            foo: { value: 42 },
                            bar: { otherValue: 0 },
                        },
    ...
    it("updates when state changes before subscribe", () => {
        ...
        renderHook(({ selector }) => {
            ...
            wrapper({ children }) {
                return (_jsxs(MockCurrentLayoutProvider, { initialState: {
                        configById: {
                            foo: { value: 42 },
                            bar: { otherValue: 0 },
                        },
                    }, children: [_jsx(ChangeState, {}), children] }));
            },
        });
