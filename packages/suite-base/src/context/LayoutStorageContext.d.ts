 ...
 */
declare const LayoutStorageContext: import("react").Context<ILayoutStorage | undefined>;
/**
 * ## useLayoutStorage
 *
 ...
 * ### 使用例
 * ```typescript
 * function LayoutPersistenceComponent() {
 *   const layoutStorage = useLayoutStorage();
 *
 *   const handleSaveLayout = async (namespace: string, layout: Layout) => {
 *     try {
 *       const savedLayout = await layoutStorage.put(namespace, layout);
 *       console.log("Layout persisted:", savedLayout.id);
 *     } catch (error) {
 *       console.error("Failed to persist layout:", error);
 *     }
 *   };
 *
 *   const handleLoadLayouts = async (namespace: string) => {
 *     try {
 *       const layouts = await layoutStorage.list(namespace);
 ...
 *   return (
 *     <div>
 *       <button onClick={() => handleLoadLayouts("user-123")}>
 *         Load Layouts
 *       </button>
 *     </div>
 *   );
 ...
 */
export declare function useLayoutStorage(): ILayoutStorage;
