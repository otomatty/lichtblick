...
describe("useOpenFile", () => {
    ...
    beforeEach(() => {
        ...
        usePlayerSelection.mockReturnValue({ selectSource });
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    function buildFile(extension, type) {
        return new File([BasicBuilder.string()], `${BasicBuilder.string()}.${extension}`, {
            type,
        });
    }
    function setup({ filesOverride, sourcesOverride } = {}) {
        ...
        showOpenFilePicker.mockResolvedValue(fsHandles);
        return {
    ...
    it("should show error if multiple file extensions are selected", async () => {
        ...
        await expect(result.current()).rejects.toThrow("Multiple file extensions detected: .mcap, .txt. All files must have the same extension.");
        expect(enqueueSnackbar).toHaveBeenCalledWith(expect.stringContaining("Multiple file extensions detected"), { variant: "error" });
    });
    it("should show error if no matching source is found", async () => {
        ...
        await expect(result.current()).rejects.toThrow("Cannot find a source to handle files with extension .mcap");
        expect(enqueueSnackbar).toHaveBeenCalledWith(expect.stringContaining("Cannot find a source to handle files"), { variant: "error" });
    });
    it("should show error if multiple files are selected but not supported", async () => {
        ...
        await expect(result.current()).rejects.toThrow("The application only support multiple files for MCAP extension.");
        expect(enqueueSnackbar).toHaveBeenCalledWith(expect.stringContaining("The application only support multiple files"), { variant: "error" });
    });
});
