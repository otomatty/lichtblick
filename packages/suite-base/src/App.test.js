...
function mockProvider(testId) {
    return jest.fn(({ children }) => _jsx("div", { "data-testid": testId, children: children }));
}
...
jest.mock("./components/SendNotificationToastAdapter", () => mockProvider("send-notification-toast-adapter"));
...
const setup = (overrides = {}) => {
    ...
    return render(_jsx(App, { ...defaultProps }));
};
...
describe("App Component MultiProvider Tests", () => {
    ...
    function extractProviderTypes() {
        const props = MultiProvider.mock.calls[0][0];
        const providerTypes = props.providers.map((provider) => provider.type);
    ...
    it("verifies that AppParametersProvider is called with correct parameters", () => {
        ...
        setup({ appParameters });
        expect(screen.getByTestId("app-parameters-provider")).toBeDefined();
        const props = AppParametersProvider.mock.calls[0][0];
    ...
    it("verifies that MultiProvider has rendered all providers when it has extraProviders", () => {
        const extraProviders = [
            ...
            _jsx("div", { "data-testid": "extra-provider" }, "1"),
            _jsx("div", { "data-testid": "extra-provider" }, "2"),
        ];
