...
class MockRosClient {
    ...
    #connectCallback;
    #messages = [];
    setup({ topics = [], types = [], typedefs = [], messages = [], }) {
        this.#topics = topics;
        this.#types = types;
    ...
    getMessagesByTopicName(topicName) {
        return this.#messages.filter(({ topic }) => topic === topicName);
    }
    getNodes(callback, _errCb) {
        callback([]);
    }
    getNodeDetails(_node, callback, _errCb) {
        callback([], [], []);
    }
}
class MockRosTopic {
