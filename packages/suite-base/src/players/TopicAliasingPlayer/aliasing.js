...
const EmptyAliasMap = new Map();
function aliasBlocks(blocks, mapping) {
    ...
    return blocks.map((block) => {
        ...
        return {
            ...block,
            messagesByTopic: _.transform(block.messagesByTopic, (acc, messages, topic) => {
...
function aliasMessages(messages, mapping) {
    if (mapping === EmptyAliasMap) {
        return messages;
    }
...
function aliasPublishedTopics(topics, mapping) {
    if (mapping === EmptyAliasMap) {
        return topics;
    }
...
function aliasSubscribedTopics(topics, mapping, subcriptions) {
    if (mapping === EmptyAliasMap) {
        return topics;
    }
...
function aliasProgress(progress, mapping) {
...
function aliasTopics(topics, mapping) {
    if (mapping === EmptyAliasMap) {
        return topics;
    }
...
function aliasTopicStats(stats, mapping) {
    if (mapping === EmptyAliasMap) {
        return stats;
    }
...
function mergeAliases(maps, inputs) {
 ...
 */
export function aliasPlayerState(inputs, subscriptions, playerState) {
    const newState = {
        ...playerState,
    ...
    if (newState.activeData) {
        ...
        if (newState.activeData.publishedTopics) {
            newState.activeData.publishedTopics = memos.aliasPublishedTopics(newState.activeData.publishedTopics, mapping);
        }
        if (newState.activeData.subscribedTopics) {
            newState.activeData.subscribedTopics = memos.aliasSubscribedTopics(newState.activeData.subscribedTopics, mapping, subscriptions);
        }
