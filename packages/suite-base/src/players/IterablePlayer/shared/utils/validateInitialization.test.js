...
describe("validateInitialization", () => {
    ...
    describe("validateAndAddDatatypes", () => {
        it("should add a warning if there is a datatype mismatch", () => {
            ...
            expect(accumulated.alerts[0].message).toBe(`Different datatypes found for schema "${datatype}"`);
        });
        ...
        describe("validateAndAddTopics", () => {
            it("should add a warning if there is a schema name mismatch", () => {
                ...
                expect(accumulated.alerts).toHaveLength(1);
                expect(accumulated.alerts[0].message).toBe(`Schema name mismatch detected for topic "${topicName}". Expected "${accumulatedTopic.schemaName}", but found "${currentTopic.schemaName}".`);
            });
