...
describe("MultiIterableSource", () => {
    ...
    describe("Initialization", () => {
        ...
        it("should merge initializations, but containing alerts", async () => {
            ...
            expect(result.alerts[0].message).toBe(`Different datatypes found for schema "${dataTypeName}"`);
            expect(result.alerts[1].message).toBe(`Schema name mismatch detected for topic "${topicName}". Expected "${init1.topics[0].schemaName}", but found "${init2.topics[0].schemaName}".`);
            expect(mockSourceConstructor).toHaveBeenCalledTimes(2);
        });
    });
});
