...
import { add, clampTime, compare, fromMillis, fromNanoSec, toRFC3339String, toString, } from "@lichtblick/rostime";
...
export class IterablePlayer {
    ...
    setSubscriptions(newSubscriptions) {
        ...
        const allTopics = new Map(this.#subscriptions.map((subscription) => [subscription.topic, subscription]));
        const preloadTopics = new Map(filterMap(this.#subscriptions, (sub) => sub.preloadType === "full" ? [sub.topic, sub] : undefined));
    ...
    async #stateInitialize() {
        ...
        this.#queueEmitState();
        try {
            const { start, end, topics, profile, topicStats, alerts, metadata, publishersByTopic, datatypes, name, } = await this.#bufferedSource.initialize();
    ...
    async #stateStartPlay() {
        ...
        const stopTime = clampTime(add(this.#start, fromNanoSec(SEEK_ON_START_NS)), this.#start, this.#end);
    ...
    async #tick() {
        ...
        const durationMillis = this.#lastTickMillis != undefined && this.#lastTickMillis !== 0
        ...
        const end = clampTime(targetTime, this.#start, this.#untilTime ?? this.#end);
