...
import { loadDecompressHandlers, parseChannel } from "@lichtblick/mcap-support";
import { isLessThan, isGreaterThan, isTimeInRangeInclusive, fromNanoSec, subtract, toSec, toRFC3339String, compare, } from "@lichtblick/rostime";
...
export class McapUnindexedIterableSource {
    #options;
    #msgEventsByChannel;
    #start;
    #end;
    ...
    async initialize() {
        ...
        const alerts = [];
        const metadata = [];
        const channelIdsWithErrors = new Set();
        let messageCount = 0;
            ...
            switch (record.type) {
                ...
                case "Channel": {
                    ...
                    if (record.schemaId !== 0 && !schema) {
                        throw new Error(`Encountered channel with schema id ${record.schemaId} but no prior schema`);
                    }
                    try {
                ...
                case "Message": {
                    ...
                    const estimatedMemorySize = estimateMessageSize(channelInfo.channel.topic, deserializedMessage);
                    messages.push({
    ...
    async *messageIterator(args) {
        ...
        for (const [channelId, msgEvents] of this.#msgEventsByChannel) {
            for (const msgEvent of msgEvents) {
                if (isTimeInRangeInclusive(msgEvent.receiveTime, start, end) &&
    ...
    async getBackfillMessages(args) {
        ...
        const msgEventsByTopic = new Map();
