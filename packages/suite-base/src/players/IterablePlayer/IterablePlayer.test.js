...
class PlayerStateStore {
    ...
    async add(state) {
        ...
        if (this.#playerStates.length > this.#expected) {
            const error = new Error(`Expected: ${this.#expected} messages, received: ${this.#playerStates.length}`);
...
describe("IterablePlayer", () => {
    ...
    it("startPlayback emits when seek-backfill state is active", async () => {
        ...
        source.messageIterator = async function* messageIterator(_args) {
        ...
        // Reset store to get state from the seeks
        store.reset(1);
        const getIsPlaying = (state) => state.activeData?.isPlaying;
    ...
    it("supports seek request during initialization", async () => {
        ...
        const baseState = {
            ...
            presence: PlayerPresence.PRESENT,
            progress: {
                fullyLoadedFractionRanges: [{ start: 0.500000001, end: 1 }],
                messageCache: undefined,
            },
            urlState: {
                sourceId: "test",
