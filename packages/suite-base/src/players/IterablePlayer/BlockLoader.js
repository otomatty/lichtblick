...
import { add, clampTime, fromNanoSec, subtract as subtractTimes, toNanoSec, } from "@lichtblick/rostime";
...
export class BlockLoader {
    ...
    #alertManager;
    #stopped = false;
    #activeChangeCondvar = new Condvar();
    #abortController;
    constructor(args) {
        ...
        this.#blockDurationNanos = Math.ceil(Math.max(args.minBlockDurationNs, totalNs / args.maxBlocks));
        ...
        for (let i = 0; i < this.#blocks.length; i++) {
            ...
            if (block) {
                ...
                for (const topic of blockTopics) {
                    ...
                    const newMessages = newMessagesByTopic[topic];
    ...
    async #load(args) {
        ...
        for (let blockId = 0; blockId < this.#blocks.length; ++blockId) {
            ...
            const cursor = this.#source.getMessageCursor?.({ ...iteratorArgs, abort: this.#abortController.signal }) ??
            ...
            for (let currentBlockId = blockId; currentBlockId <= endBlockId; ++currentBlockId) {
                ...
                for (const iterResult of results) {
                    ...
                    if (totalBlockSizeBytes > this.#maxCacheSize) {
                        this.#alertManager.addAlert("cache-full", {
                            severity: "error",
                            message: `Cache is full. Preloading for topics [${Array.from(topicsToFetch.keys()).join(", ")}] has stopped on block ${currentBlockId + 1}/${this.#blocks.length}.`,
                ...
                const newBlockSizeInBytes = (existingBlock?.sizeInBytes ?? 0) - overridenBlockMessagesSize + sizeInBytes;
    ...
    #calculateProgress(topics, currentCacheSize) {
        const fullyLoadedFractionRanges = simplify(filterMap(this.#blocks, (thisBlock, blockIndex) => {
            if (!thisBlock) {
                return;
            }
