...
export class BagIterableSource {
    #source;
    #bag;
    #readersByConnectionId = new Map();
    ...
    async initialize() {
        ...
        const numMessagesByConnectionIndex = new Array(this.#bag.connections.size).fill(0);
        ...
        for (const [id, connection] of this.#bag.connections) {
            ...
            const numMessages = (topicStats.get(connection.topic)?.numMessages ?? 0) +
    ...
    async getBackfillMessages({ topics, time, }) {
        const messages = [];
