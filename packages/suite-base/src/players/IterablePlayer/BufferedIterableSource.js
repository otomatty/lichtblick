 ...
 */
class BufferedIterableSource extends EventEmitter {
    #source;
    #readDone = false;
    #aborted = false;
    ...
    async #startProducer(args) {
        ...
        try {
            ...
            let readUntil = clampTime(addTime(this.#readHead, this.#readAheadDuration), this.#initResult.start, this.#initResult.end);
            for await (const result of sourceIterator) {
                ...
                const receiveTime = result.type === "message-event" ? result.msgEvent.receiveTime : undefined;
