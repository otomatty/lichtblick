...
class CachingIterableSource extends EventEmitter {
    ...
    #loadedRangesCache = [{ start: 0, end: 0 }];
    #initResult;
    #totalSizeBytes = 0;
    ...
    #currentReadHead = { sec: 0, nsec: 0 };
    #nextBlockId = BigInt(0);
    #evictableBlockCandidates = [];
    ...
    async *messageIterator(args) {
        ...
        const findIndexContainingPredicate = (item) => {
            return compare(item.start, readHead) <= 0 && compare(item.end, readHead) >= 0;
        };
        const findAfterPredicate = (item) => {
        ...
        for (;;) {
            ...
            if (block) {
                const cacheIndex = CachingIterableSource.#FindStartCacheItemIndex(block.items, toNanoSec(readHead));
            ...
            for await (const iterResult of sourceMessageIterator) {
                ...
                if (iterResult.type === "message-event" || iterResult.type === "stamp") {
                    const receiveTime = iterResult.type === "stamp" ? iterResult.stamp : iterResult.msgEvent.receiveTime;
     ...
     */
    #findEvictableBlockCandidates(readHead) {
        if (this.#cache.length === 0) {
            return [];
        }
        ...
        const readHeadIdx = mappedCache.findIndex((block) => compare(block.start, readHead) <= 0 && compare(block.end, readHead) >= 0);
        if (readHeadIdx === -1) {
    ...
    static #FindStartCacheItemIndex(items, key) {
