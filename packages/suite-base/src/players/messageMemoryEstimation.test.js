...
import { estimateMessageObjectSize, estimateMessageFieldSizes, estimateObjectSize, } from "./messageMemoryEstimation";
describe("memoryEstimationBySchema", () => {
    ...
    it("throws an error for an unknown type", () => {
        expect(() => estimateMessageObjectSize(new Map([["foo", { definitions: [] }]]), "UnknownType", new Map())).toThrow("Type 'UnknownType' not found in definitions");
    });
    it("handles complex types with arrays of unknown primitive type", () => {
        ...
        expect(() => estimateMessageObjectSize(datatypes, "ComplexType", new Map())).toThrow("Unknown primitive type unknownType");
    });
    ...
    it.each([
    ...
    ])("matches the size of objects with int + double fields measured with chrome devtools", ({ numFloats, numInts, measuredSize, tolerancePercent }) => {
        const datatypes = new Map([
            [
                "SomeType",
                {
        ...
        expect(Math.abs(sizeInBytes - measuredSize)).toBeLessThanOrEqual(measuredSize * (tolerancePercent / 100));
    });
    it("sum of field sizes matches total object size", () => {
        ...
        const fieldSizesSum = Object.values(fieldSizes).reduce((acc, fieldSize) => acc + fieldSize, OBJECT_BASE_SIZE);
