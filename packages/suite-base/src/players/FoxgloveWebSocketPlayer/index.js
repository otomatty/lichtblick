...
import { FoxgloveClient, ServerCapability, StatusLevel, FetchAssetStatus, BinaryOpcode, } from "@foxglove/ws-protocol";
...
import { isMsgDefEqual } from "@lichtblick/message-definition";
...
import { fromMillis, fromNanoSec, isGreaterThan, isLessThan } from "@lichtblick/rostime";
...
import { CURRENT_FRAME_MAXIMUM_SIZE_BYTES, FALLBACK_PUBLICATION_ENCODING, GET_ALL_PARAMS_PERIOD_MS, GET_ALL_PARAMS_REQUEST_ID, ROS_ENCODINGS, SUBSCRIPTION_WARNING_SUPPRESSION_MS, SUPPORTED_PUBLICATION_ENCODINGS, SUPPORTED_SERVICE_ENCODINGS, ZERO_TIME, } from "./constants";
import { dataTypeToFullName, statusLevelToAlertSeverity } from "./helpers";
const log = Log.getLogger(__dirname);
const textEncoder = new TextEncoder();
const textDecoder = new TextDecoder();
export default class FoxgloveWebSocketPlayer {
    #sourceId;
    #url; // WebSocket URL.
    #name;
    #client; // The client when we're connected.
    #id = uuidv4(); // Unique ID for this player session.
    #serverCapabilities = [];
    #playerCapabilities = [];
    #supportedEncodings;
    #listener; // Listener for _emitState().
    #closed = false; // Whether the player has been completely closed using close().
    #topics; // Topics as published by the WebSocket.
    #topicsStats = new Map(); // Topic names to topic statistics.
    #datatypes = new Map(); // Datatypes as published by the WebSocket.
    ...
    #unresolvedSubscriptions = new Set();
    #resolvedSubscriptionsByTopic = new Map();
    #resolvedSubscriptionsById = new Map();
    #channelsByTopic = new Map();
    #channelsById = new Map();
    #unsupportedChannelIds = new Set();
    #recentlyCanceledSubscriptions = new Set();
    #parameters = new Map();
    #getParameterInterval;
    #openTimeout;
    #connectionAttemptTimeout;
    #unresolvedPublications = [];
    #publicationsByTopic = new Map();
    #serviceCallEncoding;
    ...
    #fetchAssetRequests = new Map();
    #fetchedAssets = new Map();
    #parameterTypeByName = new Map();
    #messageSizeEstimateByTopic = {};
    ...
    #open = () => {
        ...
        this.#client.on("serverInfo", (event) => {
            ...
            if (maybeRosDistro) {
                ...
                for (const dataType in rosDataTypes) {
                    const msgDef = rosDataTypes[dataType];
            ...
            if (event.capabilities.includes(ServerCapability.services)) {
                ...
                if (this.#serviceCallEncoding) {
                    this.#playerCapabilities = this.#playerCapabilities.concat(PLAYER_CAPABILITIES.callServices);
            ...
            if (event.capabilities.includes(ServerCapability.parameters)) {
                this.#playerCapabilities = this.#playerCapabilities.concat(PLAYER_CAPABILITIES.getParameters, PLAYER_CAPABILITIES.setParameters);
            ...
            try {
                ...
                if (this.#parsedMessagesBytes > CURRENT_FRAME_MAXIMUM_SIZE_BYTES) {
                    this.#alerts.addAlert(`webSocketPlayer:parsedMessageCacheFull`, {
                        severity: "error",
                        message: `WebSocketPlayer maximum frame size (${(CURRENT_FRAME_MAXIMUM_SIZE_BYTES / 1000000).toFixed(2)}MB) reached. Dropping old messages. This accumulation can occur if the browser tab has been inactive.`,
                    });
        ...
        this.#client.on("parameterValues", ({ parameters, id }) => {
            const mappedParameters = parameters.map((param) => {
                return param.type === "byte_array"
                    ? {
                        ...param,
                        value: Uint8Array.from(atob(param.value), (c) => c.charCodeAt(0)),
                    }
                    : param;
            });
            const parameterTypes = parameters.map((p) => [p.name, p.type]);
            const parameterTypesMap = new Map(parameterTypes);
            ...
            for (const service of services) {
                ...
                try {
                    ...
                    (service.request == undefined && service.requestSchema == undefined) ||
                        ...
                        throw new Error(`Unsupported service request message encoding. ${requestMsgEncoding} not in list of supported encodings [${supportedEncodingsStr}]`);
                    }
                    ...
                    if (service.requestSchema || service.responseSchema) {
                        this.#alerts.addAlert(serviceAlertId, {
                            ...
                            error: new Error("requestSchema and responseSchema are deprecated and will not be supported in future versions of Lichtblick"),
                        });
                    }
                }
                catch (error) {
        ...
        this.#client.on("unadvertiseServices", (serviceIds) => {
            ...
            for (const serviceId of serviceIds) {
                const service = Object.values(this.#servicesByName).find((srv) => srv.service.id === serviceId);
                if (service) {
        ...
        this.#client.on("connectionGraphUpdate", (event) => {
            if (event.publishedTopics.length > 0 || event.removedTopics.length > 0) {
                const newMap = new Map(this.#publishedTopics ?? new Map());
            ...
            if (event.subscribedTopics.length > 0 || event.removedTopics.length > 0) {
                const newMap = new Map(this.#subscribedTopics ?? new Map());
            ...
            if (event.advertisedServices.length > 0 || event.removedServices.length > 0) {
                const newMap = new Map(this.#advertisedServices ?? new Map());
    ...
    #updateTopicsAndDatatypes() {
        ...
        const topics = Array.from(this.#channelsById.values(), (chanInfo) => ({
            name: chanInfo.channel.topic,
            schemaName: chanInfo.channel.schemaName,
        }));
        // Remove stats entries for removed topics
        const topicsSet = new Set(topics.map((topic) => topic.name));
        const topicStats = new Map(this.#topicsStats);
    ...
    setPublishers(publishers) {
        ...
        const removedPublications = currentPublications.filter((channel) => {
            return (uniquePublications.find(({ topic, schemaName }) => channel.topic === topic && channel.schemaName === schemaName) == undefined);
        });
        const newPublications = uniquePublications.filter(({ topic, schemaName }) => {
            return (currentPublications.find((publication) => publication.topic === topic && publication.schemaName === schemaName) == undefined);
        });
    ...
    publish({ topic, msg }) {
        ...
        else if (ROS_ENCODINGS.includes(clientChannel.encoding) &&
    ...
    async callService(serviceName, request) {
        ...
        return await new Promise((resolve, reject) => {
            this.#serviceResponseCbs.set(serviceCallRequest.callId, (response) => {
                try {
    ...
    #updateDataTypes(datatypes) {
        let updatedDatatypes = undefined;
