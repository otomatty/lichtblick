...
import { COMPRESSED_POINTER_SIZE, OBJECT_BASE_SIZE, ARRAY_BASE_SIZE, TYPED_ARRAY_BASE_SIZE, SMALL_INTEGER_SIZE, HEAP_NUMBER_SIZE, FIELD_SIZE_BY_PRIMITIVE, MAX_NUM_FAST_PROPERTIES, } from "./constants";
 ...
 */
export function estimateMessageObjectSize(datatypes, typeName, knownTypeSizes, checkedTypes) {
    ...
    if (nonConstantFields.length > MAX_NUM_FAST_PROPERTIES) {
        ...
        const propertiesDictSize = 16 + 5 * 8 + 2 ** Math.ceil(Math.log2((nonConstantFields.length + 2) * 1.5)) * 3 * 4;
    ...
    for (const field of nonConstantFields) {
        if (field.isComplex ?? false) {
            const count = field.isArray === true
            ...
            const complexTypeObjectSize = estimateMessageObjectSize(datatypes, field.type, knownTypeSizes, (checkedTypes ?? []).concat(field.type));
 ...
 * @returns
 */
export function estimateMessageFieldSizes(datatypes, typeName, knownTypeSizes) {
    const sizeByField = {};
    datatypes.get(typeName)?.definitions.forEach((field) => {
        const fieldSchemaName = `${typeName}-${field.name}`;
        const fieldSizeInBytes = estimateMessageObjectSize(new Map([[fieldSchemaName, { name: fieldSchemaName, definitions: [field] }], ...datatypes]), fieldSchemaName, knownTypeSizes);
...
export function estimateObjectSize(obj) {
    ...
    const estimateArraySize = (array) => COMPRESSED_POINTER_SIZE +
        ARRAY_BASE_SIZE +
        array.reduce((accumulator, value) => accumulator + estimateObjectSize(value), 0);
    const estimateMapSize = (map) => COMPRESSED_POINTER_SIZE +
        OBJECT_BASE_SIZE +
        Array.from(map.entries()).reduce((accumulator, [key, value]) => accumulator + estimateObjectSize(key) + estimateObjectSize(value), 0);
    const estimateSetSize = (set) => COMPRESSED_POINTER_SIZE +
        OBJECT_BASE_SIZE +
        Array.from(set.values()).reduce((accumulator, value) => accumulator + estimateObjectSize(value), 0);
    const estimateObjectPropertiesSize = (object) => {
        const valuesSize = Object.values(object).reduce((accumulator, value) => accumulator + estimateObjectSize(value), 0);
        ...
        if (numProps > MAX_NUM_FAST_PROPERTIES) {
            ...
            return (OBJECT_BASE_SIZE + valuesSize + propertiesDictSize - numProps * COMPRESSED_POINTER_SIZE);
        }
    ...
    switch (typeof obj) {
        ...
        case "object":
            ...
            return estimateObjectPropertiesSize(obj);
        case "symbol":
        case "function":
