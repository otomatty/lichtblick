...
import { fromMillis, isGreaterThan, toSec } from "@lichtblick/rostime";
...
export default class Ros1Player {
    #url; // rosmaster URL.
    #hostname; // ROS_HOSTNAME
    #rosNode; // Our ROS node when we're connected.
    ...
    #providerTopics; // Topics as advertised by rosmaster.
    #providerTopicsStats = new Map(); // topic names to topic statistics.
    #providerDatatypes = new Map(); // All ROS message definitions received from subscriptions and set by publishers.
    #publishedTopics = new Map(); // A map of topic names to the set of publisher IDs publishing each topic.
    #subscribedTopics = new Map(); // A map of topic names to the set of subscriber IDs subscribed to each topic.
    #services = new Map(); // A map of service names to service provider IDs that provide each service.
    #parameters = new Map(); // rosparams
    #start; // The time at which we started playing.
    #clockTime; // The most recent published `/clock` time, if available
    #requestedPublishers = []; // Requested publishers by setPublishers()
    #requestedSubscriptions = []; // Requested subscriptions by setSubscriptions()
    #parsedMessages = []; // Queue of messages that we'll send in next _emitState() call.
    #requestTopicsTimeout; // setTimeout() handle for _requestTopics().
    ...
    #emitTimer;
    #sourceId;
    constructor({ url, hostname, metricsCollector, sourceId }) {
    ...
    #clearPublishAlerts({ skipEmit = false } = {}) {
        if (this.#alerts.removeAlerts((id) => id.startsWith("msgdef:") || id.startsWith("advertise:") || id.startsWith("publish:"))) {
            if (!skipEmit) {
                this.#emitState();
            }
        }
    }
    #topicsChanged = (newTopics) => {
    ...
    #requestTopics = async () => {
        ...
        try {
            ...
            const topics = topicArrays.map(([name, schemaName]) => ({
                name,
                schemaName,
            }));
            ...
            try {
                ...
                this.#addAlert(Alert.Parameters, {
                    severity: "warn",
            ...
            this.#presence = PlayerPresence.INITIALIZING;
            this.#addAlert(Alert.Connection, {
                severity: "error",
    ...
    #handleMessage = (topic, message, sizeInBytes, schemaName, 
    ...
    setPublishers(publishers) {
        ...
        const topics = new Set(validPublishers.map(({ topic }) => topic));
    ...
    #getRosDatatypes = (datatype, messageDefinition) => {
    ...
    #addInternalSubscriptions(subscriptions) {
    ...
    #handleInternalMessage(msg) {
        const maybeClockMsg = msg.message;
        if (msg.topic === "/clock" && maybeClockMsg.clock && !isNaN(maybeClockMsg.clock.sec)) {
            ...
            this.#clockTime = time;
            msg.receiveTime = this.#getCurrentTime();
        }
    }
    async #updateConnectionGraph(rosNode) {
        try {
            const graph = await rosNode.getSystemState();
            if (!_.isEqual(this.#publishedTopics, graph.publishers) ||
