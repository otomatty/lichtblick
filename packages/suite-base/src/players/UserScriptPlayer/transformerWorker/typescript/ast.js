...
import { DIAGNOSTIC_SEVERITY, SOURCES, ERROR_CODES, } from "@lichtblick/suite-base/players/UserScriptPlayer/constants";
import { noFuncError, nonFuncError, badTypeReturnError, unionsError, functionError, noTypeLiteralsError, noIntersectionTypesError, preferArrayLiteral, classError, noTypeOfError, noMappedTypes, noTuples, limitedUnionsError, noNestedAny, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/typescript/errors";
...
export const findChild = (node, kind) => ts.forEachChild(node, (child) => {
...
export const findDeclaration = (symbol, kind) => {
...
const findImportedTypeDeclaration = (checker, node, kind) => {
...
const buildTypeMapFromParams = (typeParameters = [], typeMap) => {
...
const buildTypeMapFromArgs = (typeArguments = [], typeMap) => {
    const newTypeParamMap = {};
...
const isNodeFromRosModule = (node) => {
    return node.getSourceFile().fileName.endsWith("ros/index.d.ts");
};
function maybeSymbol(node) {
    return node?.symbol;
}
export const findDefaultExportFunction = (source, checker) => {
...
export const findReturnType = (typeChecker, node) => {
    ...
    if (symbol.declarations.length === 1) {
        ...
        declaration = symbol.declarations.find((decl) => decl.kind === ts.SyntaxKind.InterfaceDeclaration);
    }
    if (!declaration) {
        throw new DatatypeExtractionError(badTypeReturnError);
    }
 ...
 */
export const constructDatatypes = (checker, node, currentDatatype, messageDefinitionMap, sourceDatatypes, depth = 1, currentTypeParamMap = {}) => {
    if (depth > MAX_DEPTH) {
    ...
    const getRosMsgField = (name, tsNode, 
    // eslint-disable-next-line @lichtblick/no-boolean-parameters
    isArray = false, 
    // eslint-disable-next-line @lichtblick/no-boolean-parameters
    isComplex = false, typeMap = {}, innerDepth = 1) => {
        ...
        switch (tsNode.kind) {
            ...
            case ts.SyntaxKind.TypeLiteral: {
                const typeLiteral = tsNode;
                const symbolName = maybeSymbol(tsNode)?.name;
                const messageDefinition = symbolName != undefined ? messageDefinitionMap[symbolName] : undefined;
                const nestedType = isNodeFromRosModule(typeLiteral) && messageDefinition != undefined
                    ? messageDefinition
                ...
                const typeParamMap = ts.isInterfaceDeclaration(tsNode)
                    ...
                    : typeMap;
                const { datatypes: nestedDatatypes } = constructDatatypes(checker, typeLiteral, nestedType, messageDefinitionMap, sourceDatatypes, depth + 1, typeParamMap);
            ...
            case ts.SyntaxKind.TypeAliasDeclaration: {
                const typeAlias = tsNode;
                const newTypeParamMap = buildTypeMapFromParams(typeAlias.typeParameters, typeMap);
                return getRosMsgField(name, typeAlias.type, isArray, isComplex, newTypeParamMap, innerDepth + 1);
            }
            case ts.SyntaxKind.TypeReference: {
                const typeRef = tsNode;
                ...
                return getRosMsgField(name, nextNode, isArray, isComplex, buildTypeMapFromArgs(typeRef.typeArguments, typeMap), innerDepth + 1);
            }
            // i.e. 'typeof'
    ...
    const rosMsgFields = members.map((member) => {
        ...
        if (!ts.isPropertySignature(member)) {
            throw new DatatypeExtractionError({
                severity: DIAGNOSTIC_SEVERITY.Error,
                message: `Unexpected type member (kind ${member.kind}) in ${interfaceName ?? currentDatatype}`,
                source: SOURCES.DatatypeExtraction,
                code: ERROR_CODES.DatatypeExtraction.INVALID_PROPERTY,
            });
        }
        if (!member.type) {
            throw new DatatypeExtractionError({
                severity: DIAGNOSTIC_SEVERITY.Error,
                message: `Member ${member.name.getText()} has no type in ${interfaceName ?? currentDatatype}`,
                source: SOURCES.DatatypeExtraction,
                code: ERROR_CODES.DatatypeExtraction.INVALID_PROPERTY,
            });
        }
        return getRosMsgField(member.name.getText(), member.type, false, false, currentTypeParamMap, depth + 1);
    });
    return {
