...
function getFieldOffsetsAndReaders(fields) {
    const result = [];
 ...
 */
export const readPoints = (message) => {
...
export function setRayDistance(pt, distance) {
    const { x, y, z } = pt;
...
export function convertToRangeView(points, range, makeColors) {
    const colors = makeColors ? new Array(points.length) : [];
