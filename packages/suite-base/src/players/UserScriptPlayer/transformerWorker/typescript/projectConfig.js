...
import { lib_dts, lib_filename, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/typescript/lib";
import rawUserUtils from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/typescript/rawUserUtils";
import { ros_lib_dts, ros_lib_filename, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/typescript/ros";
 ...
 */
export function generateFoxgloveSchemaDeclarations() {
    const schemas = _.sortBy([...exportTypeScriptSchemas({ includeTypedArrays: true }).entries()], ([name]) => name);
...
const utilityFiles = rawUserUtils.map((utility) => ({
    ...utility,
    filePath: `${DEFAULT_STUDIO_SCRIPT_PREFIX}${utility.fileName}`,
}));
export function getUserScriptProjectConfig() {
    const declarations = [];
    declarations.push({
