...
import generateRosLib, { generateTypeDefs, typedArrayMap, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/generateRosLib";
...
const formatTypeDef = (interfaceDeclarations) => {
    const resultFile = ts.createSourceFile("testFile.ts", "", ts.ScriptTarget.Latest, 
    /*setParentNodes*/
...
describe("typegen", () => {
    describe("generateTypeDefs", () => {
        describe("basic types", () => {
            it("multiple properties", () => {
                const declarations = generateTypeDefs(new Map(Object.entries({
                    "std_msgs/ColorRGBA": {
                        definitions: [
    ...
    describe("generateRosLib", () => {
        ...
        it("more complex snapshot", () => {
            const randomTopics = Array.from(stressTestDatatypes.keys()).map((schemaName, i) => ({
                name: `/topic_${i}`,
                schemaName,
            }));
