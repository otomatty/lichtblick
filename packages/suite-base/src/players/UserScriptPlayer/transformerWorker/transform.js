...
import { constructDatatypes, DatatypeExtractionError, findDefaultExportFunction, findReturnType, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/typescript/ast";
...
import { baseCompilerOptions, transformDiagnosticToMarkerData, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/utils";
...
export const hasTransformerErrors = (scriptData) => scriptData.diagnostics.some(({ severity }) => severity === DIAGNOSTIC_SEVERITY.Error);
export const getInputTopics = (scriptData) => {
    const { sourceFile, typeChecker } = scriptData;
    if (!sourceFile || !typeChecker) {
        const error = {
            severity: DIAGNOSTIC_SEVERITY.Error,
            message: "Either the 'sourceFile' or 'typeChecker' is absent. There is a problem with the `compile` step.",
    ...
    if (inputTopicElements.some(({ kind }) => kind !== ts.SyntaxKind.StringLiteral)) {
        const error = {
            severity: DIAGNOSTIC_SEVERITY.Error,
            message: "The exported 'inputs' variable must be an array of string literals. E.g. 'export const inputs = ['/some_topics']'",
            source: SOURCES.InputTopicsChecker,
            code: ERROR_CODES.InputTopicsChecker.BAD_INPUTS_TYPE,
        };
        return {
...
export const getOutputTopic = (scriptData) => {
    const matches = /^\s*export\s+const\s+output\s*=\s*("([^"]+)"|'([^']+)')/gm.exec(scriptData.sourceCode);
...
export const validateInputTopics = (scriptData, topics) => {
...
export const compile = (scriptData) => {
    const { sourceCode, rosLib, typesLib } = scriptData;
    const options = baseCompilerOptions;
...
export const extractGlobalVariables = (scriptData) => {
...
export const extractDatatypes = (scriptData) => {
    // Do not attempt to run if there were any compile time errors.
    if (hasTransformerErrors(scriptData)) {
        return scriptData;
    }
    ...
    try {
        ...
        const { outputDatatype, datatypes } = constructDatatypes(typeChecker, typeNode, name, messageDefinitionMap, sourceDatatypes);
...
const transform = (args) => {
    ...
    const transformer = compose(getOutputTopic, compile, getInputTopics, validateInputTopics, extractDatatypes, extractGlobalVariables);
