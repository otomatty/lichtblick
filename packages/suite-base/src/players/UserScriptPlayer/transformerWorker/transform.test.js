...
import { generateEmptyTypesLib, generateTypesLib, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/generateTypesLib";
import { getOutputTopic, validateInputTopics, compile, extractDatatypes, extractGlobalVariables, compose, getInputTopics, } from "@lichtblick/suite-base/players/UserScriptPlayer/transformerWorker/transform";
...
describe("pipeline", () => {
    ...
    describe("validateInputTopics", () => {
        it.each([
            ...
            const { diagnostics } = validateInputTopics({ ...baseNodeData, inputTopics }, topics.map((name) => ({ name, schemaName: "" })));
            expect(diagnostics.length).toEqual(1);
            expect(diagnostics[0]?.severity).toEqual(DIAGNOSTIC_SEVERITY.Error);
    ...
    describe("compile", () => {
        it.each(["const x: string = 'hello foxglove'", "const num: number = 1222"])("can compile", (sourceCode) => {
    ...
    describe("compile + extractDatatypes", () => {
        ...
        const numDataType = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "num",
        ...
        const uint32DataType = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "val",
        ...
        const posDatatypes = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "pos",
        ...
        const pointDataType = new Map(Object.entries({
            [baseNodeData.name]: pointFields,
        }));
        const nestedPointDataType = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "point",
        ...
        const poseDataType = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "position",
        ...
        const timeDatatypes = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        arrayLength: undefined,
        ...
        const baseDatatypesWithNestedColor = new Map([
            ...basicDatatypes,
            [
                baseNodeData.name,
                {
        ...
        const posArrayDatatypes = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "pos",
        ...
        const mixedDatatypes = new Map(Object.entries({
            [baseNodeData.name]: {
                definitions: [
                    {
                        name: "details",
        ...
        const testCases = [
          ...
          export default publisher;`,
            ...
            {
                description: "Rejects wrong kind of TypedArray when output datatype is from @foxglove/schemas",
                sourceCode: `
          import { LaserScan } from "@foxglove/schemas";
          ...
          export default publisher;`,
                error: 2322,
                errorMessage: expect.stringContaining(`Type 'Uint32Array' is not assignable to type 'number[] | Float32Array | Float64Array'`),
            },
            {
        ...
        describe("extracts datatypes from the return type of the publisher", () => {
            ...
            filteredTestCases.forEach(({ description, sourceCode, datatypes = new Map(), error, errorMessage, outputDatatype, rosLib, }) => {
