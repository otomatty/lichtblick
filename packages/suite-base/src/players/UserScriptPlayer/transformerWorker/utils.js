...
const mapCategoryToDiagnosticSeverity = (category) => {
    switch (category) {
...
export const transformDiagnosticToMarkerData = (diagnostic) => {
    ...
    const { line: startLineNumber, character: startColumn } = diagnostic.file.getLineAndCharacterOfPosition(diagnostic.start);
    const { line: endLineNumber, character: endColumn } = diagnostic.file.getLineAndCharacterOfPosition(diagnostic.start + diagnostic.length);
    return {
