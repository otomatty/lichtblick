...
const createProperty = (name, type) => {
    return ts.factory.createPropertySignature(undefined /* modifiers */, name /* name */, undefined /* questionOrExclamationToken */, type /* type */);
};
const createTimeInterfaceDeclaration = (name) => {
    return ts.factory.createInterfaceDeclaration(modifiers /* modifiers */, name /* name */, undefined /* typeParameters */, undefined /* heritageClauses */, [
...
const jsonInterfaceDeclaration = ts.factory.createInterfaceDeclaration(modifiers, 
/* modifiers */
"json", 
/* name */
undefined, 
...
/* heritageClauses */
[]);
export const formatInterfaceName = (type) => type.replace(/\//g, "__");
...
export const generateTypeDefs = (datatypes) => {
    ...
    for (const [datatype, definition] of datatypes) {
        ...
        interfaceDeclarations[datatype] = ts.factory.createInterfaceDeclaration([ts.factory.createModifier(ts.SyntaxKind.ExportKeyword)] /* modifiers */, formatInterfaceName(datatype) /* name */, undefined /* typeParameters */, undefined /* heritageClauses */, typeMembers /* members */);
    }
    return interfaceDeclarations;
};
// Creates the entire ros.d.ts declaration file.
const generateRosLib = ({ topics, datatypes, }) => {
    let TopicsToMessageDefinition = ts.factory.createInterfaceDeclaration(modifiers, "TopicsToMessageDefinition", undefined, undefined, []);
    const typedMessage = ts.factory.createInterfaceDeclaration(modifiers, 
    /* modifiers */
    "Input", 
    /* name */
    [
        ts.factory.createTypeParameterDeclaration([], "T", ts.factory.createTypeOperatorNode(ts.SyntaxKind.KeyOfKeyword, ts.factory.createTypeReferenceNode(TopicsToMessageDefinition.name))),
    ], 
    /* typeParameters */
    undefined, 
    /* heritageClauses */
    ...
    topics.forEach(({ name, schemaName }) => {
        ...
        TopicsToMessageDefinition = ts.factory.updateInterfaceDeclaration(TopicsToMessageDefinition, 
        /* node */
        modifiers, 
        /* modifiers */
        TopicsToMessageDefinition.name, undefined, 
        /* typeParameters */
        undefined, 
        /* heritageClauses */
        [
            ...TopicsToMessageDefinition.members,
            createProperty(ts.factory.createStringLiteral(name), ts.factory.createTypeReferenceNode(`${DATATYPES_IDENTIFIER}.${formatInterfaceName(schemaName)}`)),
        ]);
    });
    const datatypesNamespace = ts.factory.createModuleDeclaration(modifiers, 
    /* modifiers */
    ts.factory.createIdentifier(DATATYPES_IDENTIFIER), ts.factory.createModuleBlock(Object.values(datatypeInterfaces).map((val) => {
        return val;
    })), ts.NodeFlags.Namespace);
    const sourceFile = ts.createSourceFile("", // This argument doesn't really matter.
    "", 
    /* sourceText */
