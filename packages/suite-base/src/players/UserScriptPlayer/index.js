...
class UserScriptPlayer {
    ...
    #lastMessageByInputTopic = new Map();
    #userScriptIdsNeedUpdate = new Set();
    #protectedState = new MutexLocked({
        userScripts: {},
        scriptRegistrations: [],
    ...
    constructor(player, userScriptActions, perfRegistry) {
        this.#player = player;
        ...
        this.#setUserScriptDiagnostics = (scriptId, diagnostics) => {
            setUserScriptDiagnostics(scriptId, diagnostics);
        };
        this.#addUserScriptLogs = (scriptId, logs) => {
            if (logs.length > 0) {
    ...
    #getTopics = memoizeWeak((topics, scriptTopics) => [
        ...topics,
        ...scriptTopics,
    ]);
    #getDatatypes = memoizeWeak((datatypes, scriptDatatypes) => {
        return scriptDatatypes.reduce((allDatatypes, userScriptDatatypes) => new Map([...allDatatypes, ...userScriptDatatypes]), new Map([...basicDatatypes, ...datatypes]));
    });
    #lastBlockRequest = { result: [] };
    // Processes input messages through scripts to create messages on output topics
    async #getMessages(inputMessages, globalVariables, scriptRegistrations) {
        ...
        for (const message of inputMessages) {
            ...
            for (const scriptRegistration of scriptRegistrations) {
                if (this.#scriptSubscriptions[scriptRegistration.output.name] &&
    ...
    async #getBlocks(blocks, globalVariables, scriptRegistrations) {
        if (shallowequal(this.#lastBlockRequest.input, {
            blocks,
            globalVariables,
            scriptRegistrations,
        })) {
        ...
        const fullRegistrations = scriptRegistrations.filter((reg) => this.#scriptSubscriptions[reg.output.name]?.preloadType === "full");
            ...
            for (const scriptRegistration of fullRegistrations) {
                ...
                for (const message of blockMessages) {
                    if (scriptRegistration.inputs.includes(message.topic)) {
                        const outputMessage = await scriptRegistration.processBlockMessage(message, globalVariables);
                        if (outputMessage) {
    ...
    async setUserScripts(userScripts) {
    ...
    async #createScriptRegistration(scriptId, userScript, state, rosLib, typesLib) {
        ...
        const scriptDatatypes = new Map([...basicDatatypes, ...datatypes]);
        ...
        const transformWorker = this.#getTransformWorker();
        const scriptData = await transformWorker.send("transform", transformMessage);
        ...
        const buildMessageProcessor = () => {
            ...
            let rpc;
            const registration = async (msgEvent, globalVariables) => {
                ...
                if (!rpc) {
                    ...
                    if (!rpc) {
                        const worker = _a.CreateRuntimeWorker();
                    ...
                    const { error, userScriptDiagnostics, userScriptLogs } = await rpc.send("registerScript", {
                        projectCode,
                ...
                const result = await rpc.send("processMessage", {
                    message: {
    ...
    #getTransformWorker() {
        if (!this.#transformRpc) {
            const worker = _a.CreateTransformWorker();
    ...
    async #resetWorkersUnlocked(state) {
        ...
        const allScriptRegistrations = await Promise.all(Object.entries(state.userScripts).map(async ([scriptId, userScript]) => await this.#createScriptRegistration(scriptId, userScript, state, rosLib, typesLib)));
        const validScriptRegistrations = [];
        const playerTopics = new Set(state.lastPlayerStateActiveData.topics.map((topic) => topic.name));
        const allScriptOutputs = new Set(allScriptRegistrations.map(({ scriptData }) => scriptData.outputTopic));
        ...
        const scriptDatatypes = state.scriptRegistrations.map(({ scriptData: { datatypes } }) => datatypes);
        ...
        if (changedTopicsRequireEmitState && this.#playerState?.activeData) {
            const newTopics = _.unionBy(this.#playerState.activeData.topics, this.#memoizedScriptTopics, (top) => top.name);
            const newDatatypes = this.#getDatatypes(this.#playerState.activeData.datatypes, this.#memoizedScriptDatatypes);
            this.#playerState = {
                ...this.#playerState,
                activeData: {
    ...
    async #onPlayerState(playerState) {
        try {
            ...
            try {
                ...
                const $timer = __addDisposableResource(env_1, this.#totalTimeMetric
                ...
                const newPlayerState = await this.#protectedState.runExclusive(async (state) => {
                    if (!state.lastPlayerStateActiveData) {
                        ...
                        let shouldReset = activeData.lastSeekTime !== state.lastPlayerStateActiveData.lastSeekTime;
                        // When topics or datatypes change we also need to re-build the scripts so we clear the cache
                        if (activeData.topics !== state.lastPlayerStateActiveData.topics ||
                     ...
                     */
                    const inputTopicsForRecompute = new Set();
                    for (const userScriptId of this.#userScriptIdsNeedUpdate) {
                        const scriptRegistration = state.scriptRegistrations.find(({ scriptId }) => scriptId === userScriptId);
                    ...
                    const computed = await this.#getMessages(messages, globalVariables, state.scriptRegistrations);
                    // These are messages generated from previously saved messages on input topics
                    const recomputed = await this.#getMessages(messagesForRecompute, globalVariables, state.scriptRegistrations);
                    ...
                    if (playerProgress.messageCache) {
                        const newBlocks = await this.#getBlocks(playerProgress.messageCache.blocks, globalVariables, state.scriptRegistrations);
    ...
    #setSubscriptionsUnlocked(subscriptions, state) {
        this.#scriptSubscriptions = getPreloadTypes(subscriptions);
        this.#player.setSubscriptions(remapVirtualSubscriptions(subscriptions, state.inputsByOutputTopic));
    }
    close = () => {
...
_a = UserScriptPlayer;
export default UserScriptPlayer;
