...
export const containsFuncDeclaration = (args) => {
...
export const stringifyFuncsInObject = (arg) => {
    if (typeof arg === "function") {
        return `${arg}`;
    }
    else if (typeof arg === "object" && arg != undefined) {
...
// Exported for tests.
export const requireImplementation = (id, projectCode) => {
    ...
    for (const [file, source] of projectCode.entries()) {
        if (requestedFile.endsWith(file)) {
            const sourceExports = {};
            const require = (reqId) => requireImplementation(reqId, projectCode);
...
export const registerScript = ({ scriptCode, projectCode, }) => {
    const userScriptLogs = [];
    const userScriptDiagnostics = [];
    self.log = function (...args) {
        ...
        if (containsFuncDeclaration(args)) {
            ...
            throw new Error(`Cannot invoke log() with a function argument (registerScript) - log(${argsToPrint.map((arg) => JSON.stringify(arg)).join(", ")})`);
        }
        userScriptLogs.push(...args.map((value) => ({ source: "registerScript", value })));
    };
    try {
        const nodeExports = {};
        const require = (id) => requireImplementation(id, projectCode);
...
export const processMessage = ({ message, globalVariables, }) => {
    const userScriptLogs = [];
    const userScriptDiagnostics = [];
    self.log = function (...args) {
        // recursively check that args do not contain a function declaration
        if (containsFuncDeclaration(args)) {
            const argsToPrint = getArgsToPrint(args);
            throw new Error(`Cannot invoke log() with a function argument (processMessage) - log(${argsToPrint.map((arg) => JSON.stringify(arg)).join(", ")})`);
        }
        userScriptLogs.push(...args.map((value) => ({ source: "processMessage", value })));
    };
    try {
