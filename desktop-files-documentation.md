# Lichtblick デスクトップ版ファイル構成ドキュメント

## プロジェクト概要

Lichtblickは自動車データ可視化のためのElectronベースのデスクトップアプリケーションです。プロジェクトはワークスペース構成を使用し、デスクトップ版とWeb版を含む複数のパッケージで構成されています。

- **プロジェクト名**: Lichtblick
- **バージョン**: 1.16.0
- **技術スタック**: Electron, TypeScript, React, Webpack
- **ライセンス**: MPL-2.0
